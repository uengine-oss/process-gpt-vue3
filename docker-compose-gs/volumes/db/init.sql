...
BEGIN
    ...
    v_ip := NULLIF(btrim(COALESCE(p_metadata->>'client_ip', '')), '');
    IF v_ip IS NULL AND v_headers ? 'x-forwarded-for' THEN
        v_ip := btrim(split_part(v_headers->>'x-forwarded-for', ',', 1));
    ELSIF v_ip IS NULL AND v_headers ? 'x-real-ip' THEN
...
-- ============================================================================
-- 에이전트 지식 히스토리/초기 셋업 로그 (GS 모드 parity)
-- ============================================================================
