...
2) Caddy 실행 (`docker-compose-gs` 폴더 기준)

```powershell
caddy run --config .\Caddyfile.kong.local
```

3) `.env` 최소 설정 (`localhost` 대신 서버 IP 권장)

```bash
SITE_URL=http://<SERVER_IP>:8088
API_EXTERNAL_URL=http://<SERVER_IP>:54322
SUPABASE_PUBLIC_URL=http://<SERVER_IP>:54322
KONG_TRUSTED_IPS=0.0.0.0/0,::/0
KONG_REAL_IP_FROM=0.0.0.0/0,::/0
KONG_REAL_IP_HEADER=X-Forwarded-For
```
...
```sql
select created_at, email, ip_address, metadata->'headers'->>'x-forwarded-for' as xff
from public.auth_login_audit
order by created_at desc
limit 10;
```

> 참고: 같은 PC에서 접속하면 `::1` 또는 동일 NAT IP로 보일 수 있습니다. 클라이언트 IP 분리는 다른 기기에서 검증하세요.
