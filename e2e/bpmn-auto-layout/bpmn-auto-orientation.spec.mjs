...
async function setAutoOrientationMode(page, mode) {
    await page.addInitScript(([key, value]) => window.localStorage.setItem(key, value), [AUTO_ORIENTATION_MODE_KEY, mode]);
}
...
test.describe('process-gpt bpmn auto orientation on viewport ratio change', () => {
    ...
    test('auto rotates by viewport ratio and relayouts after rotation', async ({ page }) => {
        ...
        expect(
            Math.max(landscapeAgainFit.widthFillRatio, landscapeAgainFit.heightFillRatio),
            JSON.stringify(landscapeAgainFit)
