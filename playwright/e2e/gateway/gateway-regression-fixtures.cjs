...
function createGatewayRegressionBpmnXml(scenario) {
    ...
    const branchTasks = branchIds.map((id) => {
    ...
    const splitFlows = branchIds.map((id, index) => {
        const props = scenario.splitType === 'parallel'
            ? {}
    ...
    const branchOutFlows = branchIds.map((id) => `
    ...
    <bpmn:sequenceFlow id="flow_merge_follow_split" sourceRef="merge_task" targetRef="follow_split_gateway">${extensionJson({})}</bpmn:sequenceFlow>
    ${followIds.map((id, index) => {
        const props = scenario.afterMerge.splitType === 'parallel'
            ? {}
            : { condition: branchConditions[index + 1], conditionMode: 'text', priority: index + 1 };
        return `<bpmn:sequenceFlow id="flow_follow_split_${id}" sourceRef="follow_split_gateway" targetRef="${id}">${extensionJson(props)}</bpmn:sequenceFlow>`;
    }).join('\n    ')}
    ${followIds.map((id) => `<bpmn:sequenceFlow id="flow_${id}_final" sourceRef="${id}" targetRef="final_task">${extensionJson({})}</bpmn:sequenceFlow>`).join('\n    ')}
    ...
    const branchOutEdges = branchIds.map((id, index) => {
    ...
    const afterMergeShapes = scenario.afterMerge ? `
      <bpmndi:BPMNShape id="follow_split_gateway_di" bpmnElement="follow_split_gateway" isMarkerVisible="true"><dc:Bounds x="930" y="${centerY - 25}" width="50" height="50" /></bpmndi:BPMNShape>
      ${followIds.map((id, index) => `<bpmndi:BPMNShape id="${id}_di" bpmnElement="${id}"><dc:Bounds x="1040" y="${branchY(index)}" width="120" height="64" /></bpmndi:BPMNShape>`).join('\n      ')}
      <bpmndi:BPMNShape id="final_task_di" bpmnElement="final_task"><dc:Bounds x="1260" y="${centerY - 40}" width="120" height="80" /></bpmndi:BPMNShape>` : '';
    const afterMergeEdges = scenario.afterMerge ? `
      ...
      ${followIds.map((id, index) => `<bpmndi:BPMNEdge id="flow_follow_split_${id}_di" bpmnElement="flow_follow_split_${id}"><di:waypoint x="980" y="${centerY}" /><di:waypoint x="1010" y="${centerY}" /><di:waypoint x="1010" y="${branchY(index) + 32}" /><di:waypoint x="1040" y="${branchY(index) + 32}" /></bpmndi:BPMNEdge>`).join('\n      ')}
      ${followIds.map((id, index) => `<bpmndi:BPMNEdge id="flow_${id}_final_di" bpmnElement="flow_${id}_final"><di:waypoint x="1160" y="${branchY(index) + 32}" /><di:waypoint x="1210" y="${branchY(index) + 32}" /><di:waypoint x="1210" y="${centerY}" /><di:waypoint x="1260" y="${centerY}" /></bpmndi:BPMNEdge>`).join('\n      ')}
...
<bpmn:definitions xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns:bpmn="http://www.omg.org/spec/BPMN/20100524/MODEL" xmlns:bpmndi="http://www.omg.org/spec/BPMN/20100524/DI" xmlns:uengine="http://uengine" xmlns:dc="http://www.omg.org/spec/DD/20100524/DC" xmlns:di="http://www.omg.org/spec/DD/20100524/DI" id="Definitions_${processId}" targetNamespace="http://bpmn.io/schema/bpmn" exporter="bpmn-js" exporterVersion="17.9.1">
  ...
  <bpmn:process id="${processId}" name="${escapeXml(scenario.name)}" isExecutable="true">
    ...
    ${branchOutFlows}
    ${joinFlowXml}
    ${scenario.afterMerge ? afterMergeFlowsXml : '<bpmn:sequenceFlow id="flow_merge_end" sourceRef="merge_task" targetRef="end">' + extensionJson({}) + '</bpmn:sequenceFlow>'}
  </bpmn:process>
  <bpmndi:BPMNDiagram id="BPMNDiagram_1">
    <bpmndi:BPMNPlane id="BPMNPlane_1" bpmnElement="Collaboration_${processId}">
      ...
      <bpmndi:BPMNShape id="start_di" bpmnElement="start"><dc:Bounds x="130" y="${centerY - 18}" width="36" height="36" /></bpmndi:BPMNShape>
      <bpmndi:BPMNShape id="initial_di" bpmnElement="initial"><dc:Bounds x="210" y="${centerY - 40}" width="110" height="80" /></bpmndi:BPMNShape>
      <bpmndi:BPMNShape id="split_gateway_di" bpmnElement="split_gateway" isMarkerVisible="true"><dc:Bounds x="332" y="${centerY - 25}" width="50" height="50" /></bpmndi:BPMNShape>
      ${branchShapes}
      ${joinShape}
      <bpmndi:BPMNShape id="merge_task_di" bpmnElement="merge_task"><dc:Bounds x="780" y="${centerY - 40}" width="120" height="80" /></bpmndi:BPMNShape>
      ${afterMergeShapes}
      <bpmndi:BPMNShape id="end_di" bpmnElement="end"><dc:Bounds x="${endX}" y="${centerY - 18}" width="36" height="36" /></bpmndi:BPMNShape>
      ...
      ${branchOutEdges}
      ${joinEdge}
      ${scenario.afterMerge ? afterMergeEdges : `<bpmndi:BPMNEdge id="flow_merge_end_di" bpmnElement="flow_merge_end"><di:waypoint x="900" y="${centerY}" /><di:waypoint x="960" y="${centerY}" /></bpmndi:BPMNEdge>`}
    </bpmndi:BPMNPlane>
  </bpmndi:BPMNDiagram>
</bpmn:definitions>`;
}
...
<bpmn:definitions xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns:bpmn="http://www.omg.org/spec/BPMN/20100524/MODEL" xmlns:bpmndi="http://www.omg.org/spec/BPMN/20100524/DI" xmlns:uengine="http://uengine" xmlns:dc="http://www.omg.org/spec/DD/20100524/DC" xmlns:di="http://www.omg.org/spec/DD/20100524/DI" id="Definitions_${processId}" targetNamespace="http://bpmn.io/schema/bpmn" exporter="bpmn-js" exporterVersion="17.9.1">
  ...
  <bpmn:process id="${processId}" name="${escapeXml(scenario.name)}" isExecutable="true">
    ...
    <bpmn:userTask id="branch_1" name="branch_1">${extensionJson({ role: { name: 'tester' }, tool: 'formHandler:branch_1' })}<bpmn:incoming>flow_split_branch_1</bpmn:incoming><bpmn:outgoing>flow_branch_1_end</bpmn:outgoing></bpmn:userTask>
    <bpmn:userTask id="branch_2" name="branch_2">${extensionJson({ role: { name: 'tester' }, tool: 'formHandler:branch_2' })}<bpmn:incoming>flow_split_branch_2</bpmn:incoming><bpmn:outgoing>flow_branch_2_end</bpmn:outgoing></bpmn:userTask>
    <bpmn:userTask id="branch_3" name="branch_3">${extensionJson({ role: { name: 'tester' }, tool: 'formHandler:branch_3' })}<bpmn:incoming>flow_split_branch_3</bpmn:incoming><bpmn:outgoing>flow_branch_3_nested_split</bpmn:outgoing></bpmn:userTask>
    <bpmn:userTask id="branch_4" name="branch_4">${extensionJson({ role: { name: 'tester' }, tool: 'formHandler:branch_4' })}<bpmn:incoming>flow_split_branch_4</bpmn:incoming><bpmn:outgoing>flow_branch_4_end</bpmn:outgoing></bpmn:userTask>
    <bpmn:inclusiveGateway id="nested_split_gateway" name="nested inclusive split">${extensionJson({ role: { name: 'tester' } })}<bpmn:incoming>flow_branch_3_nested_split</bpmn:incoming><bpmn:outgoing>flow_nested_split_nested_branch_1</bpmn:outgoing><bpmn:outgoing>flow_nested_split_nested_branch_2</bpmn:outgoing><bpmn:outgoing>flow_nested_split_nested_branch_3</bpmn:outgoing></bpmn:inclusiveGateway>
    <bpmn:userTask id="nested_branch_1" name="nested_branch_1">${extensionJson({ role: { name: 'tester' }, tool: 'formHandler:nested_branch_1' })}<bpmn:incoming>flow_nested_split_nested_branch_1</bpmn:incoming><bpmn:outgoing>flow_nested_branch_1_nested_merge</bpmn:outgoing></bpmn:userTask>
    <bpmn:userTask id="nested_branch_2" name="nested_branch_2">${extensionJson({ role: { name: 'tester' }, tool: 'formHandler:nested_branch_2' })}<bpmn:incoming>flow_nested_split_nested_branch_2</bpmn:incoming><bpmn:outgoing>flow_nested_branch_2_nested_merge</bpmn:outgoing></bpmn:userTask>
    <bpmn:userTask id="nested_branch_3" name="nested_branch_3">${extensionJson({ role: { name: 'tester' }, tool: 'formHandler:nested_branch_3' })}<bpmn:incoming>flow_nested_split_nested_branch_3</bpmn:incoming><bpmn:outgoing>flow_nested_branch_3_nested_merge</bpmn:outgoing></bpmn:userTask>
    <bpmn:userTask id="nested_merge_task" name="nested merge task">${extensionJson({ role: { name: 'tester' }, tool: 'formHandler:nested_merge' })}<bpmn:incoming>flow_nested_branch_1_nested_merge</bpmn:incoming><bpmn:incoming>flow_nested_branch_2_nested_merge</bpmn:incoming><bpmn:incoming>flow_nested_branch_3_nested_merge</bpmn:incoming><bpmn:outgoing>flow_nested_merge_end</bpmn:outgoing></bpmn:userTask>
    <bpmn:endEvent id="end" name="end">${extensionJson({ role: { name: 'tester' } })}<bpmn:incoming>flow_branch_1_end</bpmn:incoming><bpmn:incoming>flow_branch_2_end</bpmn:incoming><bpmn:incoming>flow_branch_4_end</bpmn:incoming><bpmn:incoming>flow_nested_merge_end</bpmn:incoming></bpmn:endEvent>
    <bpmn:sequenceFlow id="flow_start_initial" sourceRef="start" targetRef="initial">${extensionJson({})}</bpmn:sequenceFlow>
    <bpmn:sequenceFlow id="flow_initial_split" sourceRef="initial" targetRef="split_gateway">${extensionJson({})}</bpmn:sequenceFlow>
    ...
    <bpmn:sequenceFlow id="flow_branch_3_nested_split" sourceRef="branch_3" targetRef="nested_split_gateway">${extensionJson({})}</bpmn:sequenceFlow>
    <bpmn:sequenceFlow id="flow_branch_4_end" sourceRef="branch_4" targetRef="end">${extensionJson({})}</bpmn:sequenceFlow>
    ${[1, 2, 3].map((i) => `<bpmn:sequenceFlow id="flow_nested_split_nested_branch_${i}" sourceRef="nested_split_gateway" targetRef="nested_branch_${i}">${extensionJson({ condition: branchConditions[i], conditionMode: 'text', priority: i })}</bpmn:sequenceFlow>`).join('\n    ')}
    ${[1, 2, 3].map((i) => `<bpmn:sequenceFlow id="flow_nested_branch_${i}_nested_merge" sourceRef="nested_branch_${i}" targetRef="nested_merge_task">${extensionJson({})}</bpmn:sequenceFlow>`).join('\n    ')}
  ...
  <bpmndi:BPMNDiagram id="BPMNDiagram_1"><bpmndi:BPMNPlane id="BPMNPlane_1" bpmnElement="Collaboration_${processId}">
    ...
    ${[1, 2, 3, 4].map((i) => `<bpmndi:BPMNShape id="branch_${i}_di" bpmnElement="branch_${i}"><dc:Bounds x="470" y="${110 + i * 90}" width="110" height="64" /></bpmndi:BPMNShape>`).join('\n    ')}
    ...
    ${[1, 2, 3].map((i) => `<bpmndi:BPMNShape id="nested_branch_${i}_di" bpmnElement="nested_branch_${i}"><dc:Bounds x="790" y="${185 + i * 90}" width="130" height="64" /></bpmndi:BPMNShape>`).join('\n    ')}
...
<bpmn:definitions xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns:bpmn="http://www.omg.org/spec/BPMN/20100524/MODEL" xmlns:bpmndi="http://www.omg.org/spec/BPMN/20100524/DI" xmlns:uengine="http://uengine" xmlns:dc="http://www.omg.org/spec/DD/20100524/DC" xmlns:di="http://www.omg.org/spec/DD/20100524/DI" id="Definitions_${processId}" targetNamespace="http://bpmn.io/schema/bpmn" exporter="bpmn-js" exporterVersion="17.9.1">
  ...
  <bpmn:process id="${processId}" name="${escapeXml(scenario.name)}" isExecutable="true">
    ...
    <bpmn:userTask id="initial" name="initial">${extensionJson({ role: { name: 'tester' }, tool: 'formHandler:initial' })}<bpmn:incoming>flow_start_initial</bpmn:incoming><bpmn:outgoing>flow_initial_split</bpmn:outgoing></bpmn:userTask>
    <bpmn:inclusiveGateway id="split_gateway" name="inclusive split">${extensionJson({ role: { name: 'tester' } })}<bpmn:incoming>flow_initial_split</bpmn:incoming><bpmn:outgoing>flow_split_branch_1</bpmn:outgoing><bpmn:outgoing>flow_split_branch_2</bpmn:outgoing><bpmn:outgoing>flow_split_branch_3</bpmn:outgoing></bpmn:inclusiveGateway>
    ${[1, 2, 3].map((i) => `<bpmn:userTask id="branch_${i}" name="branch_${i}">${extensionJson({ role: { name: 'tester' }, tool: `formHandler:branch_${i}` })}<bpmn:incoming>flow_split_branch_${i}</bpmn:incoming><bpmn:outgoing>flow_branch_${i}_end</bpmn:outgoing></bpmn:userTask><bpmn:endEvent id="branch_${i}_end" name="branch_${i}_end"><bpmn:incoming>flow_branch_${i}_end</bpmn:incoming></bpmn:endEvent>`).join('\n    ')}
    <bpmn:sequenceFlow id="flow_start_initial" sourceRef="start" targetRef="initial">${extensionJson({})}</bpmn:sequenceFlow>
    <bpmn:sequenceFlow id="flow_initial_split" sourceRef="initial" targetRef="split_gateway">${extensionJson({})}</bpmn:sequenceFlow>
    ${[1, 2, 3].map((i) => `<bpmn:sequenceFlow id="flow_split_branch_${i}" sourceRef="split_gateway" targetRef="branch_${i}">${extensionJson({ condition: branchConditions[i], conditionMode: 'text', priority: i })}</bpmn:sequenceFlow><bpmn:sequenceFlow id="flow_branch_${i}_end" sourceRef="branch_${i}" targetRef="branch_${i}_end">${extensionJson({})}</bpmn:sequenceFlow>`).join('\n    ')}
  </bpmn:process>
  <bpmndi:BPMNDiagram id="BPMNDiagram_1"><bpmndi:BPMNPlane id="BPMNPlane_1" bpmnElement="Collaboration_${processId}">
    ...
    ${[1, 2, 3].map((i) => `<bpmndi:BPMNShape id="branch_${i}_di" bpmnElement="branch_${i}"><dc:Bounds x="510" y="${100 + i * 90}" width="110" height="64" /></bpmndi:BPMNShape><bpmndi:BPMNShape id="branch_${i}_end_di" bpmnElement="branch_${i}_end"><dc:Bounds x="750" y="${114 + i * 90}" width="36" height="36" /></bpmndi:BPMNShape>`).join('\n    ')}
