...
export async function syncDepartmentsFromOrgChart(): Promise<{ success: boolean; count: number; error?: string }> {
    try {
        ...
        const orgData = typeof configData.value === 'string' ? JSON.parse(configData.value) : configData.value;
        ...
        function extractDepartments(node: any, parentId: string | null = null, level = 0, pathPrefix = '') {
            if (!node) return;
        ...
        const { error: upsertError } = await supabase.from('departments').upsert(deptRecords, { onConflict: 'id' });
