...
const hasParallelApproval = computed(
    () => !!(approvalState.value?.hq_reviewer_name || approvalState.value?.field_reviewer_name || approvalState.value?.hq_status)
);
...
async function loadBpmnData(procDefId: string) {
    try {
        ...
        if (versions && versions.length > 0) {
            ...
            let currentVerIdx = version ? versions.findIndex((v: any) => String(v.version) === String(version)) : -1;
...
function setupRealtimeSubscription() {
    ...
    console.log(
        '[Realtime] Subscribing to proc_def_approval_state, initialTargetId:',
        initialTargetId,
        'approvalState.id:',
        ...
        <template v-else>
            ...
            <div class="rd-body">
                ...
                <div class="rd-left">
                    ...
                    <div
                        v-else-if="(snapshotCompareMode === 'published' || snapshotCompareMode === 'previous') && prevXml"
                        class="rd-bpmn-compare"
                    >
                            ...
                            >
                                ...
                                <div class="rd-he-card">
                                    <div class="rd-he-top">
                                        <div
                                            class="rd-he-avatar"
                                            :style="{ background: getAvatarColor(entry.actor_name || entry.actor_id) }"
                                        >
        ...
        <v-dialog v-model="showCancelDialog" max-width="440" persistent>
            <v-card rounded="lg" elevation="8">
                ...
                <v-card-actions class="px-4 pb-4">
                    ...
                    <v-btn color="warning" variant="flat" size="small" :loading="actionLoading" @click="handleCancel"> 취소 확인 </v-btn>
                </v-card-actions>
            </v-card>
        </v-dialog>
        ...
        <v-dialog v-model="showResolveDialog" max-width="480" persistent>
            <v-card rounded="lg" elevation="8">
                ...
                <v-card-text class="pt-3">
                    ...
                    <p class="text-body-2 text-medium-emphasis mt-3 mb-2">조치 내용을 입력하세요 <span class="text-error">*</span></p>
                    <v-textarea
...
.rd-btn--end-feedback:not(.rd-btn--disabled):hover {
    background: #7c3aed;
}
/* Publish: 초록 solid */
.rd-btn--approve {
...
.rd-change-list::-webkit-scrollbar {
    width: 4px;
}
...
.rd-bpmn-bar--snapshot {
    background: #ede9fe;
}
.rd-bpmn-badge--snapshot {
    background: #ddd6fe;
    color: #5b21b6;
}
...
.rd-public-banner--urgent .rd-public-banner-left {
    color: #be123c;
}
.rd-public-banner-text {
    font-weight: 600;
}
