    ...
    <div class="data-freeze-manager">
        ...
        <div class="page-header">
            <div class="page-header-left">
                <h1 class="page-title">프로세스 수정 잠금 관리</h1>
            </div>
        </div>

        <!-- Scope Tabs (Vuetify v-btn-toggle) -->
        <div class="pa-4 pb-0">
            <v-btn-toggle v-model="scope" mandatory density="compact" color="primary" variant="text">
                <v-btn
                ...
                >
                    {{ s.labelKo }}
                    <v-chip size="x-small" variant="tonal" class="ms-2">{{ getLockedCount(s.value) }}</v-chip>
                </v-btn>
            </v-btn-toggle>
        </div>
...
export default defineComponent({
    ...
    setup() {
        ...
        const scopeOptions = [
            { value: 'domain', labelKo: '도메인', icon: 'mdi-domain' },
            { value: 'mega_process', labelKo: '메가 프로세스', icon: 'mdi-sitemap-outline' },
            { value: 'major_process', labelKo: '메이저 프로세스', icon: 'mdi-file-tree' },
            { value: 'subprocess', labelKo: '프로세스', icon: 'mdi-file-document-outline' }
        ];
        ...
        const allItems = computed(() => {
            ...
            if (scope.value === 'major_process') {
                if (pm?.mega_proc_list?.length) {
                    ...
                    pm.mega_proc_list.forEach((mega) => {
                        (mega.major_proc_list || []).forEach((major) => {
                            if (!major?.id || seen.has(major.id)) return;
                            seen.add(major.id);
        ...
        function formatDate(dateStr) {
            if (!dateStr) return '-';
            return formatDateKST(dateStr, dateStr);
        }
...
/* Scope Tabs Wrapper — 좌우 padding + 하단 border 만 페이지 고유 */
