    ...
    <div class="sysops-wrapper">
        ...
        <div class="section-card" :class="{ 'section-card--danger': maintenanceForm.enabled }">
            <div class="section-header" :class="{ 'section-header--danger': maintenanceForm.enabled }">
                ...
                <div v-if="maintenanceForm.enabled" class="danger-badge">점검 중</div>
            </div>
            <div class="section-body">
                <!-- Big danger toggle -->
                ...
                <template v-if="maintenanceForm.enabled && (store.maintenanceMode.activated_by || store.maintenanceMode.activated_at)">
                    ...
                    <div class="activated-info">
                        <div v-if="store.maintenanceMode.activated_by" class="activated-row">
                            ...
                            <span class="activated-value">{{ store.maintenanceMode.activated_by }}<template v-if="store.maintenanceMode.activated_role"> ({{ store.maintenanceMode.activated_role }})</template></span>
                        </div>
        ...
        <div class="section-card">
            ...
            <div class="section-body">
                <v-alert v-if="cutoverFeedback.text" :type="cutoverFeedback.type" variant="tonal" density="compact" class="mb-4">
                    {{ cutoverFeedback.text }}
                </v-alert>
                ...
                <template v-else>
                        ...
                        <div v-for="job in store.cutoverJobs" :key="job.id" class="cutover-job">
                            <div class="cutover-job__top">
                                ...
                                <div class="d-flex align-center ga-2 flex-wrap">
                                    <v-chip size="small" :color="approvalStatusColor(job.approval_status)" variant="tonal">
                            ...
                            <div v-if="job.maintenance_message" class="cutover-job__maintenance">
                                점검 사유: {{ job.maintenance_message }}
                            </div>
                            <div class="cutover-job__actions">
                                <v-btn size="small" variant="text" class="text-none" @click="openCutoverJob(job)">
                                    상세 보기
                                </v-btn>
                                <v-btn size="small" variant="text" class="text-none" @click="openApprovalBoard(job)">
                                    결재 라인 검토
                                </v-btn>
                                <v-btn
                                    size="small"
                                    ...
                                    variant="flat"
                                    class="text-none"
                                    :disabled="!canRunCutover(job) || cutoverActionLoading"
                                    :loading="cutoverActionLoading && selectedCutoverJob?.id === job.id"
                                    @click="runCutover(job)"
                                >
                                    Cut-over 적용
                                </v-btn>
                            </div>
                    ...
                    <div v-if="selectedCutoverJob" class="cutover-detail">
                        <div class="cutover-detail__header">
                            ...
                            <div class="d-flex align-center ga-2 flex-wrap">
                                <v-chip size="small" :color="approvalStatusColor(selectedCutoverJob.approval_status)" variant="tonal">
                                    {{ approvalStatusLabel(selectedCutoverJob.approval_status) }}
                                </v-chip>
                        ...
                        <div class="cutover-detail__grid">
                            <div class="cutover-detail__panel">
                                ...
                                <div class="cutover-detail__timeline">
                                    ...
                                    <div class="cutover-detail__timeline-row">
                                        <span class="cutover-detail__timeline-key">Approval</span>
                                        <span>
                                            {{ approvalStatusLabel(selectedCutoverJob.approval_status) }}
                                            <template v-if="selectedCutoverJob.approved_at">
                                                · {{ formatDateTime(selectedCutoverJob.approved_at) }} · {{ selectedCutoverJob.approved_by || 'system' }}
                                            </template>
                                            <template v-else-if="selectedCutoverJob.rejected_at">
                                                · {{ formatDateTime(selectedCutoverJob.rejected_at) }} · {{ selectedCutoverJob.rejected_by || 'system' }}
                                            </template>
                                        </span>
                                    </div>
                                    <div class="cutover-detail__timeline-row">
                        ...
                        <div class="cutover-job__actions mt-4">
                            <v-btn size="small" variant="outlined" class="text-none" @click="openApprovalBoard(selectedCutoverJob)">
                                결재 라인 검토
                            </v-btn>
                            <v-btn
                                size="small"
                                ...
                                variant="flat"
                                class="text-none"
                                :disabled="!canRunCutover(selectedCutoverJob) || cutoverActionLoading"
                                :loading="cutoverActionLoading"
                                @click="runCutover(selectedCutoverJob)"
                            >
                                Cut-over 적용
                            </v-btn>
                        </div>
                        ...
                        <div v-if="selectedCutoverJob.approval_comment" class="cutover-detail__panel cutover-detail__panel--full">
                            <div class="cutover-detail__panel-title">Approval Comment</div>
                            <div class="cutover-detail__summary-item">
                                <v-icon size="14" color="primary">mdi-comment-text-outline</v-icon>
                                <span>{{ selectedCutoverJob.approval_comment }}</span>
                            </div>
                        </div>
...
export default defineComponent({
    ...
    setup() {
        ...
        const cutoverActionLoading = ref(false);
        const cutoverFeedback = ref({ type: 'info', text: '' });
        ...
        function openApprovalBoard(job) {
            router.push({
                path: '/review-board/restructure',
                query: job?.id ? { jobId: job.id } : undefined
            });
        }

        function canRunCutover(job) {
            if (!job) return false;
            if (job.status === 'completed' || job.status === 'running') return false;
            if (job.approval_status !== 'approved') return false;
            if (!store.maintenanceMode.enabled) return false;
            return true;
        }
        ...
        async function runCutover(job) {
            if (!job?.id) return;
            selectedCutoverJobId.value = job.id;
            cutoverActionLoading.value = true;
            cutoverFeedback.value = { type: 'info', text: '' };
            try {
                await store.runCutoverJob(job.id);
                cutoverFeedback.value = { type: 'success', text: '구조개편 cut-over가 적용되었습니다.' };
            } catch (e) {
                console.error('Failed to run restructure cutover:', e);
                cutoverFeedback.value = { type: 'error', text: e?.message || '구조개편 cut-over 적용에 실패했습니다.' };
            } finally {
                cutoverActionLoading.value = false;
            }
        }
