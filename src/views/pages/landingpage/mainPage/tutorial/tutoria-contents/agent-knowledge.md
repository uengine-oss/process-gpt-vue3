...
2. 관리팀에 '+' 버튼을 클릭하여 조직원 추가를 진행하며, 이때 '신규 에이전트 추가' 탭으로 이동하여 아래 화면과 같이 다음의 항목에 수정합니다.


![](../../uengine-image/process-gpt/knowledge/2.에이전트생성1.png)
...
![](../../uengine-image/process-gpt/knowledge/3.에이전트생성2.png)
...
![](../../uengine-image/process-gpt/knowledge/4.에이전트생성3.png)
...
3. 생성이 완료되면 상단 메뉴 '채팅' > '유저'를 클릭하여 이전 단계에서 생성한 에이전트가 채팅목록에 표시되며, 아래 화면과 같이 '채팅'을 클릭하여 대화화면을 생성합니다.


4. 채팅화면에서 세무사 Agent에게 아래와 같이 회사와 관련된 지식을 주입하여 훈련을 진행하면 아래와 같이 세무사 Agent가 사용자의 대화내용을 기반으로 사내 지식을 학습합니다.

![](../../uengine-image/process-gpt/knowledge/6.훈련지식주입.png)

5. 이후, 에이전트에게 훈련 시킨 내용을 기반으로 사내 관련 질문을 하면 Agent가 학습한 내용을 기반으로 관련된 최적의 답변을 제공합니다.
