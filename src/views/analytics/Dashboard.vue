    ...
    try {
        ...
        if (response.success && response.data) {
            ...
            fteHeatmapData.value = {
                ...
                data: data.processes?.map((proc) => data.departments?.map((dept) => data.matrix?.[proc]?.[dept]?.fte || 0) || []) || []
            };
        }
    } catch (e) {
...
async function loadBottleneckAnalysis() {
    try {
        ...
        if (response.success && response.data) {
            // 대기시간 기준 정렬 (병목 심한 순)
            bottleneckData.value = response.data.sort((a, b) => (b.avg_wait_time_sec || 0) - (a.avg_wait_time_sec || 0)).slice(0, 5);
        }
    } catch (e) {
...
async function syncDepartments() {
    ...
    try {
        ...
        if (result.success) {
            ...
            // 데이터 새로고침
            await Promise.all([loadDepartmentTaskStats(), loadFteHeatmap()]);
        } else {
                        ...
                        <v-card-text class="pa-4">
                            <div class="d-flex justify-space-between align-center mb-3">
                                <h3 class="text-subtitle-1 font-weight-semibold text-textPrimary">
                                    {{ $t('analyticsDashboard.taskDistribution') }}
                                </h3>
                            ...
                            <div v-if="canRenderTaskDistribution && hasTaskDistributionData" class="doughnut-wrapper">
                                <Doughnut :chartData="safeTaskDistributionData" :chartOptions="doughnutOptions" />
                            </div>
                ...
                <v-col cols="12" md="8" class="d-flex">
                    <v-card variant="outlined" class="rounded-lg flex-grow-1">
                        <v-card-text class="pa-4">
                            <div class="d-flex justify-space-between align-center mb-3">
                                <h3 class="text-subtitle-1 font-weight-semibold text-textPrimary">
                                    {{ $t('analyticsDashboard.monthlyTrend') }}
                                </h3>
                            ...
                            <div v-if="canRenderMonthlyTrend" class="chart-wrapper">
                                <Line :chartData="safeMonthlyTrendData" :chartOptions="lineChartOptions" />
                            </div>
            ...
            <v-row class="mb-2">
                        ...
                        <v-card-text class="pa-4">
                            ...
                            <div v-else class="text-center pa-4">
                                ...
                                <p class="text-body-2 text-grey100 mb-0">
                                    {{ $t('analyticsDashboard.noFteData') }}<br />{{ $t('analyticsDashboard.checkAfterExecution') }}
                                </p>
                            </div>
                        </v-card-text>
                    </v-card>
                ...
                <v-col cols="12" md="5" class="d-flex">
                    <v-card variant="outlined" class="rounded-lg flex-grow-1">
                        <v-card-text class="pa-4">
                            <div class="d-flex justify-space-between align-center mb-4">
                                <h3 class="text-h6 font-weight-semibold text-textPrimary">
                                    {{ $t('analyticsDashboard.domainSubProcess') }}
                                </h3>
                                <v-chip size="small" color="warning" variant="tonal">
                                    {{
                                        $t('analyticsDashboard.totalCount', {
                                            count: domainData.reduce((a, b) => a + b.subProcessCount, 0)
                                        })
                                    }}
                                </v-chip>
                            </div>
                            <div v-if="domainData.length > 0" class="domain-list">
                                <div v-for="item in domainData" :key="item.domain" class="domain-item">
                            ...
                            <div v-else class="text-center pa-4">
                                ...
                                <p class="text-body-2 text-grey100 mb-0">
                                    {{ $t('analyticsDashboard.setDomainHint') }}<br />{{ $t('analyticsDashboard.setDomainHint2') }}
                                </p>
                            </div>
                        </v-card-text>
                    </v-card>
                    ...
                    <v-card variant="outlined" class="rounded-lg flex-grow-1">
                        <v-card-text class="pa-4">
                            ...
                            <div v-if="canRenderDepartmentChart" class="chart-wrapper">
                                <Bar :chartData="safeDepartmentChartData" :chartOptions="barChartOptions" />
                            </div>
                            <div v-else-if="loading" class="d-flex align-center justify-center" style="height: 150px">
                                <v-progress-circular indeterminate color="primary" />
                            </div>
                        ...
                        <v-card-text class="pa-4">
                            <div class="d-flex justify-space-between align-center mb-4">
                                ...
                                <v-chip size="small" color="info" variant="tonal">{{
                                    $t('analyticsDashboard.performanceComparison')
                                }}</v-chip>
                            </div>
                                ...
                                <div class="comparison-col agent-col">
                                    ...
                                    <div class="comparison-stat">
                                        <span class="stat-value text-primary"
                                            >{{ getAgentStats?.avg_duration_sec?.toFixed(0) || 0
                                            }}{{ $t('analyticsDashboard.seconds') }}</span
                                        >
                                        <span class="stat-label">{{ $t('analyticsDashboard.avgProcessingTime') }}</span>
                                    </div>
                                    <div class="comparison-stat">
                                        <span class="stat-value text-success"
                                            >{{ getAgentStats?.first_time_right_pct?.toFixed(1) || 0 }}%</span
                                        >
                                ...
                                <div class="comparison-col human-col">
                                    ...
                                    <div class="comparison-stat">
                                        <span class="stat-value text-indigo"
                                            >{{ getHumanStats?.avg_duration_sec?.toFixed(0) || 0
                                            }}{{ $t('analyticsDashboard.seconds') }}</span
                                        >
                                        <span class="stat-label">{{ $t('analyticsDashboard.avgProcessingTime') }}</span>
                                    </div>
                                    <div class="comparison-stat">
                                        <span class="stat-value text-success"
                                            >{{ getHumanStats?.first_time_right_pct?.toFixed(1) || 0 }}%</span
                                        >
                                        <span class="stat-label">{{ $t('analyticsDashboard.firstTimeRight') }}</span>
                                    </div>
                                    <div class="comparison-stat">
                ...
                <v-col cols="12" md="6">
                    <v-card variant="outlined" class="rounded-lg">
                        <v-card-text class="pa-4">
                            <div class="d-flex justify-space-between align-center mb-4">
                                <h3 class="text-h6 font-weight-semibold text-textPrimary">
                                    {{ $t('analyticsDashboard.processPerformance') }}
                                </h3>
                            ...
                            <div v-if="processPerformanceData.length > 0" class="process-perf-list">
                                <div v-for="proc in processPerformanceData" :key="proc.proc_def_id" class="process-perf-item">
                                    <div class="d-flex justify-space-between align-center mb-1">
                                        ...
                                        <span class="text-caption text-grey100">
                                            {{ proc.completed_instances }}/{{ proc.total_instances }}
                                            {{ $t('analyticsDashboard.completed') }}
                                        </span>
                                    </div>
                ...
                <v-col cols="12" md="6">
                    <v-card variant="outlined" class="rounded-lg">
                        <v-card-text class="pa-4">
                            <div class="d-flex justify-space-between align-center mb-4">
                                <h3 class="text-h6 font-weight-semibold text-textPrimary">
                                    {{ $t('analyticsDashboard.bottleneckAnalysis') }}
                                </h3>
                            ...
                            <div v-if="bottleneckData.length > 0" class="bottleneck-list">
                                <div v-for="(item, idx) in bottleneckData" :key="idx" class="bottleneck-item">
                                    <div class="d-flex align-center mb-2">
                                        <v-avatar
                                            size="28"
                                            :color="idx === 0 ? 'error' : idx === 1 ? 'warning' : 'grey-lighten-1'"
                                            class="mr-3"
                                        >
                                            <span class="text-caption font-weight-bold" :class="idx < 2 ? 'text-white' : ''">{{
                                                idx + 1
                                            }}</span>
                                        </v-avatar>
                                        ...
                                        <div class="text-right">
                                            <div class="text-body-2 font-weight-bold text-error">
                                                {{ formatDuration(item.avg_wait_time_sec) }}
                                            </div>
                                    ...
                                    <div class="d-flex justify-space-between text-caption mt-1">
                                        <span class="text-error"
                                            >{{ $t('analyticsDashboard.wait') }}: {{ formatDuration(item.avg_wait_time_sec) }}</span
                                        >
                                        <span class="text-primary"
                                            >{{ $t('analyticsDashboard.processing') }}:
                                            {{ formatDuration(item.avg_processing_time_sec) }}</span
                                        >
                    ...
                    <v-table class="month-table" hover>
                        ...
                        <tbody>
                            <tr v-for="instance in store.instances" :key="instance.id" class="month-item">
                                <td class="text-body-2 font-weight-medium text-textPrimary">{{ instance.id?.slice(0, 8) }}...</td>
                                <td class="text-body-2 text-grey100">{{ instance.process_name || 'Unnamed Process' }}</td>
                                <td>
                                    <v-chip size="small" :color="instance.status === 'completed' ? 'success' : 'warning'" variant="tonal">
                                        {{ instance.status }}
                                    </v-chip>
                                </td>
