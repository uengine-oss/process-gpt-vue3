            ...
            <v-card elevation="10" class="is-work-height">
                ...
                <div class="d-flex justify-space-between align-center px-6 py-4">
                    ...
                    <div class="d-flex ga-2 align-center">
                        ...
                        />
                        <v-btn variant="text" prepend-icon="mdi-upload" @click="schemaDialog = true"> 스키마 업로드 </v-btn>
                    </div>
                </div>
                <v-divider />
                ...
                <div class="pa-6" style="overflow: auto; height: calc(100vh - 200px)">
                    <!-- No Schema Alert -->
                    <v-alert v-if="store.cubes.length === 0 && !store.loading" type="info" variant="tonal" class="mb-4">
                        <v-alert-title>스키마가 필요합니다</v-alert-title>
                        <div class="text-body-2">피벗 테이블을 사용하려면 먼저 Mondrian XML 스키마를 업로드하세요.</div>
                        <template #append>
                            <v-btn variant="flat" color="info" size="small" @click="schemaDialog = true"> 스키마 업로드 </v-btn>
                        </template>
                    </v-alert>
