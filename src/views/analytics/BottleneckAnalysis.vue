...
const summaryStats = computed(() => {
    ...
    if (isEtlData) {
        ...
        const totalWeightedDuration = executionData.value.reduce(
            (sum, e) => sum + (e.avg_processing_time_sec || 0) * (e.execution_count || 0),
            0
        );
        ...
        const durations = executionData.value.filter((e) => e.actual_duration).map((e) => parseDuration(e.actual_duration));
        const avgSeconds = durations.length > 0 ? durations.reduce((a, b) => a + b, 0) / durations.length : 0;
...
function calculateActivityMetricsFromETL() {
    ...
    executionData.value.forEach((item: any) => {
        ...
        const capacity = fteCapacity.value.get(config.roleName) ||
            fteCapacity.value.get('Default') || {
                availableFte: defaultAvailableFte.value,
                hoursPerDay: 8
            };
...
function calculateActivityMetrics() {
    ...
    activityGroups.forEach((items, activityId) => {
        ...
        const reworkRate = uniqueInstances.size < items.length ? ((items.length - uniqueInstances.size) / items.length) * 100 : 0;
...
function calculatePeakFte(items: any[], standardMinutes: number): number {
    ...
    sortedItems.forEach((item, idx) => {
        ...
        const endTime = item.completed_at ? new Date(item.completed_at).getTime() : startTime + standardMs; // 완료 시간이 없으면 표준 시간 사용

        // 이 시점에 동시에 실행 중인 인스턴스 수 계산
        let concurrent = 0;
        sortedItems.forEach((other) => {
            const otherStart = new Date(other.started_at).getTime();
            const otherEnd = other.completed_at ? new Date(other.completed_at).getTime() : otherStart + standardMs;
...
function applyHeatmapOverlay() {
    ...
    `;
      ...
      <div style="margin-top: 4px; padding-top: 4px; border-top: 1px solid rgba(255,255,255,0.1);">
        ${proxy.$t('bottleneckAnalysis.bottleneckScore')}: <strong style="color: ${color};">${Math.round(
...
function getMetricValue(metrics: any): number {
    switch (selectedMetric.value) {
        ...
        case 'bottleneck':
            return metrics.normalizedBottleneck;
        case 'rework':
            return metrics.reworkRate;
        case 'waiting':
            return metrics.waitingTime;
        // FTE metrics
        case 'workloadFte':
            return metrics.workloadFte || 0;
        case 'peakFte':
            return metrics.peakFte || 0;
        case 'loadRatio':
            return metrics.loadRatio || 0;
        default:
            return 0;
    }
}
...
function getDisplayValue(metrics: any): string {
    switch (selectedMetric.value) {
        case 'duration':
            return formatDuration(metrics.avgDuration);
        case 'frequency':
            return `${metrics.frequency}${proxy.$t('bottleneckAnalysis.times')}`;
        case 'bottleneck':
            return `${Math.round(metrics.normalizedBottleneck)}%`;
        case 'rework':
            return `${metrics.reworkRate.toFixed(1)}%`;
        case 'waiting':
            return formatDuration(metrics.waitingTime);
        // FTE metrics
        case 'workloadFte':
            return `${(metrics.workloadFte || 0).toFixed(2)} FTE`;
        case 'peakFte':
            return `${(metrics.peakFte || 0).toFixed(1)} FTE`;
        case 'loadRatio':
            return `${Math.round(metrics.loadRatio || 0)}%`;
        default:
            return '';
    }
}
        ...
        <v-card-text class="pa-4">
                    ...
                    <v-card-text>
                        ...
                        <v-row dense>
                            <v-col v-for="(def, key) in metricDefinitions" :key="key" cols="12" md="4">
                                <div class="metric-def-item pa-3 rounded-lg" :class="{ 'metric-def-active': selectedMetric === key }">
                                    <div class="d-flex align-center mb-2">
                                        <v-icon :icon="def.icon" :color="def.color" size="18" class="mr-2" />
                                        <span class="text-body-2 font-weight-semibold">{{ def.title }}</span>
                                        <v-chip v-if="selectedMetric === key" size="x-small" color="primary" class="ml-2">{{
                                            $t('bottleneckAnalysis.selected')
                                        }}</v-chip>
                                    </div>
                    ...
                    <v-row dense class="mt-3">
                        <v-col cols="12">
                            <div class="metric-category-tabs">
                                ...
                                <v-btn-toggle v-model="selectedMetric" mandatory density="compact" color="primary" class="metric-toggle">
            ...
            <div v-if="!selectedProcess" class="empty-state">
                ...
                <p class="text-body-2 text-grey100">
                    {{ $t('bottleneckAnalysis.selectProcessDesc') }}<br />{{ $t('bottleneckAnalysis.selectProcessDesc2') }}
                </p>
            </div>
                        ...
                        <v-card variant="outlined" class="rounded-lg">
                            <v-card-text class="pa-0">
                                ...
                                <div v-else-if="bpmnXml" class="bpmn-wrapper">
                                    <BpmnUengineViewer :key="bpmnKey" ref="bpmnViewerRef" :bpmn="bpmnXml" />
                                </div>
                        ...
                        <v-card variant="outlined" class="rounded-lg mb-4">
                            <v-card-text class="pa-4">
                                ...
                                <div v-if="bottleneckRanking.length > 0" class="ranking-list">
                                    <div v-for="(item, idx) in bottleneckRanking" :key="item.id" class="ranking-item">
                            ...
                            <v-card-text class="pa-4">
                                <div class="d-flex align-center mb-3">
                                    ...
                                    <span class="text-subtitle-2 font-weight-semibold">{{
                                        $t('bottleneckAnalysis.durationDistribution')
                                    }}</span>
                                </div>
                ...
                <v-card variant="outlined" class="rounded-lg mb-4">
                    <v-card-text class="pa-4">
                        <div class="d-flex justify-space-between align-center mb-3">
                            ...
                            <v-btn size="small" color="gray" rounded="pill" variant="flat" prepend-icon="mdi-plus" @click="addNewRole">
...
.legend-gradient {
    width: 100px;
    height: 10px;
    background: linear-gradient(90deg, var(--cds-text-success), #8bc34a, #fbc02d, var(--cds-text-warning), var(--cds-text-danger));
    border-radius: 5px;
...
.stat-icon.primary {
    background: linear-gradient(135deg, hsl(var(--accent-brand)), #00a8e8);
}
.stat-icon.success {
    background: linear-gradient(135deg, var(--cds-text-success), #2ecc71);
}
.stat-icon.info {
    background: linear-gradient(135deg, #46caeb, #17a2b8);
}
.stat-icon.warning {
    background: linear-gradient(135deg, var(--cds-text-warning), #f39c12);
}
...
.rank-badge.rank-1 {
    background: linear-gradient(135deg, #ea4335, var(--cds-text-danger));
}
.rank-badge.rank-2 {
    background: linear-gradient(135deg, #fbbc04, var(--cds-text-warning));
}
.rank-badge.rank-3 {
    background: linear-gradient(135deg, var(--cds-text-muted), var(--cds-text-muted));
}
