        ...
        <v-card class="skt-version-diff-card">
            ...
            <v-card-text class="skt-version-diff-body pa-0">
                <div class="skt-version-diff-viewers">
                    <!-- 통합 상단 바 -->
                    <div class="skt-version-diff-unified-bar">
                        <div class="skt-version-diff-viewer-bar">
                            ...
                            >
                                ...
                                v{{ internalSelectedVersion }} 를 현재버전으로 반영하기
                            </v-btn>
                        </div>
                        <div class="skt-version-diff-viewer-bar">
                    ...
                    <!-- 뷰어 캔버스 영역 -->
                    <div class="skt-version-diff-canvas-row">
                        <div class="skt-version-diff-viewer-canvas">
                            <BpmnUengineViewer
                                ...
                                @rendered="onViewerRendered(diffViewerOld)"
                            />
                        ...
                        <div class="skt-version-diff-viewer-canvas">
                            <BpmnUengineViewer
                                ...
                                @rendered="onViewerRendered(diffViewerNew)"
                            />
                            <div v-else class="skt-version-diff-empty">
                                <v-icon size="32" color="grey-lighten-1">mdi-file-document-outline</v-icon>
                                <div class="text-caption text-medium-emphasis mt-2">BPMN 데이터 없음</div>
                ...
                <div v-if="diffChanges.length > 0" class="skt-version-diff-changes-summary">
                    ...
                    <div class="skt-version-diff-changes-list">
                        ...
                        >
                            ...
                            <div v-if="change.fieldChanges?.length" class="skt-version-diff-change-fields">
                                <span
                                    v-for="fieldChange in change.fieldChanges"
                                    :key="`${change.id}-${fieldChange}`"
                                    class="skt-version-diff-change-field-chip"
                                >
                                    {{ fieldChange }}
                                </span>
                            </div>
...
type SktBpmnChange = BpmnChange & { fieldChanges?: string[] };
...
const diffChanges = ref<SktBpmnChange[]>([]);
...
function runDiffComputation(oldXml: string, newXml: string) {
    ...
    setTimeout(() => {
        try {
            ...
            diffChanges.value = result.changes as SktBpmnChange[];
...
function onViewerRendered(viewer: Record<string, unknown> | null | undefined) {
    if (!viewer) return;
    nextTick(() => {
        setTimeout(() => {
...
.skt-version-diff-canvas-row {
    display: flex;
    flex: 1;
    min-height: 0;
}

.skt-version-diff-unified-bar > .skt-version-diff-viewer-bar:first-child,
.skt-version-diff-canvas-row > .skt-version-diff-viewer-canvas:first-child {
...
.skt-version-diff-change-fields {
    display: flex;
    flex-wrap: wrap;
    gap: 6px;
    width: 100%;
...
.skt-version-diff-change-field-chip {
    display: inline-flex;
    align-items: center;
    padding: 3px 8px;
    border-radius: 999px;
