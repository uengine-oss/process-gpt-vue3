                ...
                <div class="diff-list" v-if="changes.length > 0">
                    ...
                    >
                        ...
                        <div class="diff-item-content mt-1">
                            ...
                            <div v-if="change.fieldChanges?.length" class="diff-field-change-list mt-2">
                                <span v-for="fieldChange in change.fieldChanges" :key="`${change.id}-${fieldChange}`" class="diff-field-change-chip">
                                    {{ fieldChange }}
                                </span>
                            </div>
                        </div>
...
export default {
    ...
    methods: {
        ...
        formatDate(timestamp) {
            if (!timestamp) return '';
            try {
                return formatKST(timestamp, 'YYYY-MM-DD HH:mm');
            } catch {
                return timestamp;
            }
        },
...
.diff-field-change-list {
    display: flex;
    flex-wrap: wrap;
    gap: 6px;
}
.diff-field-change-chip {
    display: inline-flex;
    align-items: center;
    padding: 3px 8px;
    border-radius: 999px;
