            ...
            <div class="header-left">
                ...
                <div class="header-title-wrap">
                    <div class="header-title-line">
                        ...
                        <v-chip v-if="activePr" size="x-small" :color="prStatusColor" variant="tonal" class="ml-2">
                            {{ prStatusLabel }}
                        </v-chip>
                    </div>
                    <div v-if="activePr" class="header-pr-meta">
                        ...
                        <b>{{ activePr.requester_name || '요청자' }}</b
                        >님이 병합 요청
                        <span class="meta-dot">&middot;</span>
        ...
        <div class="comparison-body">
            ...
            <div class="comparison-right-panel">
                <div class="rail-tabs">
                    ...
                    <button
                        class="rail-tab"
                        :class="{ 'rail-tab--active': rightTab === 'pr' }"
                        @click="
                            rightTab = 'pr';
                ...
                <div v-show="rightTab === 'changes'" class="rail-pane">
                    ...
                    <div v-if="changes.length > 0" class="diff-hint">항목을 누르면 위 다이어그램의 해당 노드로 이동합니다.</div>
                ...
                <div v-show="rightTab === 'pr'" class="rail-pane">
                    ...
                    <template v-if="selectedPrDetail">
                        ...
                        <PrReviewForm
                            v-if="
                                selectedPrDetail.status === 'OPEN' ||
                                selectedPrDetail.status === 'APPROVED' ||
                                selectedPrDetail.status === 'CHANGES_REQUESTED'
                            "
                        ...
                        <div v-if="isOwner && canMerge && selectedPrDetail.status !== 'MERGED'" class="pr-detail-merge">
                            <v-btn variant="flat" color="deep-purple" size="small" block :loading="isMerging" @click="mergePr()">
                ...
                <div v-show="rightTab === 'history'" class="rail-pane">
                    ...
                    <div v-for="(ver, idx) in versions" :key="ver.version" class="history-row" :class="{ 'history-row--cur': idx === 0 }">
                        ...
                        <div class="history-meta">
                            ...
                            <div class="history-sub">{{ getMergeInfoForVersion(ver) }}{{ formatRelativeTime(ver.timeStamp) }}</div>
                        </div>
...
import {
    ...
    prStatusColor as _prStatusColor,
        ...
        if (nEl.elementType === 'lane') {
            ...
            if (JSON.stringify(oA) !== JSON.stringify(nA)) {
                changes.push({
                    type: 'modified',
                    id,
                    name: nEl.name || oEl.name,
                    elementType: nEl.elementType,
                    description: descMod(oEl, nEl)
                });
                dA[id] = 'modified';
                dB[id] = 'modified';
            }
...
function descAdd(el) {
    return `${fmtType(el.elementType)} 추가${el.name ? ': ' + el.name : ''}`;
}
function descRem(el) {
    return `${fmtType(el.elementType)} 삭제${el.name ? ': ' + el.name : ''}`;
}
function descMod(o, n) {
    return o.name !== n.name ? `이름 변경: "${o.name}" -> "${n.name}"` : `${fmtType(n.elementType)} 속성 변경`;
}
function fmtType(t) {
    const m = {
        task: 'Task',
...
export default {
    ...
    methods: {
        ...
        focusNode(id) {
            ...
            if (v?.bpmnViewer) {
                try {
                    const er = v.bpmnViewer.get('elementRegistry');
                    const el = er.get(id);
                    if (el) v.bpmnViewer.get('canvas').scrollToElement(el);
                } catch (e) {}
            }
        },
        ...
        async loadInitialData() {
            try {
                ...
                if (isUEngine && listForVersion.length > 0) {
                    ...
                    const vrs = await Promise.all(
                        listForVersion.map((d) => {
                    ...
                    versionList.forEach((v) => {
                        if (v.proc_def_id && !latestVersionMap[v.proc_def_id]) latestVersionMap[v.proc_def_id] = v.version;
                    });
                }
                listForVersion.forEach((d) => {
                    const id = d.id || d.file_name;
                    if (id && latestVersionMap[id]) d.version = latestVersionMap[id];
                });
            ...
            try {
                if (this.selectedVersionA === '__current__') {
                    ...
                    if (v) {
                        let xml = v.snapshot || '';
                        if (isUE && !xml) {
                            const s = await backend.getDefinitionVersions(this.selectedProcessId, {
                                key: 'snapshot',
        ...
        runDiff() {
            ...
            setTimeout(() => {
                try {
                    ...
                    this.diffActivitiesA = r.diffActivitiesA;
                    this.diffActivitiesB = r.diffActivitiesB;
                    this.viewerKeyA++;
                    this.viewerKeyB++;
                } catch (e) {
        ...
        onViewerRendered(viewer) {
            this.$nextTick(() => {
                setTimeout(() => {
                    if (viewer?.bpmnViewer) {
                        try {
                            viewer.bpmnViewer.get('canvas').zoom('fit-viewport', 'auto');
                        } catch (e) {}
                    }
                }, 150);
            });
        ...
        exportDiff() {
            ...
            const l = [
                `Version Comparison: ${this.selectedProcessName}`,
                `Version A: ${this.selectedVersionA === '__current__' ? 'Current' : 'v' + this.selectedVersionA}`,
                `Version B: ${this.selectedVersionB === '__current__' ? 'Current' : 'v' + this.selectedVersionB}`,
                '',
                'Changes:'
            ];
            this.changes.forEach((c) => {
                l.push(`  [${c.type.toUpperCase()}] ${fmtType(c.elementType)}: ${c.name || c.id}`);
        ...
        onResize(e) {
            ...
            this.leftPanelWidth = Math.max(180, Math.min(400, this.resizeStartWidth + (e.clientX - this.resizeStartX)));
        },
        stopResize() {
            this.resizing = false;
        }
...
.review-opt--sel.review-opt--comment {
    border-color: #697084;
    background: #f1f2f5;
    color: #1e2330;
}
.review-opt--sel.review-opt--approve {
    border-color: #22a05b;
    background: #e4f6ec;
    color: #137a40;
}
.review-opt--sel.review-opt--changes {
    border-color: #e0922b;
    background: #fcf1dd;
    color: #9a630f;
}
