 ...
 */
export function pushSequenceLog(level: SequenceLogLevel, tag: string, message: string) {
    sequenceLogLines.value = [...sequenceLogLines.value, { ts: Date.now(), level, tag, message }];
    trim();
}
