...
export const useCubeStore = defineStore('cube', () => {
    ...
    function buildDrillDownConfig(): PivotConfig {
        ...
        for (const key of Object.keys(expandedItems.value)) {
            ...
            if (nextLevel) {
                if (type === 'col') {
                    const exists = config.columns.find((c) => c.dimension === dimName && c.level === nextLevel.name);
                    if (!exists) {
                        const currentIdx = config.columns.findIndex((c) => c.dimension === dimName && c.level === level);
                        if (currentIdx !== -1) {
                    ...
                    const exists = config.rows.find((r) => r.dimension === dimName && r.level === nextLevel.name);
                    if (!exists) {
                        const currentIdx = config.rows.findIndex((r) => r.dimension === dimName && r.level === level);
                        if (currentIdx !== -1) {
