    ...
    methods: {
        convertCSVToJSON(content) {
            ...
            const taskElements = elements.filter(
                (el) => el.elementType === 'Activity' || el.elementType === 'CallActivity' || el.elementType === 'Gateway'
            );

            taskElements.forEach((task) => {
                const id = task.id;

                const hasIncoming = elements.some((el) => el.elementType === 'Sequence' && el.target === id);
                const hasOutgoing = elements.some((el) => el.elementType === 'Sequence' && el.source === id);
