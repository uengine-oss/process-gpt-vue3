<template>
    <div>
        ...
        <v-row class="ma-0 pa-0 pl-4 align-center">
            ...
            <!-- 필터 -->
            <v-menu v-model="showFilter" :close-on-content-click="false" location="end">
                ...
                <v-card min-width="300" v-if="!isLoading">
                    <v-list class="pa-4">
                        <v-list-item v-if="copyFilter.period" class="pa-0">
                            <div flat>
                                ...
                                <v-card-text class="ma-0 pa-0">
                                    <v-col class="pa-0">
                                        ...
                                        <div v-if="filterConfig" v-for="key in Object.keys(filterConfig)" :key="key" class="pa-0">
                                            <v-select
                                                class="mt-4"
        ...
        <v-divider class="my-2 mb-0"></v-divider>
        <v-infinite-scroll @load="load" class="list-page-completed-box">
            ...
            <template v-slot:empty>
                <div v-if="!loading && items.length === 0 && searchWord" class="pa-2" style="width: 100%">
                    <v-alert type="info" class="pa-4" color="grey-lighten-2" text-color="grey-darken-3">{{
                        $t('completedListPage.noSearchResults')
                    }}</v-alert>
                </div>
            </template>
        </v-infinite-scroll>
...
export default {
    ...
    methods: {
        ...
        openFilter() {
            ...
            if (this.copyFilter.period && this.copyFilter.period.startDate)
                this.startDateByPicker = new Date(this.copyFilter.period.startDate);
