              ...
              <v-btn @click="onResetView">초기화</v-btn> -->
                ...
                <div v-if="isMobile" class="pa-2 d-flex flex-wrap gap-2 align-center">
                    <div class="d-flex align-center gap-1">
                        <div class="legend-color" style="background: #fa896b"></div>
                        {{ $t('BSCard.finance') }}
                    </div>
                    <div class="d-flex align-center gap-1">
                        <div class="legend-color" style="background: #0074ba"></div>
                        {{ $t('BSCard.customer') }}
                    </div>
                    <div class="d-flex align-center gap-1">
                        <div class="legend-color" style="background: #01c0c8"></div>
                        {{ $t('BSCard.process') }}
                    </div>
                    <div class="d-flex align-center gap-1">
                        <div class="legend-color" style="background: #763ebd"></div>
                        {{ $t('BSCard.learning') }}
                    </div>
                </div>
    ...
    <v-dialog v-model="addDialog" max-width="500">
        <v-card>
            <v-row class="ma-0 pa-4">
                ...
                <v-spacer></v-spacer>
                <v-btn @click="addDialog = false" class="ml-auto" variant="text" density="compact" icon>
            ...
            <v-card-text class="pa-4 pb-0">
                ...
                <v-text-field v-model="strategyForm.name" :label="$t('BSCard.strategyName')" variant="outlined" dense required />
    ...
    <v-dialog v-model="editDialog" max-width="500">
        <v-card>
            <v-row class="ma-0 pa-4">
                ...
                <v-spacer></v-spacer>
                <v-btn @click="editDialog = false" class="ml-auto" variant="text" density="compact" icon>
                    <v-icon>mdi-close</v-icon>
                </v-btn>
            </v-row>
            ...
            <v-card-text class="pa-4 pb-0">
                ...
                <v-select
                    ...
                    :item-props="getItemProps"
                />
                <v-text-field v-model="editForm.name" :label="$t('BSCard.strategyName')" variant="outlined" dense />
            ...
            <v-card-actions>
                <v-spacer />
                <v-btn @click="saveEditedStrategy" color="primary" variant="flat" rounded>{{ $t('BSCard.save') }}</v-btn>
            </v-card-actions>
        </v-card>
    </v-dialog>
            ...
            <v-card-actions>
                ...
                <v-btn
                    ...
                    :disabled="
                        ...
                        selectedConnectionSource.id === selectedConnectionTarget.id
                    "
                >
...
export default {
    ...
    methods: {
        ...
        async saveStrategy() {
            ...
            const usedIndexes = this.jsonData.strategies.filter((s) => s.perspective === perspective).map((s) => s.index ?? 0);
        ...
        getUpperStrategies(id, perspectiveKey) {
            ...
            const currentPerspective = perspectiveKey ? perspectiveKey : this.jsonData.strategies.find((s) => s.id === id)?.perspective;
