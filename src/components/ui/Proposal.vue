                    ...
                    <div class="w-100" style="height: calc(100vh - 320px)">
                                ...
                                <v-sheet class="bg-lightsecondary rounded-md px-3 py-2">
                                    <div v-if="chat.action == 'agents'">
                                        <div style="margin: 15px">
                                            요청 주신 "{{ newMessage }}"작업을 위해서 팀 구성을 아래와 같이 하였습니다.
                                        </div>

                                        <div
                                            v-for="agent in chat.content"
                                            :key="agent.name"
                                                ...
                                                <div v-if="content.link" style="marin-top: 20px">
                                                    ...
                                                    <div v-else style="display: flex; align-items: center; justify-content: space-between">
                                                        ...
                                                        <a :href="content.link" target="_blank" style="margin-right: 10px">
                                                            <img
                                                                v-if="generatePreviewImage(content.link)"
                                                                :src="generatePreviewImage(content.link)"
                                                                alt="preview"
                        ...
                        <div v-if="result && result.chats.length == 0 && isRunning" style="margin: 5%">
                            ...
                            <div class="progress-border" :class="{ animate: borderCompletedAnimated }">
                                ...
                                <v-progress-linear indeterminate class="my-progress-linear"></v-progress-linear>
                            </div>
                        </div>
                    </div>
                </div>
    ...
    methods: {
        ...
        receiveAgent() {
            ...
            me.ws.onmessage = (event) => {
                ...
                if (event.data.startsWith('Chain started with inputs: ')) {
                    ...
                    event.data.includes('You are a tool for') ||
                    event.data.includes("{'topic'")
                ) {
                    ...
                    try {
                        ...
                        // setting
                        me.result.agents = data.agents.map((agent, index) => ({
                            ...agent,
                            profile: `/src/assets/images/profile/user-${index + 1}.jpg`
                        }));
                        me.result.tools = {
        ...
        convertAgentsContent(data, agents) {
            ...
            let content = agents.map((agent, index) => ({
                ...agent,
                explanation:
                    `${agent.name}은 ${agent.backstory}을 바탕으로 ${agent.goal}을 목표를 가진다.<br/><br/>` +
                    `${
                        data.tasks
                            .filter((task) => task.agent === agent.name)
                            .map((task) => `* 업무내용 <br/> - ${task.description}`)
                            .join('\n') || ''
                    }`
            }));
