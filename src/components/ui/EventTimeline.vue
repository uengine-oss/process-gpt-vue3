                    ...
                    <div v-if="item.type === 'task'" class="task-card">
                        ...
                        <div class="task-header">
                            <div class="task-left">
                                <div class="task-avatar">
                                    <img
                                        v-if="item.payload.agentProfile"
                        ...
                        <div v-if="item.payload.crewType === 'browser-use' && !item.payload.isCompleted" class="browser-container">
                            <div class="browser-preview" @click="openBrowserDialog(item.id)">
                                <iframe :src="browserIframeUrl" class="browser-iframe" frameborder="0" allowfullscreen> </iframe>
                        ...
                        <div v-if="item.payload.isHumanAsked" class="human-query-input">
                            ...
                            <div class="query-content">
                                ...
                                <div
                                    v-else-if="!item.payload.isCompleted && item.payload.humanQueryData.type === 'select'"
                                    class="input-field"
                                >
                                    <select v-model="humanQueryAnswers[item.payload.id]" class="query-select">
                                        ...
                                        <option v-for="opt in item.payload.humanQueryData.options" :key="opt" :value="opt">
                                            {{ opt }}
                                        </option>
                                    </select>
                                </div>
                                <div
                                    v-else-if="!item.payload.isCompleted && item.payload.humanQueryData.type === 'confirm'"
                                    class="confirm-hint"
                                >
                            ...
                            <v-row v-if="!item.payload.isCompleted" class="query-actions ma-0 pa-0">
                                <v-spacer></v-spacer>
                                <v-btn
                                    @click="onCancelHumanQuery(item.payload)"
                            ...
                            <div v-else class="query-completed">
                                <span class="completed-pill" :class="getHumanResultClass(item.payload)">{{
                                    getHumanResultText(item.payload)
                                }}</span>
                                <span v-if="getHumanResultDetail(item.payload)" class="completed-detail">{{
                                    getHumanResultDetail(item.payload)
                                }}</span>
                            </div>
                        </div>
                        ...
                        <div v-else-if="item.payload.isCompleted && item.payload.content" class="task-result">
                            <div class="result-header">
                                <v-row class="ma-0 pa-0 align-center">
                                    ...
                                    <v-spacer></v-spacer>
                                    <v-btn
                                        v-if="shouldShowSubmitButton(item.payload)"
                                    ...
                                    <template v-if="isDeepResearchCustom">
                                        <div class="slides-header">
                                            ...
                                            <div class="slide-navigation">
                                                <button
                                                    ...
                                                    :disabled="getSlideIndex(item.payload.id) === 0"
                                                    class="nav-btn"
                                                >
                                                    ←
                                                </button>
                                                <span class="slide-counter"
                                                    >{{ getSlideIndex(item.payload.id) + 1 }} / {{ getSlideCount(item.payload) }}</span
                                                >
                                                <button
                                                    @click="nextSlide(item.payload.id, getSlideCount(item.payload))"
                                                    :disabled="getSlideIndex(item.payload.id) === getSlideCount(item.payload) - 1"
                                                    class="nav-btn"
                                                >
                                                    →
                                                </button>
                                        ...
                                        <div v-if="getSlideImages(item.payload).length" class="slide-image-carousel">
                                            ...
                                            <div class="slide-indicators">
                                                <span
                                                    v-for="(img, index) in getSlideImages(item.payload)"
                                                    :key="index"
                                        ...
                                        <div v-if="!getSlideImages(item.payload).length" class="slide-indicators">
                                            <span
                                                v-for="(slide, index) in getSlides(item.payload.content)"
                                                :key="index"
                                                :class="['indicator', { active: index === getSlideIndex(item.payload.id) }]"
                                                @click="goToSlide(item.payload.id, index)"
                                            ></span>
                                    ...
                                    <template v-else>
                                        <div class="slides-header">
                                            ...
                                            <div class="slide-navigation">
                                                ...
                                                <span class="slide-counter"
                                                    >{{ getSlideIndex(item.payload.id) + 1 }} /
                                                    {{ getSlides(item.payload.content).length }}</span
                                                >
                                                <button
                                                    @click="nextSlide(item.payload.id)"
                                                    :disabled="
                                                        getSlideIndex(item.payload.id) === getSlides(item.payload.content).length - 1
                                                    "
                                                    class="nav-btn"
                                                >
                                                    →
                                ...
                                <div v-else-if="item.payload.crewType === 'browser-use'" class="pa-4 browser-result">
                                    <div class="browser-result-header">
                                        ...
                                        <div class="browser-status" :class="{ success: item.payload.outputRaw?.success }">
                                            {{
                                                item.payload.outputRaw?.success
                                                    ? `✅ ${$t('EventTimeline.completed')}`
                                    ...
                                    <div v-if="item.payload.outputRaw?.generated_files" class="browser-files">
                                        ...
                                        <div class="files-list">
                                            <div
                                                v-for="[fileName, file] in filteredFiles(item.payload.outputRaw.generated_files)"
                                                :key="fileName"
                                                class="file-item"
                                            >
                                                ...
                                                <div v-else-if="file.parsed_error" class="file-error">
                                                    ...
                                                    <span class="error-text"
                                                        >{{ $t('EventTimeline.parsingFailed') }}: {{ file.parsed_error }}</span
                                                    >
                                                </div>
                                                ...
                                                <div v-else-if="file.content || file.parsed_content" class="file-content">
                                                    <div
                                                        ...
                                                        v-html="formatMarkdownContent(file.content || file.parsed_content)"
                                                    ></div>
                                                </div>
                                            </div>
                                        </div>
                                        ...
                                        <!-- 이미지 슬라이드 -->
                                        <div
                                            v-if="getImageSlides(item.payload.outputRaw.generated_files).length > 0"
                                            class="image-slides-container"
                                        >
                                            <div class="image-slides-header">
                                                <div class="header-info">
                                                    <h5>{{ $t('EventTimeline.imageGallery') }}</h5>
                                                    <span class="slide-hint"
                                                        >{{ getImageSlides(item.payload.outputRaw.generated_files).length
                                                        }}{{ $t('EventTimeline.images') }}</span
                                                    >
                                                </div>
                                                <div class="slide-navigation">
                                                    <button @click="previousImageSlide()" :disabled="imageIndex === 0" class="nav-btn">
                                                        ←
                                                    </button>
                                            ...
                                            <div class="image-slide-content">
                                                <div class="image-wrapper">
                                                    ...
                                                    />
                                                    <button
                                                        @click="openImageDialog(item.payload.outputRaw.generated_files)"
            ...
            <v-card class="image-dialog-card">
                ...
                <v-card-text class="image-dialog-content">
                    <div class="dialog-image-container">
                        <button @click="previousImageSlide()" :disabled="imageIndex === 0" class="dialog-nav-btn dialog-nav-left">
...
export default {
    ...
    emits: [
        'update:humanQueryAnswers',
        'update:expandedTasks',
        'update:slideIndexes',
        'onCancelHumanQuery',
        'onConfirmHumanQuery',
        'submitTask',
    ...
    watch: {
        timeline: {
            handler(newTimeline) {
                ...
                const browserUseTask = newTimeline.find(
                    (item) =>
                        item.type === 'task' &&
    ...
    methods: {
        ...
        shouldShowExpandControls(payload) {
            ...
            const rawJson = typeof payload.content === 'string' ? payload.content : JSON.stringify(payload.content, null, 2);
...
.json-container:not(.expanded).has-expand-controls::after {
    ...
    right: 0;
    height: 40px;
    background: linear-gradient(transparent, var(--cds-bg-neutral));
    pointer-events: none;
}
...
.human-query-input {
    background: var(--cds-bg-neutral); /* lighter than blue-50 */
    border: 1px solid var(--cds-border); /* blue-200 */
    border-radius: 10px;
...
.role-pill {
    ...
    background: var(--cds-bg-accent); /* blue-100 */
    padding: 2px 10px;
    border-radius: 999px;
}
...
.query-select {
    ...
    border-radius: 25px;
    font-size: 14px;
    color: var(--cds-text-primary); /* gray-800 */
    background: var(--cds-surface-2);
}
...
.query-select:focus {
    outline: none;
    border-color: hsl(var(--accent-brand)); /* blue-400 */
}

.confirm-hint {
    font-size: 13px;
    color: hsl(var(--accent-brand)); /* blue-700 */
    font-style: italic;
}
...
.engine-tag {
    ...
    font-size: 12px;
    font-weight: 500;
    border: 1px solid var(--cds-bg-accent);
}
