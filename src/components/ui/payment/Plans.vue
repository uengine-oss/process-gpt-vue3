<template>
    <div class="subscription-plans">
        ...
        <div class="toggle-row">
            <v-switch v-model="isYearly" color="primary" :label="isYearly ? '매월' : '연 단위 (절약하기)'" hide-details inset></v-switch>
        </div>
        <div v-if="!loading" class="plans-container" style="display: flex; flex-wrap: wrap; gap: 20px">
            <div
                v-for="plan in filteredList"
                :key="plan.name"
                :class="['plan-card', { highlight: plan.badge.popular }]"
        ...
        <v-dialog v-model="dialog" width="1200">
            <Payment :order="orderInfo" payBtn="구독하기">
                <template v-slot:terms>
                    구독을 클릭하면 약관에 따라 향후 결제에 대해 Process GPT에서 귀하에게 대금을 청구할 수 있습니다. 언제든지 구독을 취소할
                    수 있습니다.
                </template>
            </Payment>
        </v-dialog>
