    ...
    <div class="customHeight">
        <div>
            ...
            <perfect-scrollbar class="rightpartHeight h-100">
                <v-btn style="position: absolute; left: 45%;" @click="getMoreChat()">get more chat</v-btn>
                <div class="d-flex">
                    <div class="w-100" style="height: calc(100vh - 360px);">
                        <div v-for="(message, index) in filteredMessages" :key="index" class="pa-5">
                            ...
                            <div v-else class="d-flex align-items-start gap-3 mb-1">
                                ...
                                <div>
                                    <small class="text-medium-emphasis text-subtitle-2" v-if="message.role == 'system'">
                                        System,
                                        {{ message.timeStamp.split(':')[0] + ':' + message.timeStamp.split(':')[1] }}
                                    </small>
                                    <small class="text-medium-emphasis text-subtitle-2" v-else>
                                        {{ message.name }},
                                        {{ message.timeStamp.split(':')[0] + ':' + message.timeStamp.split(':')[1] }}
                                    </small>
