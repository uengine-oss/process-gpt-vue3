                ...
                <v-sheet class="bg-lightsecondary rounded-md px-3 py-2">
                    <div v-if="message.content.type == 'agents'">
                        <div style="margin: 15px">
                            요청 주신 "{{ agentInfo.draftPrompt }}"작업을 위해서 팀 구성을 아래와 같이 하였습니다..
                        </div>
                        <div
                            v-for="agent in message.content.data"
                            :key="agent.name"
                                    ...
                                    <div v-else style="display: flex; align-items: center; justify-content: space-between">
                                        ...
                                        <div style="margin-top: 20px">
                                            ...
                                            <div style="margin-top: auto; font-size: x-small">
                                                <a :href="item.link" target="_blank" style="color: #0000ee; text-decoration: underline">{{
                                                    item.link
                                                }}</a>
                                            </div>
                                        </div>
                                        <a :href="item.link" target="_blank" style="margin-right: 10px">
                                            <img
                                                v-if="generatePreviewImage(item.link)"
                                                :src="generatePreviewImage(item.link)"
                                                alt="preview"
