<template>
    <template v-if="is_multidata_mode === 'true'">
        <v-card-title class="d-flex justify-space-between pa-0">
            ...
            <v-btn @click="addItem" color="primary" variant="text" icon>
                <Icons :icon="'plus'" />
            </v-btn>
        </v-card-title>
    ...
    <template v-else>
        <v-card class="pa-0 mb-0 form-layout-card" variant="outlined">
            <v-card-title v-if="alias && alias.length" class="d-flex justify-space-between">
                <div class="text-h5 font-weight-semibold" style="white-space: normal; word-break: break-all">
                    {{ alias && alias.length > 0 ? alias : name }}
                </div>
            </v-card-title>
...
export default {
    ...
    created() {
        if (this.is_multidata_mode === 'true' && this.modelValue[this.name] === undefined) this.modelValue[this.name] = [{}];
