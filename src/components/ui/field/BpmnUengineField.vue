<template>
    <div>
        <v-card class="rounded-lg mb-2" variant="outlined" hover @click="openDialog">
            <!-- 헤더 영역 -->
        ...
        <v-dialog v-model="showDialog" persistent max-width="1600px" transition="dialog-transition">
            <v-card>
                <div class="d-flex pa-4">
                    ...
                    <v-btn icon class="ml-auto" variant="text" density="compact" @click="closeDialog">
                ...
                <v-row v-if="!localReadonly" class="ma-0 pa-4">
                    <v-spacer></v-spacer>
                    <v-btn class="rounded-pill mr-2" density="compact" @click="closeDialog"> 취소 </v-btn>
                    <v-btn class="rounded-pill" density="compact" :color="themeColor" @click="saveBpmn"> 저장 </v-btn>
                </v-row>
            </v-card>
        </v-dialog>
    </div>
...
export default {
    ...
    computed: {
        ...
        effectiveBpmn() {
            return this.localModelValue && this.localModelValue.length > 0 ? this.localModelValue : DEFAULT_BPMN_XML;
        }
    },
    ...
    methods: {
        ...
        async loadProcessList() {
            try {
                ...
                definitions = definitions.filter((def) => def.path && !def.isDeleted);
