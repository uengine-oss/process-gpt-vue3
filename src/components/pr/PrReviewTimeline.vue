<template>
    <div class="pr-review-timeline">
        <div v-if="!reviews.length" class="text-center text-caption text-medium-emphasis pa-6">아직 리뷰가 없습니다</div>
        <div v-for="r in reviews" :key="r.id" class="rtl-item">
            <div :class="['rtl-ico', actionIconClass(r.action)]">
                <svg
                    v-if="r.action === 'APPROVED'"
                    width="13"
                    height="13"
                    viewBox="0 0 24 24"
                    fill="none"
            ...
            <div class="rtl-body">
                <div class="rtl-header">
                    ...
                    <span v-if="r.action !== 'COMMENT'" :class="['rtl-badge', r.action === 'APPROVED' ? 'rtl-badge-ok' : 'rtl-badge-chg']">
