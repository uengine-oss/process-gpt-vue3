<template>
    <v-sheet>
        <div class="px-6 pt-3">
            <v-row
                class="align-center flex-fill border border-borderColor header-search rounded-pill px-5 ma-0 pa-0"
                style="min-width: 100%"
            >
    ...
    <v-dialog v-model="dialog" persistent max-width="600px">
        <v-card class="ma-0 pa-4">
            <v-row class="ma-0 pa-0">
                ...
                <v-spacer></v-spacer>
                <v-btn @click="dialog = false" icon variant="text" density="comfortable" style="margin-top: -8px">
            ...
            <v-card-text class="ma-0 pa-0 pb-2 pt-4">
                ...
                <v-autocomplete
                    ...
                    multiple
                    :label="$t('chatListing.selectParticipants')"
                    small-chips
                    :item-avatar="'image'"
                ...
                >
                    <template v-slot:chip="{ props, item }">
                        ...
                        <v-chip
                            ...
                            v-bind="props"
                            prepend-avatar="/src/assets/images/chat/chat-icon.png"
                            text="System"
                        ></v-chip>
                        <v-chip
                            v-else
    ...
    <v-dialog v-model="deleteDialog" persistent max-width="600px">
        <v-card class="pa-4">
            <v-row class="ma-0 pa-0">
                ...
                <v-spacer></v-spacer>
                <v-btn @click="deleteDialog = false" icon variant="text" density="comfortable" size="24">
                    <Icons :icon="'close'" :size="16" />
                </v-btn>
            </v-row>
            <v-card-text class="ma-0 pa-0 pt-4 pb-4"> "{{ inputObj.name }}" {{ $t('chatListing.confirmDeleteChatRoom') }} </v-card-text>
            <v-row class="ma-0 pa-0">
                <v-spacer></v-spacer>
    ...
    <perfect-scrollbar>
        <v-list class="chat-listing-lgScroll">
            ...
            >
                <!---Avatar-->
                <template v-slot:prepend>
                    <div style="position: relative; display: inline-block" class="mr-2">
                        <v-avatar color="#f0f5f9" size="large" style="width: 50px; height: 50px; display: flex; flex-wrap: wrap">
                            <template v-if="chat.participants.length">
                                ...
                                >
                                    ...
                                    <img
                                        :src="getProfile(chat.participants.find((participant) => participant.email !== userInfo.email))"
                                ...
                                <template v-else>
                                    ...
                                    <div
                                        v-for="(participant, index) in chat.participants
                                            .filter((participant) => participant.email !== userInfo.email)
                                            .slice(0, 4)"
                                        :key="participant.id"
                                        style="width: 50%; height: 50%; position: relative"
                                    >
                                        <img
                                            :src="getProfile(participant)"
                        ...
                        <v-chip
                            v-if="
                                ...
                                chat.participants.find((participant) => participant.email == userInfo.email).isExistUnReadMessage
                            "
                            color="primary"
                            variant="flat"
                            size="x-small"
                ...
                <div
                    ...
                    :class="{
                        'font-weight-bold': chat.participants.find((participant) => participant.email == userInfo.email)
                            .isExistUnReadMessage
                    }"
                >
...
export default {
    ...
    computed: {
        ...
        nameRules() {
            return [(v) => !!v || this.$t('chatListing.enterChatRoomName')];
        },
        participantsRules() {
            return [(v) => (v && v.length > 0) || this.$t('chatListing.selectAtLeastOneParticipant')];
        }
    },
    watch: {
        chatRoomId: {
