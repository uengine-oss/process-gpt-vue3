<template>
    <div>
        <v-card v-if="!isViewProcess" elevation="10">
        ...
        <v-card v-else elevation="10">
            <div class="pt-5 pl-6 pr-6 d-flex align-center">
                <h6 class="text-h6">{{ selectedProc.mega.label }}</h6>
                <v-icon>mdi-chevron-right</v-icon>
                <h6 class="text-h6">{{ selectedProc.major.label }}</h6>
                <div>
                    <v-icon class="cursor-pointer">mdi-chevron-right</v-icon>
                    <v-menu activator="parent">
                        <v-list v-if="selectedProc.major.sub_proc_list" 
                            density="compact" 
