<template>
    <div class="definition-map-wrapper">
                ...
                >
                    ...
                    <template v-if="!globalIsMobile.value">
                        ...
                        <v-tooltip location="bottom" v-if="!useLock">
                            <template v-slot:activator="{ props }">
                                <v-btn v-bind="props" icon variant="text" size="24"
                    ...
                    <div v-else class="d-flex align-center ga-2" style="justify-content: flex-end;">
                        <v-btn v-if="enableEdit && orphanProcessCount > 0"
                            @click="openOrphanDialog"
                            color="gray" rounded="pill" variant="flat" size="small"
                        >{{ $t('processDefinitionMap.mobileOrphanBtn') }} ({{ orphanProcessCount }})</v-btn>

                        <v-btn v-if="!gs"
                            @click="capturePng"
                            color="gray" rounded="pill" variant="flat" size="small"
                        >{{ $t('processDefinitionMap.mobileDownloadImageBtn') }}</v-btn>

                        <v-btn v-if="isExecutionByProject"
                            @click="closePDM()"
                            color="gray" rounded="pill" variant="flat" size="small"
                        >{{ $t('processDefinitionMap.mobileCloseBtn') }}</v-btn>

                        <v-btn v-if="useLock && !lock && isAdmin && !isViewMode"
                            @click="openAlertDialog"
                            color="primary" rounded="pill" variant="flat" size="small"
                        >{{ $t('processDefinitionMap.mobileEditBtn') }}</v-btn>

                        <v-btn v-if="useLock && lock && isAdmin && userName == editUser"
                            @click="openAlertDialog"
                            color="primary" rounded="pill" variant="flat" size="small" class="cp-lock"
        ...
        <!-- 미분류 프로세스 관리 다이얼로그 -->
        <v-dialog v-model="orphanDialog.show"
            :fullscreen="isMobile"
                ...
                <v-card-text class="pa-0" style="overflow-y: auto">
                    <v-list v-if="orphanProcesses.length > 0">
                        <template v-for="proc in orphanProcesses" :key="proc.id">
                            <v-list-item class="orphan-process-item"
                            ...
                            >
                                ...
                                <v-list-item-subtitle class="text-caption" style="white-space: normal; word-break: break-all;">ID: {{ proc.id }}</v-list-item-subtitle>

                                <template #append>
                                    <v-btn color="gray" rounded variant="flat" size="small" @click="toggleAssignPanel(proc)">
                                        {{ assignDialog.expandedId === proc.id ? $t('processDefinitionMap.closeAssign') : $t('processDefinitionMap.assignCategory') }}
                                    </v-btn>
                                </template>
                            </v-list-item>
                            <!-- 인라인 분류 패널 -->
                            <div v-if="assignDialog.expandedId === proc.id" class="pa-4" style="background: rgba(var(--v-theme-primary), 0.1);">
                                <v-select
                                ...
                                >
                                    <template #prepend-item>
                                        <v-list-item @click="showNewMegaInput = true">
                                            ...
                                            <v-list-item-title class="text-primary">{{ $t('processDefinitionMap.createNewMega') }}</v-list-item-title>
                                        </v-list-item>
                                        <v-divider class="my-1" />
                                    </template>
                                </v-select>
                                <v-text-field v-if="showNewMegaInput"
                                ...
                                >
                                    <template #append-inner>
                                        <v-btn variant="text" color="primary" size="small" @click="createNewMega">{{ $t('processDefinitionMap.createBtn') }}</v-btn>
                                    </template>
                                </v-text-field>
                                <v-select
                                ...
                                >
                                    <template #prepend-item>
                                        <v-list-item @click="showNewMajorInput = true" :disabled="!assignDialog.selectedMega">
                                            <template #prepend><v-icon color="primary">mdi-plus</v-icon></template>
                                            <v-list-item-title class="text-primary">{{ $t('processDefinitionMap.createNewMajor') }}</v-list-item-title>
                                        </v-list-item>
                                        <v-divider class="my-1" />
                                    </template>
                                </v-select>
                                <v-text-field v-if="showNewMajorInput && assignDialog.selectedMega"
                                ...
                                >
                                    <template #append-inner>
                                        <v-btn variant="text" color="primary" size="small" @click="createNewMajor">{{ $t('processDefinitionMap.createBtn') }}</v-btn>
                                    </template>
                                </v-text-field>
                                <div class="d-flex justify-end mt-3">
                                    <v-btn color="primary" rounded="pill" variant="flat" size="small"
