<template>
    <div class="pa-4">
            ...
            <v-row align="center" no-gutters>
                <v-col>
                    ...
                    <p class="text-body-2 text--secondary ma-0 mt-1">
                        {{
                            isUseDataSource
                                ? $t('accountTab.experimentalFeatureExplanationOn')
                                : $t('accountTab.experimentalFeatureExplanationOff')
                        }}
                    </p>
                </v-col>
                <v-col cols="auto">
                    <v-switch v-model="isUseDataSource" color="primary" hide-details @change="toggleDataSourceUsage"></v-switch>
                </v-col>
            </v-row>

            <!-- 실험 기능 안내 -->
            <v-divider class="my-3"></v-divider>
            <v-alert dense outlined type="info" color="gray">
                <v-row class="ma-0 pa-0">
                    <!-- <v-icon color="orange" class="mr-2">mdi-information-outline</v-icon> -->
                    <span class="text-body-2">
                        <div>{{ $t('accountTab.experimentalFeature') }}:</div>
                        {{ $t('accountTab.experimentalFeatureWarning') }}
                    </span>
                </v-row>
            </v-alert>
        </v-card>
        ...
        <v-row class="ma-0 pa-0 mb-4">
            <v-spacer></v-spacer>
            <v-btn color="primary" variant="flat" rounded @click="openAddDialog" :disabled="!isUseDataSource">
        ...
        <!-- 데이터소스 다이얼로그 -->
        <v-dialog v-model="dialog" max-width="800" persistent :fullscreen="isMobile">
            <v-card class="ma-0 pa-0">
                <v-row class="ma-0 pa-4 pb-0 align-center">
                    <v-card-title class="pa-0">{{ $t('accountTab.addConnectionInfo') }} </v-card-title>
                    <v-chip density="compact" color="orange" text-color="white" class="ma-0 ml-2" style="font-size: 12px">
                        <v-icon small left>mdi-flask</v-icon>
                        {{ $t('accountTab.experimentalFeature') }}
                    </v-chip>
                    <v-spacer></v-spacer>
                    <v-btn @click="dialog = false" class="ml-auto" variant="text" density="compact" icon>
                        <v-icon>mdi-close</v-icon>
                    </v-btn>
                </v-row>

                <v-card-text max-height="500" class="pa-4 pb-0">
                    <v-text-field v-model="newDataSource.name" label="Name" outlined dense />

                    <!-- Method + Endpoint -->
                    <v-row>
                        <v-col cols="12">
                            <v-text-field v-model="newDataSource.endpoint" label="URL" outlined dense />
                        </v-col>
                    </v-row>
                    ...
                    <div class="mt-4">
                        ...
                        <div class="connection-info-tab-headers pt-2">
                            <div
                                ...
                                style="gap: 12px"
                            >
                                <v-text-field v-model="header.key" label="Header Name" outlined dense hide-details style="flex: 1" />
                                <v-text-field v-model="header.value" label="Header Value" outlined dense hide-details style="flex: 1" />
                                <v-btn
                ...
                <v-row class="ma-0 pa-0 mt-2 pb-4">
                    <v-spacer></v-spacer>
                    <v-btn @click="saveDataSource" :loading="saving" color="primary" variant="flat" rounded class="mr-4">{{
                        $t('accountTab.save')
                    }}</v-btn>
                </v-row>
            </v-card>
        ...
        <div :class="{ 'datasource-disabled': !isUseDataSource }">
            <v-card class="mt-4" elevation="2" v-for="(ds, index) in dataSources" :key="index">
                <v-card-text class="pa-4 pb-0">
                    ...
                    <v-row class="ma-0 pa-0 mb-4 align-center" v-if="ds.headers && ds.headers.length > 0">
                        ...
                        <v-chip v-for="(h, i) in ds.headers" :key="'h-' + i" x-small class="ml-2 connection-info-tab" outlined>
                            {{ h.key }}: {{ '*'.repeat(h.value.length) }}
                        </v-chip>
                    </v-row>

                    <v-row v-if="ds.auth.enabled" class="ma-0 pa-0 mb-4 align-center">
                ...
                <v-card-actions class="pa-4 pt-0">
                    ...
                    <v-btn
                        @click="requestDeleteDataSource(ds)"
                    ...
                    <v-btn icon small @click="editDataSource(ds)" :disabled="!isUseDataSource">
        ...
        return {
            ...
            dataSources: [
                ...
                },
                {
                    ...
                    headers: [{ key: 'APPID', value: 'your_api_key_here' }],
                    parameters: [],
                    auth: {
                        enabled: false,
                        username: '',
