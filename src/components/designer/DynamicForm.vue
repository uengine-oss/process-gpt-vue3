...
export default {
    ...
    methods: {
        ...
        executeCode(name) {
            const codeInfo = this.codeInfos[name];
            if (!codeInfo || !codeInfo.code) return null;

            // Security hardening: do not execute arbitrary code strings from form definitions.
            // This prevents code-injection vectors flagged by SAST.
            console.warn(`[DynamicForm] Blocked custom code execution for "${name}" (eventType: ${codeInfo.eventType || 'unknown'})`);
            return null;
        },
