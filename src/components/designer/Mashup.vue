...
export default {
    ...
    emits: ['onChangeKEditorContent', 'onSaveFormDefinition', 'onInitKEditorContent'],
    ...
    methods: {
        ...
        kEditorContentToHtml(kEditorContent, isWithSection = true) {
            ...
            doc.querySelectorAll("div[id^='vuemount_']").forEach((vueRenderElement) => {
                ...
                componentRef.settingInfos.forEach((settingInfo) => {
                    if (settingInfo.settingType === 'items')
                        ...
                        else newElement.innerText = componentRef[settingInfo.dataToUse];
            ...
            doc.querySelectorAll('[class^="col-sm-"]').forEach((node) => {
                node.setAttribute(
                    ...
                    node
                        .getAttribute('class')
                        .split(' ')
                        .filter((className) => className.includes('col-sm-'))
                        .join(' ')
                );
            ...
            if (isWithSection) return doc.body.innerHTML.replace(/&quot;/g, `'`).replace('<br>', '\n');
            else return doc.body.firstChild.innerHTML.replace(/&quot;/g, `'`).replace('<br>', '\n');
        },
        ...
        renderWithDynamicComponent(snippetContent, vueRenderUUID, isCopy = false) {
            ...
            if (isCopy) {
                snipptDom.body.querySelectorAll('[name]').forEach((el) => el.setAttribute('name', el.getAttribute('name') + `-copy`));
            } else {
                const nameSeq = Object.values(window.mashup.componentRefs).filter((componentRef) => componentRef.localName).length + 1;
                snipptDom.body.querySelectorAll('[name]').forEach((el) => el.setAttribute('name', el.getAttribute('name') + `-${nameSeq}`));
            }
        ...
        getUserInputableComponentNames() {
            return Object.values(window.mashup.componentRefs)
                .filter(
                    (componentRef) =>
                        componentRef.tagName.toLowerCase() !== 'code-field' &&
                        componentRef.localName !== undefined &&
                        componentRef.localAlias !== undefined
                )
    ...
    mounted() {
        ...
        window.mashup.kEditor.keditor({
            ...
            onInitSidebar: function (self) {
                ...
                axios.get(`${window.location.origin}/snippets/default/snippets.html`, { headers }).then(function (resp) {
                    ...
                    if (self.options.snippetsTooltipEnabled || self.options.tabTooltipEnabled) {
                        self.body
                            .find('#' + self.options.snippetsListId)
            ...
            containerSettingInitFunction: function (form, keditor) {
                ...
                form.append('<div class="form-horizontal">' + '   <h3>Here is Setting Container Panel!</h3>' + '</div>');
            },
            ...
            componentSettingShowFunction: function (form, container, keditor) {
                ...
                try {
                    const vueRenderElement = new DOMParser()
                        .parseFromString(container[0].innerHTML, 'text/html')
            ...
            onInitComponent: function (comp) {
                ...
                if (comp.hasClass('keditor-container')) {
                    ...
                    if (comp && comp.length > 0 && comp[0].querySelectorAll("div[id^='vuemount_']").length > 0) {
                        ...
                        if (prevVueRenderId) {
                            const renderedComponents = window.mashup.kEditor[0].children[0].querySelectorAll(
                                `div[id='${prevVueRenderId}']`
                            );
                            if (renderedComponents.length == 2) {
                                let htmlToRender = window.mashup.kEditorContentToHtml(
                                    renderedComponents[0].outerHTML,
                                    !(renderedComponents[0].tagName.toLowerCase() === 'section')
                                );
