<template>
    <div v-if="inputItems.length > 0" class="input-group" style="width: 100%; margin-top: 5px">
        <div v-for="item in inputItems" :key="item.key" class="pa-1">
            <div v-if="Array.isArray(item.value)">
                <v-card variant="outlined" class="pa-2">
                    <v-row class="ma-0 pa-0">
                        ...
                        <v-spacer></v-spacer>
                        <v-btn @click="addSubItem(item)" color="primary" variant="text" icon density="comfortable">
                        ...
                        <template v-for="(subItem, index) in item.value" :key="index">
                            <div class="d-flex align-center mt-2">
                                <v-text-field
                                    ...
                                    outlined
                                ></v-text-field>
                                <v-btn @click="deleteSubItem(item, index)" class="ml-auto" icon variant="text" density="comfortable">
                                    <Icons :icon="'trash'" />
                                </v-btn>
                            </div>
                        </template>
            ...
            <v-text-field v-else v-model="item.value" :label="item.key" :disabled="isComplete" hide-details></v-text-field>
        </div>
    </div>
    <div v-else>
