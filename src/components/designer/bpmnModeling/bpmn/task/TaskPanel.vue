    ...
    >
        <template v-slot:edit-property>
            ...
            <div v-if="value._type == 'org.uengine.kernel.ScriptActivity'">
                <v-select v-model="value.language" :items="langList" item-value="langList.index" label="실행 언어"></v-select>
                <v-textarea v-model="value.script" label="스크립트" dense></v-textarea>
                <v-textarea v-model="value.out.name" label="실행 결과 변수 매핑" dense></v-textarea>
            </div>
            ...
            <div v-if="value._type == 'org.uengine.kernel.bpmn.SendTask'">
                <v-text-field v-model="value.inputPayloadTemplate" label="입력 데이터 (JSON/XML)"></v-text-field>
                <v-text-field v-model="value.dataInput.name" label="결과 입력 변수"></v-text-field>
            </div>
            ...
            <div v-if="value._type == 'org.uengine.kernel.HumanActivity'">
                <v-text-field v-model="value.role.name" label="역할" readonly></v-text-field>
                <label>파라미터</label>
                <bpmn-parameter-contexts :parameter-contexts="value.parameters" :value="value" :multi="true"></bpmn-parameter-contexts>
            </div>
            ...
            <div v-if="value._type == 'org.uengine.kernel.bpmn.ServiceTask'">
                <v-text-field v-model="value.role.name" label="역할 (추후 레인에서 자동설정)" readonly></v-text-field>
                ...
                <bpmn-parameter-contexts :parameter-contexts="value.outputMapping" :value="definition"></bpmn-parameter-contexts>
                <v-checkbox v-model="value.skipIfNotFound" label="리소스 없을 경우 (404) 오류 처리 하지 않음"></v-checkbox>
            </div>
