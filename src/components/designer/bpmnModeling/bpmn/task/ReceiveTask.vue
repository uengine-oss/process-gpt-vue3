<template>
    <div>
        ...
        <bpmn-task-panel v-if="drawer" v-model="value" :linkList="links" @close="closePanel" @giveJSONHint="giveJSONHint"></bpmn-task-panel>
    </div>
</template>
...
export default {
    ...
    data: function () {
        return {
            links: null,
            receive_image: '/static/image/symbol/message.png'
        };
    },
    watch: {
        drawer: function (opened) {
            if (opened) {
                ...
                this.$root
                    .codi('eureka/apps/' + this.value.role.name)
                    .get()
    ...
    created: function () {
        if (!this.value.role) this.value.role = { name: '' };
    },
    mounted: function () {},
    methods: {
