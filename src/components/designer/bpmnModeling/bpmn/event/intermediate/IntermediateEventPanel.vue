<template>
    <bpmn-common-panel v-model="value" :image="image" :is-read-only="readOnly" :width-style="widthStyle" @close="closePanel">
        <template v-slot:edit-property>
            ...
            <div v-if="value._type == 'org.uengine.kernel.bpmn.MessageIntermediateCatchEvent'">
                <v-text-field v-model="value.servicePath" label="Service Path"></v-text-field>
                <v-text-field v-model="value.correlationKey" label="Correlation Key"></v-text-field>
                <v-text-field v-model="value.dataOutput.name" label="Data for Output"></v-text-field>
                <label>Data Mapping</label>
                <bpmn-parameter-contexts :parameter-contexts="value.dataOutputMapping" :definition="definition"></bpmn-parameter-contexts>
            </div>
            ...
            <div v-if="value._type == 'org.uengine.kernel.bpmn.ConditionalCatchEvent'">
                ...
                <v-checkbox v-model="complexCondition" label="Complex Rule"></v-checkbox>
                <div v-if="complexCondition">
                    <org-uengine-kernel-Or v-model="value.condition" :definition="definition"></org-uengine-kernel-Or>
                </div>
                <org-uengine-kernel-Evaluate v-else v-model="value.condition" :definition="definition"></org-uengine-kernel-Evaluate>
                <v-text-field v-model="value.pollingIntervalInSecond" label="Polling Interval (in seconds)" type="number"></v-text-field>
            </div>
            ...
            <div v-if="value._type == 'org.uengine.kernel.bpmn.MessageIntermediateThrowEvent'">
                <v-text-field v-model="value.role.name" label="Target Service Pool" readonly></v-text-field>
                <v-text-field v-model="value.uriTemplate" label="호출 URI 패턴"></v-text-field>
                <v-checkbox
                ...
                <v-select
                    ...
                    label="호출 서비스 선택"
                ></v-select>
                <v-select v-model="value.method" :items="methodList" label="호출 메서드"></v-select>
                <div v-if="'GET,DELETE'.indexOf(value.method) == -1">
                    <label>Input</label>
                    <v-switch v-model="value.useJSONTemplate" label="Use JSON Template"></v-switch>
                    <v-textarea v-if="useJSONTemplate" v-model="value.uriTemplate" label="입력 데이터 (JSON template)"></v-textarea>
                ...
                <div v-for="(header, index) in value.headers" :key="index">
                    <v-text-field v-model="value.headers[index].name" label="Name"></v-text-field>
                    <v-text-field v-model="value.headers[index].value" label="Value"></v-text-field>
                    <v-btn @click="deleteHeader(index)">delete</v-btn>
                </div>
                <br />
                <v-btn @click="addHeader" color="primary" class="my-5">Add header</v-btn>
                <br />
                <label>Output</label>
                <bpmn-parameter-contexts :parameter-contexts="value.outputMapping" :definition="definition"></bpmn-parameter-contexts>
                <v-checkbox v-model.number="value.skipIfNotFound" label="리소스 없을 경우 (404) 오류 처리 하지 않음"></v-checkbox>
                <v-checkbox v-model.number="value.queuingEnabled" label="Queuing Enabled"></v-checkbox>
            </div>
            ...
            <div v-if="value._type == 'org.uengine.kernel.bpmn.TimerIntermediateEvent'">
                ...
                ></v-select>
                <v-text-field v-if="value.scheduleType == 'cron'" v-model="value.expression" label="Quartz Cron Expression">
            ...
            <v-text-field
                v-if="
                    value._type != 'org.uengine.kernel.bpmn.CatchingErrorEvent' &&
            ...
            <v-text-field
                v-if="
                    value._type == 'org.uengine.kernel.bpmn.EscalationIntermediateCatchEvent' ||
