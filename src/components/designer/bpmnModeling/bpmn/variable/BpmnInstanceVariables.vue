<template>
    <v-card outlined class="px-3 py-3 my-2">
                ...
                <v-row>
                    <v-col v-if="forSubProcess" cols="4">
                        <v-select
                            ...
                            v-model="parameters[idx].argument.text"
                            :items="calleeDefinition.processVariableDescriptors"
                            item-text="name"
                            item-value="name"
                            label="피호출측 변수"
                        ></v-select>
                        <v-text-field v-else v-model="parameters[idx].argument.text" label="input" id="input"></v-text-field>
                    </v-col>

                    <v-col v-else cols="4">
                        <v-text-field name="input" id="input" label="아규먼트" v-model="parameters[idx].argument.text"></v-text-field>
                    </v-col>

                    <v-col v-if="parameterContext.transformerMapping" cols="3">
                        <v-select
                            :value="parameters[idx].transformerMapping.transformer"
                    ...
                    <v-col cols="2">
                        <v-select
                            :value="parameters[idx].direction"
                            @change="changed()"
                            style="min-width: 20px"
        ...
        <v-card-actions>
            <v-spacer></v-spacer>
            <v-btn text color="primary" class="my-3" @click="addMapping"> Add Mapping </v-btn>
        </v-card-actions>
    </v-card>
</template>
