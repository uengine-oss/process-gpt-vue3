        ...
        <v-navigation-drawer absolute permanent right style="width: 600px">
            ...
            <v-list class="pt-0" dense flat>
                ...
                <div v-if="tab == 0" style="margin-top: 10px">
                    <v-card flat>
                        <v-card-text>
                            <v-text-field v-if="tracingTag !== null" v-model="tracingTag" label="액티비티 ID*" counter="50"></v-text-field>

                            <!-- v-if="value.condition._type == 'org.uengine.kernel.Evaluate'" start -->
                            <div
                                v-if="
                                    value.condition._type == 'org.uengine.kernel.Evaluate' ||
                                    value.condition._type == 'org.uengine.five.kernel.LoopCountEvaluate'
                                "
                            >
                                <v-text-field v-model="value.name" label="Sequence Name" autofocus></v-text-field>
                                <v-checkbox v-model="isOtherwise" label="Otherwise Condition"></v-checkbox>
                                <v-text-field v-model="value.priority" label="Priority" type="number"></v-text-field>
                                <v-checkbox v-model="isComplexCondition" label="Complex Rule"></v-checkbox>
                            ...
                            <div v-if="value.condition._type == 'org.uengine.kernel.ExpressionEvaluateCondition'">
                                <v-text-field v-model="value.name" label="릴레이션 이름" autofocus></v-text-field>
                                <v-checkbox v-model="isOtherwise" label="Otherwise Condition"></v-checkbox>
                                <v-text-field
                                    v-if="!isOtherwise"
                            ...
                            <div v-if="value._type == 'org.uengine.kernel.bpmn.MessageFlow'">
                                <v-text-field v-model="value.name" label="Message Flow Name" autofocus></v-text-field>
                                <v-switch v-model="value.localCall" label="Local Call"></v-switch>
                            </div>
                ...
                <div v-if="tab == 1">
                    <v-card>
                        <bpmn-mapper v-model:drawer="drawer" :definition="definition" v-model="value"></bpmn-mapper>
                    </v-card>
                </div>
                <div v-if="tab == 2">
                    <v-card flat>
                        <v-card-text>
                            ...
                            <v-row>
                                <v-col cols="6" class="py-0">
                                    <v-text-field label="width" v-model="width" type="number"></v-text-field>
                                </v-col>
                                <v-col cols="6" class="py-0">
                                    <v-text-field label="height" v-model="height" type="number"></v-text-field>
                                </v-col>
                            </v-row>
                            <v-text-field
                                v-for="(item, idx) in style"
                                :key="idx"
