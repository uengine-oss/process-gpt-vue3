<template>
    <div style="height: calc(100vh - 155px); position: relative;">
        <v-btn @click="save" icon variant="text" density="comfortable" style="position: absolute; top: 12px; right: 12px;">
            <Icons :icon="'close'" class="cursor-pointer" :size="16"/>
        </v-btn>
        <v-card-text style="overflow: auto; height: 100%; width: 700px" class="pa-4">
            <div v-if="!(isGPTMode && panelName == 'gpt-user-task-panel')">
                ...
                <div style="margin:0px 0px 10px 0px;">{{ $t('BpnmPropertyPanel.role') }}: {{ role.name }}</div>
