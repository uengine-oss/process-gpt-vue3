            ...
            <div>
                ...
                <v-row>
                    <bpmn-parameter-contexts :for-sub-process="true" :definition-variables="definitionVariables"
                        ...
                        :parameter-contexts="copyUengineProperties.variableBindings"></bpmn-parameter-contexts>
                </v-row>
            </div>
            <div>
                <v-row class="ma-0 pa-0">
                ...
                <v-row>
                    <bpmn-role-parameter-contexts :role-bindings="copyUengineProperties.roleBindings"
                        :is-view-mode="isViewMode" :callee-definition-roles="calleeDefinitionRoles"
...
export default {
    ...
    async mounted() {
        ...
        if (!this.copyUengineProperties.variableBindings)
            this.copyUengineProperties.variableBindings = []
            ...
            (process.laneSets || []).forEach(laneSet => {
                (laneSet.lanes || []).forEach(lane => {
                    ...
                    if (lane?.name?.length > 0)
                        me.calleeDefinitionRoles.push(lane.name);
                });
            });
        });
        ...
        if (this.copyUengineProperties.definitionId)
            this.setDefinitionInfo(this.copyUengineProperties.definitionId);
    },
    computed: {
        inputData() {
