<template>
    <div>
        <v-layout>
            <opengraph v-if="drawer" :enableContextmenu="false" v-on:connectShape="onConnectShape" v-on:canvasReady="bindEvents">
                ...
                <div v-for="(relation, idx) in mapperRelations" :key="idx">
                    <edge-element
                        ...
                        movable
                        deletable
                        v-model:vertices="relation.vertices"
                        :_style="{
                            'edge-type': 'straight'
                        }"
                ...
                <rectangle-element
                    ...
                    v-model:x="transformer.location.x"
                    v-model:y="transformer.location.y"
                    :width="100"
                    :height="20"
        ...
        <div class="ma-6" style="padding-bottom: 30px">
            ...
            <div v-if="editingTransformer" style="margin-top: 10px">
                ...
                ></v-select>
                <v-text-field v-model="editingTransformer.name" label="이름"></v-text-field>
            </div>

            <div v-if="editingVariable" style="margin-top: 10px">
                <v-text-field label="Name" v-model="editingVariable.name"></v-text-field>
                <v-text-field label="Display Name" v-model="editingVariable.displayName.text"></v-text-field>
                <v-text-field label="Default Value In String" v-model="editingVariable.defaultValueInString"></v-text-field>
                <v-switch v-model="value.global" label="Global"></v-switch>
                <v-select v-model="value.persistOption" label="Persist Option"></v-select>
            </div>
        </div>
    </div>
</template>
...
<script>
import MapperContextMenuVue from './MapperContextMenu.vue';
...
export default {
    ...
    data: function () {
        return {
            ...
            menuList: [{ name: 'Edit' }, { name: 'Delete' }]
        };
    },
    computed: {
        left: function () {
    ...
    methods: {
        ...
        selectedVariable: function (variable) {
            if (variable.typeClassName == 'class') this.editingVariable = variable;
        },
