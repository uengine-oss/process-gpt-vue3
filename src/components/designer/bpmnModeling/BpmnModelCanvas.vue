            ...
            <div v-for="relationId in Object.keys(value.relations)" :key="relationId">
                ...
                <!-- <bpmn-message-flow
                    v-if="
                        ...
                        value.relations[relationId]._type == 'org.uengine.kernel.bpmn.MessageFlow'
                    "
...
export default {
    ...
    data() {
        return {
            ...
            // ModelCanvas
            saveItems: [{ title: 'Save to Server' }, { title: 'Download model File' }, { title: 'Duplicate' }],
    ...
    methods: {
        ...
        loadStatus: function (definition) {
            var me = this;
            me.$root
                .codi('instance{/id}/variables')
                .get({ id: me.instanceId })
                .then(function (response) {
                    for (var key in response.data) {
                        ...
                        if (tracingTagAndPropName && tracingTagAndPropName.length > 1) {
                            ...
                            if (activity) {
                                if (!activity._instanceInfo) activity._instanceInfo = {};
                        ...
                        $.each(Object.values(value.elements), function (i, child) {
                            ...
                            if (child) {
                                if (child._type == 'org.uengine.kernel.HumanActivity') {
                                    child.role.name = me.getWhereRoleAmIByTracingTag(child.tracingTag);
        ...
        instanceOpenMenu(event) {
            ...
            if (
                ...
                if (_status == 'Completed') {
                    ...
                    $('.custom-menu')
                        .finish()
                        .show(0) // In the right position (the mouse)
                        .css({
        ...
        settingEventStormingData(projectData) {
            ...
            me.$nextTick(function () {
                ...
                if (modelForElements.Relation && modelForElements.Relation.length > 0) {
                    ...
                    // relation
                    vertices = [
                        [startEventElement.elementView.x, startEventElement.elementView.y],
                        [cmdTaskElement.elementView.x, cmdTaskElement.elementView.y]
                    ];
                    edgeInfo = {
                        component: 'bpmn-relation',
                    ...
                    commandToEvent.forEach(function (item, index) {
                        ...
                        vertices = [
                            [cmdTaskElement.elementView.x, cmdTaskElement.elementView.y],
                            [commandEventElement.elementView.x, commandEventElement.elementView.y]
                        ];
                        edgeInfo = {
                            component: 'bpmn-relation',
                            from: cmdTaskElement,
                    ...
                    var eventToPolicy = modelForElements.Relation.filter(function (relation) {
                        if (relation.sourceElement._type.endsWith('Event') && relation.targetElement._type.endsWith('Policy')) {
                    ...
                    eventToPolicy.forEach(function (item, index) {
                        commandEventList.forEach(function (event) {
                            if (event.name == item.sourceElement.name) {
                                ...
                                vertices = [
                                    [event.elementView.x, event.elementView.y],
                                    [policyEventInfo.x, policyEventInfo.y]
                                ];
                                edgeInfo = {
                                    component: 'bpmn-relation',
                                    from: event,
                    ...
                    var policyToCommand = modelForElements.Relation.filter(function (relation) {
                        if (relation.sourceElement._type.endsWith('Policy') && relation.targetElement._type.endsWith('Command')) {
                    ...
                    commandToEvent.forEach(function (item, index) {
                        cmdTaskList.forEach(function (task) {
                            if (item.sourceElement.name == task.name) {
                                ...
                                // relation
                                vertices = [
                                    [task.elementView.x, task.elementView.y],
                                    [commnadEventInfo.x, commnadEventInfo.y]
                                ];
                                edgeInfo = {
                                    component: 'bpmn-relation',
                                    from: task,
                ...
                if (element) {
                    if (element._type.endsWith('Role')) {
                        if (element.name == boundedContext.name) {
                            if (
                                innerElement.elementView.y > element.elementView.y &&
