    ...
    <div class="canvas-panel">
        <v-container style="width: 500px; height: 100px; z-index: 1000;">
            <v-row v-if="!monitor" no-gutters>
                <v-col>
                    <v-text-field style="margin-right: 10px; margin-top: 8px; max-width: 120px" label="Process Name"
                        v-model="projectName"></v-text-field>
                </v-col>
                <v-col>
                    <v-menu offset-y open-on-hover left>
                        <template v-slot:activator="{ props }">
                            <v-btn class="bpmn-btn" text style="margin-right: 15px; margin-top: 15px;" color="primary" dark
                                v-on="props" @click="openDefinitionSettings">
                ...
                <v-col>
                    <v-menu offset-y open-on-hover left>
                        <template v-slot:activator="{ props }">
                            <v-btn class="bpmn-btn" text style="margin-right: 15px; margin-top: 15px;" color="orange" dark
                                v-on="props" @click="openProcessVariables">
                ...
                <v-col>
                    <v-menu offset-y open-on-hover left>
                        <template v-slot:activator="{ props }">
                            <v-btn class="bpmn-btn" v-if="readOnly" style="margin-right: 5px; margin-top: 15px;"
                                color="primary" @click="saveComposition('fork')" text>
                                <v-icon>{{ icon.fork }}</v-icon>
                                <div class="bpmn-btn-text">FORK</div>
                            </v-btn>
                            <v-btn class="bpmn-btn" v-else style="margin-right: 5px; margin-top: 15px;" color="primary"
                                @click="saveComposition('save')" v-on="props" text>
            ...
            <v-row v-else no-gutters justify="end" style="margin-right: 15px;">
                <!--인스턴스 이름-->
                <v-text-field style="margin-right: 10px; margin-top: 8px; max-width: 300px" v-model="instanceName"
                    label="Instance Name" type="text" readonly></v-text-field>
                ...
                <div style="margin-right: 10px;">
                    ...
                    <v-menu offset-y open-on-hover left>
                        <template v-slot:activator="{ props }">
                            <v-btn text style="margin-right: 15px; margin-top: 15px;" id="instanceVariables" color="primary"
                                dark v-on="props" @click="openInstanceVariables">
                                Variables
                            </v-btn>
                        </template>
                    </v-menu>
                    ...
                    <v-menu offset-y open-on-hover left>
                        <template v-slot:activator="{ props }">
                            <v-btn text style="margin-right: 15px; margin-top: 15px;" id="userPicker" color="orange" dark
                                v-on="props" @click="openUserPicker">
                                Role Mappings
                            </v-btn>
                        </template>
                    </v-menu>
                ...
                <!-- 웹페이지 버튼들 끝 -->
                <div v-if="versions" style="margin-right: 10px;">
                    <v-select v-for="version in versions.slice().reverse()" :key="version" v-on="on"
                        v-model="selectedVersion" @change="changeVersion" :value="version"
                        style="margin-top: 10px; margin-right: 15px; max-width: 150px;">
                        <v-chip v-if="productionVersionId == version.versionId">production</v-chip>
                    </v-select>
                </div>

                <bpmn-instance-variables :id="instanceId" :definition="value" v-if="value"
        ...
        <opengraph focus-canvas-on-select wheelScalable :dragPageMovable="dragPageMovable" :enableContextmenu="false"
            :enableRootContextmenu="false" :enableHotkeyCtrlC="false" :enableHotkeyCtrlV="false" :enableHotkeyDelete="false"
            :slider="false" :movable="!monitor" :resizable="!monitor" :selectable="!monitor" :connectable="!monitor"
            :width="100000" :imageBase="imageBase" :height="100000" v-if="modelValue" ref="bpmnOpengraph"
            ...
            <div v-for="elementId in Object.keys(value.elements)" :key="elementId">
                ...
                <component v-if="elementId && value.elements[elementId] != null"
                    ...
                    :definition="value" :ref="elementId"></component>
            </div>
            ...
            <div v-for="relationId in Object.keys(value.relations)" :key="relationId">
                <bpmn-message-flow
                    ...
                    :value="value.relations[relationId]" :definition="value" :ref="relationId"></bpmn-message-flow>
                <bpmn-relation v-else-if="relationId && value.relations[relationId] != null"
                    :value="value.relations[relationId]" :definition="value" :ref="relationId"></bpmn-relation>
            </div>
        </opengraph>
        ...
        <div class="tools" style="top:100px;">
            <v-card v-if="!monitor">
                ...
                <v-tooltip right>
                    <template v-slot:activator="{ props }">
                        <v-btn text icon right @click.native="undo" v-bind="props">
                ...
                <v-tooltip right>
                    <template v-slot:activator="{ props }">
                        <v-btn text icon right @click.native="redo" v-bind="props">
        ...
        <bpmn-component-changer v-if="value" :data="componentChangerData"></bpmn-component-changer>
...
import ModelCanvas from "../modeling/ModelCanvas.vue"
import BpmnObjectGrid from "./BpmnObjectGrid.vue";
import ModelStorageDialog from "../modeling/ModelStorageDialog.vue";
import ModelCanvasShareDialog from "../modeling/ModelCanvasShareDialog.vue";
import _ from 'lodash'

// props -> components
// value -> components v-model 
    ...
    methods: {
        ...
        */
        ...
        async initiateProcess() {
            ...
            me.$http.post(`/api/instance?defPath=${me.projectId}&simulation=true`, { "roleMapping": map })
            // this.save(
            //     function () {
            //         var def = {};
