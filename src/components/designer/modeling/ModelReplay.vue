<template xmlns:v-on="http://www.w3.org/1999/xhtml"></template>
    ...
    methods: {
        ...
        async validateStorageCondition(item) {
            ...
            var validate = await me.isValidatePath(
                `db://definitions/${me.modelProjectId}/versionLists/${item.version.replaceAll('.', '-')}`
            );
            if (
                !(validate.status && !item.version.replaceAll('.', '-').includes('/') && !item.version.replaceAll('.', '-').includes(':'))
            ) {
        ...
        async addVersion() {
            ...
            try {
                ...
                if (check) {
                    ...
                    me.$refs['modeler-image-generator'].save(me.projectName, me.canvas).then(async function (img) {
                        // var versionValueObj = {
        ...
        async init() {
            ...
            try {
                ...
                if (me.$route.params.queueKey) {
                    me.slider.current = 'null'.includes(me.$route.params.queueKey)
                        ? queueListLength
                    ...
                    try {
                        ...
                        try {
                            ...
                            console.log(
                                ...
                                JSON.parse(child.childValue.item)
                            );
                            // me.watch_off(`db://definitions/${me.projectId}/queue`)
                            // if( child && child.childKey ){
                            //     me.delete(`db://definitions/${me.projectId}/queue/${child.childKey}`)
            ...
            if (action.includes('relation')) {
                if (action.includes('Push')) {
                    ...
                    comment.display =
                        ...
                        source.name +
                        ' " [ ' +
                        source._type.split('.')[4] +
                        ' ] -> "' +
                        target.name +
                        ' " [ ' +
                        target._type.split('.')[4] +
                        ' ]' +
                        '를 연결합니다.';
                    comment.detail = '';
                ...
                if (action.includes('Push')) {
                    ...
                    comment.detail =
                        '[ X: ' +
                        JSON.parse(item.before).x +
                        '/ Y: ' +
                        JSON.parse(item.before).y +
                        '] \n' +
                        ' => [ X: ' +
                        JSON.parse(item.after).x +
                        '/ Y: ' +
                        JSON.parse(item.after).y +
                        '] \n';
                } else if (action.includes('Modify')) {
                    var diff = item;
