<template>
    <v-card class="mx-auto my-8 container-setting-box" elevation="16" max-width="600">
        ...
        <v-card-text>
            <v-text-field
                ref="name"
                :label="$t('ContainerSettingPanel.id')"
                v-model.trim="localContainerProps.name"
                @keyup.enter="save"
                persistent-placeholder
            ></v-text-field>

            <v-text-field
                :label="$t('ContainerSettingPanel.name')"
                v-model.trim="localContainerProps.alias"
                @keyup.enter="save"
                persistent-placeholder
            ></v-text-field>

            <v-checkbox
                :label="$t('ContainerSettingPanel.multDataMode')"
                v-model="localContainerProps.isMultiDataMode"
                @keyup.enter="save"
            ></v-checkbox>
        </v-card-text>
