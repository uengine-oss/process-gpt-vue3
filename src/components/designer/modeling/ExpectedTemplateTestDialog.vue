        ...
        <v-card style="z-index: 2">
            <div style="display: flex; height: 88vh">
                <div style="width: 400px; overflow-y: scroll">
                    <v-card-title style="margin-bottom: -15px"> Expected Template Test </v-card-title>

                    <div>
                        <v-list nav dense>
                            <v-list-group :value="true" style="margin-left: -15px" no-action sub-group>
                                ...
                                <v-list-item
                                    v-for="(filePath, i) in diffList"
                                    :key="i"
                                    :style="selectedFilePath == filePath ? 'background-color: aliceblue;' : ''"
                                >
                                    <div style="cursor: pointer" @click="onSelected('diff', filePath)">
                                        <v-row>
                                            <Icons
                                            ...
                                            <v-col style="margin-left: -35px; margin-right: 25px">
                                                <v-list-item-subtitle style="font-size: x-small">
                            ...
                            <v-list-group :value="true" style="margin-left: -15px" no-action sub-group>
                                ...
                                <v-list-item
                                    v-for="(filePath, i) in existOnlyActual"
                                    :key="i"
                                    :style="selectedFilePath == filePath ? 'background-color: aliceblue;' : ''"
                                >
                                    <div style="cursor: pointer" @click="onSelected('actual', filePath)">
                                        <v-row>
                                            <Icons
                            ...
                            <v-list-group :value="true" style="margin-left: -15px" no-action sub-group>
                                ...
                                <v-list-item
                                    v-for="(filePath, i) in existOnlyExpected"
                                    :key="i"
                                    :style="selectedFilePath == filePath ? 'background-color: aliceblue;' : ''"
                                >
                                    <div style="cursor: pointer" @click="onSelected('expected', filePath)">
                                        <v-row>
                                            <Icons
    ...
    methods: {
        onSelected(type, filePath) {
            ...
            if (type == 'diff') {
                ...
                me.selectedTemplateFramework[0].code =
                    me.templateFrameWorkList[me.selectedTemplateFramework[0].template][expectedFilePath].content;
                me.isDiff = true;
            } else {
                if (type == 'expected') {
