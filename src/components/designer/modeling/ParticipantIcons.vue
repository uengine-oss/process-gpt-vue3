<template>
    <v-layout>
        <v-row justify="end" align="center" id="participant-lists">
            <div v-for="(participant, index) in participantLists" style="width: 30px">
                <v-tooltip bottom>
                    <template v-slot:activator="{ on }">
                        <v-avatar v-if="participantLists.length - 8 == index" size="40">
                            <v-icon> mdi-account-multiple</v-icon>
                        </v-avatar>

                        <v-avatar
                            :style="participantLists.length > 1 ? 'display:block;' : 'display:none;'"
            ...
            <div style="margin-left: 10px">
                <v-tooltip bottom>
                    <template v-slot:activator="{ on }">
                        <v-card
                            :style="participantLists.length >= 6 ? 'display:block;' : 'display:none;'"
