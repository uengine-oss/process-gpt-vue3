<template>
    <div>
        <sub-controller v-if="clone.rotate && !isHexagonalModeling" :image="'rotate.png'" @click.prevent.stop="rotate()"> </sub-controller>
        ...
        <sub-controller v-if="!isHexagonalModeling && selectedList" :image="'content-cut.png'" @click.prevent="cutElement"></sub-controller>
        <sub-controller
            v-if="!isHexagonalModeling && clipboard"
            :image="'content-paste.png'"
            @click.prevent="pasteElement"
        ></sub-controller>
        <sub-controller v-if="!isHexagonalModeling" :image="'copy.png'" @click.prevent.stop="copyPaste()"></sub-controller>

        <div v-if="isHexagonalModeling && type.endsWith('BoundedContext')">
            <sub-controller :image="'../event/aggregate.png'" v-on:click="addAggregateElement"></sub-controller>
        </div>
        <div v-if="isHexagonalModeling && type.endsWith('Aggregate')">
            <sub-controller :image="'../event/event.png'" v-on:click="addEventElement"></sub-controller>
            <sub-controller :image="'../event/command.png'" v-on:click="addCommandElement"></sub-controller>
            <sub-controller :image="'../event/policy.png'" v-on:click="addPolicyElement"></sub-controller>
        </div>

        <div v-if="canvasType == 'cm'">
            <sub-controller :image="'../event/link-box-variant-outline.png'" v-on:click="moveToModel"></sub-controller>
        </div>
        <!--        <sub-controller-->
...
export default {
    ...
    mounted: function () {
        var me = this;
        if (!me.readOnly) this.clone.rotate = true;
    ...
    methods: {
        ...
        ide() {
            ...
            me.$http
                .get(
                    `https://api.${me.getTenantId()}/api/v1/namespaces/default/services/ide-for-${userName}-${userGroup}-${modelingDesigner.projectName.toLowerCase()}-${me.$parent.$parent.value.name.toLowerCase()}`
                )
                .then(function (result) {
                    window.open(
                        `#/IdeLoadingPage?param=ide-for-${userName}-${userGroup}-${modelingDesigner.projectName.toLowerCase()}-${me.$parent.$parent.value.name.toLowerCase()}`,
                        '_blank'
                    );
                })
                .catch(function (e) {
