    ...
    <v-card>
        <v-card-text>
                                ...
                                <v-tooltip bottom>
                                    <template v-slot:activator="{ on }">
                                        ...
                                        <div v-if="item.request == true || item.request == false">
                                            <v-btn color="primary" v-on="on" @click="acceptRequest(item)" small>
                                                수락
                                            </v-btn>
                                        </div>
                                        <div v-else>
                                            <v-btn v-if="item.write" :disabled="item.userName == 'Everyone'" text
                                                v-on="on" @click="item.write = false">
                                                <v-icon>mdi-pencil</v-icon>
                                            </v-btn>
                                            <v-btn v-else text v-on="on" :disabled="item.userName == 'Everyone'"
            ...
            <v-row dense style="height: 40px;">
                <v-text-field v-model="inviteUser.email" label="User to invite" :error-messages="getCallbackMsg"
                    @keydown.enter="add()"></v-text-field>
                <v-select :append-icon="shareIconTypes" v-model="inviteUser.permission" @keydown.enter="add()"
                    :items="permissionTypes" reverse style="width:10px;"></v-select>
                <v-btn small text :style="getPlusStyle" @click="add()">
