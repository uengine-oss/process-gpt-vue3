                            ...
                            <td class="text-center">
                                <v-col align="start" style="padding-top: 10px">
                                    <v-row style="place-items: center">
                                        ...
                                        <v-chip v-if="item.request == true || item.request == false" small style="color: dodgerblue"
                                            >공유요청
                                        </v-chip>
                                    </v-row>
                                ...
                                <v-tooltip bottom>
                                    <template v-slot:activator="{ on }">
                                        <div v-if="item.request == true || item.request == false">
                                            <v-btn color="primary" v-on="on" @click="acceptRequest(item)" small> 수락 </v-btn>
                                        </div>
                                        <div v-else>
                                            <v-btn
                                                v-if="item.write"
                                            ...
                                            <v-btn v-else text v-on="on" :disabled="item.userName == 'Everyone'" @click="item.write = true">
...
export default {
    ...
    watch: {
        'inviteUser.email': _.debounce(function (newVal) {
            var me = this;
            var email = newVal;
