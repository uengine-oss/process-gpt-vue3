            ...
            <v-card fluid style="background-color: white; width: 500px">
                ...
                <div>
                    ...
                    <v-btn
                        ...
                        :loading="startTemplateGenerate"
                        text
                        @click="processTemplateUseOpenAi(openAiPrompt)"
                        >generate</v-btn
                    >
                </div>
                <!-- v-if="openAiResult" -->
                <div style="margin-top: 10px">
                    <v-textarea v-model="openAiResult" style="white-space: pre; margin: 10px; margin-bottom: -10px" dense rows="23" filled>
                    </v-textarea>
        ...
        async processTemplateUseOpenAi(prompt) {
            ...
            if (prompt != '' && prompt != null) {
                if (me.mode == 'uml') {
                    ...
                    content =
                        `Please create an event storming model in json for a ` +
                        prompt +
...
That result must to split the mode with two different bounded context so that the model have to be separated with a core domain from supporting domain. and that response must be json code only without natural language.`;
            ...
            if (me.openaiToken && startGen) {
                ...
                let respones = await axios.post(`https://api.openai.com/v1/completions`, data, { headers: header }).catch(function (error) {
