...
export default {
    ...
    computed: {
        myId() {
            if (this.userInfo.email) return this.userInfo.email;
    ...
    methods: {
                ...
                try {
                    ...
                    if (classInfo) {
                        ...
                        if (classInfo.openClass || !classInfo.connectionKey) {
                            var enrolledUserList = await me.list(
                                `db://labs/${me.getTenantId().split('.')[0] + '/' + me.getClassPath('enrolledUsers')}`
                            );
                            if (!enrolledUserList) {
                                var enrolledUserList = await me.list(
                                    `db://labs/${me.getTenantId().split('.')[0] + '/' + me.getOldClassPath('enrolledUsers')}`
                                );
                            }
        ...
        getClassPath(path) {
            if (this.classId) {
                ...
                if (this.$route.query.classId) var classId = this.$route.query.classId.replace('@', '/');
            }
