    ...
    <v-card>
        <v-card-title class="d-flex justify-space-between align-center">
            <div class="d-flex align-center ga-2">
                ...
                <v-chip v-if="hasResult" size="small" color="success" variant="tonal"> {{ store.queryResult?.row_count || 0 }} 행 </v-chip>
            ...
            <div class="d-flex ga-2">
                ...
                <v-btn variant="text" size="small" :prepend-icon="showSQL ? 'mdi-eye-off' : 'mdi-code-tags'" @click="showSQL = !showSQL">
                    SQL
                </v-btn>
                <v-btn variant="tonal" size="small" prepend-icon="mdi-download" :disabled="!hasResult" @click="exportCSV"> CSV </v-btn>
            </div>
        </v-card-title>
            ...
            <div v-if="showSQL && store.generatedSQL">
                <v-sheet color="grey-darken-4" class="pa-4">
                    <div class="d-flex justify-space-between align-center mb-2">
                        ...
                        <v-btn variant="text" size="x-small" color="grey-lighten-1" prepend-icon="mdi-content-copy" @click="copySQL">
                            복사
                        </v-btn>
                    </div>
                    <pre
                        class="text-body-2 text-grey-lighten-2 ma-0 overflow-auto"
                        style="max-height: 150px"
                    ><code>{{ store.generatedSQL }}</code></pre>
                </v-sheet>
                <v-divider />
            </div>
        </v-expand-transition>
        ...
        <v-card-text class="pa-0">
            ...
            <!-- Error -->
            <v-alert v-else-if="store.error" type="error" variant="tonal" class="ma-4" closable @click:close="store.error = null">
                {{ store.error }}
            </v-alert>
            ...
            <div v-else class="overflow-auto" style="max-height: 500px">
                <v-table density="compact" hover>
                    <thead>
                        <tr>
                            <th v-for="col in store.queryResult?.columns" :key="col" class="text-left font-weight-bold bg-grey-lighten-4">
                                {{ col }}
                            </th>
                        </tr>
                    </thead>
