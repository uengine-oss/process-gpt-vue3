...
export default class WorkItemAgentGenerator extends AIGenerator {
    ...
    constructor(client, language) {
        ...
        this.previousMessages = [{
            ...
            "생성해야할 답변 형식" 이 존재한다면 반드시 항상 예시값을 생성해야한다. 답변이 "" 또는 "null" 이 되어서는 안된다. 답변은 반드시 "생성해야할 답변 형식" 에 적절한 예시값을 채워둔 형식으로 생성해야한다. 반드시 결과를 생성해내야한다.
            "현재 작업 입력 양식" 의 용도는 생성될 예시들의 데이터 경로 파악하기 위함이다. 
            "현재 작업 입력 양식" 이 아닌 "생성해야할 답변 형식" 에 존재하는 "키" 의 예시값들만 생성해야한다. 그 이외의 키에 대한 예시 생성은 기능에 치명적인 오류 발생을 야기하기때문에 절대 생성해선 안된다.
            예를 들어 "생성해야할 답변 형식" 이 
            {
                applicant_name: "...",
                end_date: "...", 
