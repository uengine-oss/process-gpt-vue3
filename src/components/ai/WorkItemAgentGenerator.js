...
export default class WorkItemAgentGenerator extends AIGenerator {
    ...
    constructor(client, language) {
        ...
        this.previousMessages = [{
            ...
            만약 "이전 작업 리스트" 내 "inputFormData" 정보가 존재하지 않는다면, 제공받은 정보들의 전체적인 맥락을 파악하고 "생성해야할 답변 형식"에 가장 적절한 예시를 생성해야한다. 참고할만한 이전 예시값(inputFormData)가 없더라도 전체적인 맥락을 보고 "null" 이 아닌 "생성해야할 답변 형식" 에 맞게 존재하는 키에 적절한 예시값들을 반드시 생성해야한다. 
            "생성해야할 답변 형식" 이 존재한다면 반드시 항상 예시값을 생성해야한다. 답변이 "null" 이 되어서는 안된다. 답변은 반드시 "생성해야할 답변 형식" 에 적절한 예시값을 채워둔 형식으로 생성해야한다.
