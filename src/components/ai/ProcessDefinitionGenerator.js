...
export default class ProcessDefinitionGenerator extends AIGenerator {
  ...
  getDefinitionGuideSection(jsonStructure) {
    ...
    - "기존 프로세스 정보(원본)" 과 사용자가 제공한 엑셀 시트 내용은 구분해야해. 사용자의 생성 요청인지 기존 프로세스가 존재하던것에서 수정인지 판단을 잘해야해. 
    예를 들어 대화내용중 "시트: 1.프로세스정보][{항목:열1," 같이 엑셀 내용은 "기존 프로세스 정보"가 아닌 생성해야할 프로세스 정보라고 판단하여 수정이 아닌 생성이라고 판단해야해. 
    그리고 "기존 프로세스 정보"에 유의미한 정보가 들어가있지 않은 경우 예를 들어 "elements" 또는 "activites" 의 어떠한 내용이 포함되어있지 않은 경우(하나 이상의 유의미한 "element", "activity" 가 존재하지 않는 경우)에는 "기존 프로세스 정보"가 존재하지 않는다고 판단하여야함.
    - 위 조건을 통과하여 "기존 프로세스 정보"가 있다고 판단한 경우에는 반드시 어떠한 경우, 요청에도 "modifications" 항목을 포함하는 수정 형식으로 생성해야함. "기존 프로세스 정보"가 존재하는데 수정이 아닌 생성 형식으로 답변을 하면 치명적인 오류가 발생하기때문에 가장 중요함. 이미 프로세스가 존재하기때문에 다시 생성해선 안됨. 기존 프로세스정보가 존재하는 경우 어떠한 경우, 요청에도 재생성이 아닌 기존 프로세스 정보를 참고하여 수정 형식으로 답변해야함. 기존 프로세스 정보가 없는 경우(프로세스 정보가 업데이트 되지않은 "{{ 기존 프로세스 정보 }}" 상태)에만 새로 생성해야한다. 아닌 경우는 무조건 "modifications 형식으로 생성"
    ...
    - 프로세스 변경: 프로세스 정의의 일 부분이 변경될 때는 다음과 같이 변경된 부분만 리턴해줘:
      ...
      이때 지킬 사항:
       ...
       15. 대화내용중 "시트: 1.프로세스정보][{항목:열1," 같이 엑셀 내용은 "기존 프로세스 정보"가 아닌 생성해야할 프로세스 정보라고 판단하여 수정이 아닌 생성이라고 판단해야해. 
       그리고 "기존 프로세스 정보"에 유의미한 정보가 들어가있지 않은 경우 예를 들어 "elements" 또는 "activites" 의 어떠한 내용이 포함되어있지 않은 경우(하나 이상의 유의미한 "element", "activity" 가 존재하지 않는 경우)에는 "기존 프로세스 정보"가 존재하지 않는다고 판단하여야함.
       위 조건을 통과하여 "기존 프로세스 정보"가 있다고 판단한 경우에는 반드시 어떠한 경우, 요청에도 "modifications" 항목을 포함하는 수정 형식으로 생성해야함. "기존 프로세스 정보"가 존재하는데 수정이 아닌 생성 형식으로 답변을 하면 치명적인 오류가 발생하기때문에 가장 중요함. 이미 프로세스가 존재하기때문에 다시 생성해선 안됨. 기존 프로세스정보가 존재하는 경우 어떠한 경우, 요청에도 재생성이 아닌 기존 프로세스 정보를 참고하여 수정 형식으로 답변해야함. 기존 프로세스 정보가 없는 경우(프로세스 정보가 업데이트 되지않은 "{{ 기존 프로세스 정보 }}" 상태)에만 새로 생성해야한다. 아닌 경우는 무조건 "modifications 형식으로 생성"
    \`\`\`
      { 
