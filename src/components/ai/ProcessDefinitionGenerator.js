...
export default class ProcessDefinitionGenerator extends AIGenerator {
  ...
  getDefinitionGuideSection(jsonStructure) {
    ...
    사용자의 요청을 보고 요청의 내용을 분석하여 알맞는 답변 형식대로 생성해야해. 가장 중요한 원칙은 "사용자 의도 우선"이야.
    우선 사용자의 요청은 3가지 형식으로 들어올 수 있어.
    1. 프로세스 생성: 프로세스를 생성하고 싶다고 요청하는 경우
    2. 프로세스 수정: 프로세스를 수정하고 싶다고 요청하는 경우
    3. 프로세스 질의: 프로세스 내용에 대하여 질문한 경우

    프로세스 생성 또는 수정의 경우 아래 설명에 따라 답변을 생성하고, 프로세스 내용에 대한 질문/설명 요청의 경우 아래의 형식대로 답변해야해.
    특히 사용자가 이미지 첨부 후 "설명", "요약", "분석", "무엇인지 알려줘"와 같은 질의를 하면 절대 프로세스를 새로 생성하거나 수정하지 말고 질의 응답만 해야해.
    사용자가 명시적으로 "생성", "수정", "변경", "추가", "삭제"를 요청한 경우에만 프로세스 JSON(생성/수정)을 반환해.
    판단 우선순위는 아래와 같아:
    1) 질의/설명/분석 요청이면 askProcessDef로만 응답
    2) 생성/수정 의도가 명확할 때만 프로세스 JSON 응답
    3) 의도가 애매하면 생성하지 말고 askProcessDef로 필요한 정보를 질문
    \`\`\`
    {
    ...
    (생성/수정 요청일 때만) 결과는 다음 JSON 형식을 따라야 합니다:
    \`\`\`
    ${jsonStructure}             
    \`\`\`
    ...
    결과 출력 형식 규칙(매우 중요):
    - "프로세스 생성/수정" 요청일 때만 프로세스 JSON(또는 modifications JSON)을 반환해.
    - "프로세스 질의/설명/분석" 요청일 때는 askProcessDef 형식만 반환하고, 프로세스 생성 JSON은 반환하지 마.
    - 사용자의 의도가 애매하면 생성하지 말고 askProcessDef 형식으로 필요한 정보를 질문해.
    - 최종 출력은 markdown 설명 없이 "단일 JSON 객체"만 반환해. (코드블록/부가 설명 금지)
    
    추가 출력 제약:
    - 사용자가 생성/수정을 명시하지 않은 경우 절대로 processDefinitionId, elements, modifications를 생성하지 말 것.
    - 생성/수정 응답일 때만 megaProcessId, majorProcessId를 반드시 포함하고 빈 문자열이면 안됨.
    - 생성/수정 응답에서 분류 판단이 애매하면 megaProcessId="미분류", majorProcessId="미분류"를 사용.
    `.trim();
  }
