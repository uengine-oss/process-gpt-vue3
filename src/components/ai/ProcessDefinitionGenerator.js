...
export default class ProcessDefinitionGenerator extends AIGenerator {
  ...
  getDefinitionGuideSection(jsonStructure) {
    ...
    - 각 액티비티의 담당자 역할
    - 프로세스에서 사용되는 데이터 변수
    - "기존 프로세스 정보"가 존재하는 경우 반드시 "modifications" 항목을 포함하는 수정 형식으로 생성해야함. 이미 프로세스가 존재하기때문에 다시 생성해선 안됨. 기존 프로세스정보가 존재하는 경우 어떠한 경우, 요청에도 재생성이 아닌 기존 프로세스 정보를 참고하여 수정 형식으로 답변해야함. 기존 프로세스 정보가 없는 경우(프로세스 정보가 업데이트 되지않은 "{{ 기존 프로세스 정보 }}" 상태)에만 새로 생성해야한다. 아닌 경우는 무조건 "modifications 형식으로 생성"
    ...
    - 프로세스 변경: 프로세스 정의의 일 부분이 변경될 때는 다음과 같이 변경된 부분만 리턴해줘:
      ...
      이때 지킬 사항:
       ...
       15. "기존 프로세스 정보"가 존재하는 경우 반드시 "modifications" 항목을 포함하는 수정 형식으로 생성해야함. 이미 프로세스가 존재하기때문에 다시 생성해선 안됨. 기존 프로세스정보가 존재하는 경우 어떠한 경우, 요청에도 재생성이 아닌 기존 프로세스 정보를 참고하여 수정 형식으로 답변해야함. 기존 프로세스 정보가 없는 경우(프로세스 정보가 업데이트 되지않은 "{{ 기존 프로세스 정보 }}" 상태)에만 새로 생성해야한다. 아닌 경우는 무조건 "modifications 형식으로 생성"
    \`\`\`
      { 
        "modifications": [
