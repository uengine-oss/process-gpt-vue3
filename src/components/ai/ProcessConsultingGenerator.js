    ...
    constructor(client, language) {
        ...
        this.previousMessages = [
            ...
            **답변 작성 규칙:**
            ...
            2) JSON 앞뒤에 설명문, 인삿말, 코드블록(\`\`\`json), 주석, 마크다운 본문을 절대 추가하지 않는다.
            3) 허용 키는 "content", "answerType" 이다. answerType은 반드시 "consulting" 또는 "generateProcessDef" 중 하나다.
            4) content는 반드시 문자열이어야 한다. 빈 문자열은 금지한다.
            5) JSON.parse가 즉시 가능한 유효한 JSON 문법만 사용한다. (후행 쉼표, 작은따옴표 key/value 금지)
            6) 규칙을 지키지 못한 출력은 시스템 장애를 유발한다. 어떤 경우에도 위 형식을 깨지 않는다.

            [최종 자기검증]
            - 응답 직전에 반드시 스스로 점검: "JSON 객체 1개인가?", "answerType이 허용값인가?", "content가 문자열인가?"
