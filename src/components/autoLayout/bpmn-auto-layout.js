        ...
        try {
            ...
            Object.entries(snapshotToRestore.elements).forEach(([elementId, data]) => {
                ...
                if (typeof data.x === 'number' && typeof data.y === 'number') {
                    ...
                    if (data.width && data.height && (data.width !== element.width || data.height !== element.height)) {
            ...
            elements.forEach((element) => {
                if (element.businessObject && element.businessObject.$type) {
                    ...
                    // 레인 처리 - participant 안에 레인이 있는 경우
                        ...
                        // 방법 1: 좌표 기반 확인
                        elements.forEach((child) => {
                            if (child.id !== element.id && child.businessObject && !child.labelTarget && isElementInLane(child, element)) {
                                childElements.push(child.id);
                            }
                        });
                        ...
                        if (element.businessObject.flowNodeRef) {
                            element.businessObject.flowNodeRef.forEach((ref) => {
                                const childElement = elements.find((el) => el.businessObject && el.businessObject.id === ref.id);
