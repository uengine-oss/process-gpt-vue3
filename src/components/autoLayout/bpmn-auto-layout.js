...
import { computeModelerLayout } from './structured-layout/modeler-layout.js';
import { applyComputedLayout } from './structured-layout/apply-layout.js';
import {
  computeFinalSequenceFlowWaypoints,
  deferSequenceFlowRouting,
  setSequenceFlowRoutingFinalizer,
...
import { capturePhaseGrid, assignPhaseGrid } from './bpmn-auto-layout/phase-grid.js';
import {
  spGetNearestSubProcessId,
  spRouteInternalFeedback,
  layoutSingleSubProcess,
  resizeSubProcessToContents,
} from './bpmn-auto-layout/subprocess-pass.js';
...
(function(global) {
    ...
    try {
      ...
      elements.forEach(element => {
        ...
        if (element.labels && element.labels.length > 0) {
          ...
          element.labels.forEach(l => {
            ...
            const centerX = l.x + lw / 2;
            const centerY = l.y + lh / 2;
            const outsideX = centerX < ex || centerX > ex + ew;
            const outsideY = centerY < ey || centerY > ey + eh;
            nodeWidth = Math.max(nodeWidth, outsideX ? ew + lw + labelGap : lw);
            nodeHeight = Math.max(nodeHeight, outsideY ? eh + lh + labelGap : lh);
          });
        }
      ...
      function buildLaneMembershipContext(allElements) {
        ...
        laneElements.forEach((lane) => {
          ...
          const refs = lane.businessObject && Array.isArray(lane.businessObject.flowNodeRef)
        ...
        function resolveLaneForElement(element) {
          ...
          if (['bpmn:Lane', 'bpmn:Participant', 'phase:Phase', 'phase:PhaseContainer'].includes(element.businessObject.$type)) {
            return null;
          }
          ...
          const candidateLaneIds = participant && participantLaneIdsById[participant.id]
      ...
      elements.forEach(element => {
        if (element.businessObject && element.businessObject.$type) {
          ...
          // 레인 처리 - participant 안에 레인이 있는 경우
            ...
            graph.createGroup(element.id, childElements);
            graph.getGroup(element.id).minimumBreadth = 70;
          }
      ...
      if (!horizontal && layoutOptions.minimumLayerDistance == null) {
        const hasPhaseContainer = elements.some(element => element.type === 'phase:PhaseContainer');
        mergedOptions.minimumLayerDistance = hasPhaseContainer ? 50 : 26;
        mergedOptions.compactLayerDistance = 26;
        mergedOptions.adaptiveLayerSpacing = !hasPhaseContainer;
      }
      const layout = new EnhancedSugiyamaLayout(graph, horizontal, mergedOptions);
      layout.run();
      const phaseGrids = options.phaseGrids || capturePhaseGrid(elements, horizontal);
      const phasePlan = assignPhaseGrid(graph, phaseGrids, horizontal, mergedOptions);
      ...
      function getLaneBandRectForElement(element, pad = lanePadding) {
        ...
        return {
          ...
          minX: x + pad + (horizontal ? LANE_LABEL_HEADER_SIZE : 0),
          maxX: x + width - pad,
          minY: y + pad + (horizontal ? 0 : LANE_LABEL_HEADER_SIZE),
          maxY: y + height - pad,
        };
      }
      ...
      function clampNodePositionToLaneBand(element, position, pad = lanePadding) {
        ...
        return {
          x: maxX < rect.minX
            ? (rect.minX + rect.maxX - (element.width || 0)) / 2
            : clamp(position.x, rect.minX, maxX),
          y: maxY < rect.minY
            ? (rect.minY + rect.maxY - (element.height || 0)) / 2
            : clamp(position.y, rect.minY, maxY),
        };
      }

      function syncLaneAndParticipantBounds(allElements) {
        const lanePad = lanePadding;
        const minLaneSpan = lanePad * 2 + LANE_LABEL_HEADER_SIZE;
        ...
        laneIds.forEach((laneId) => {
          ...
          if (horizontal) {
            const contentLeft = Math.min(...members.map((element) => element.x));
            ...
            const nextX = Math.min(lane.x, contentLeft - lanePad - LANE_LABEL_HEADER_SIZE);
            const nextRight = Math.max(lane.x + lane.width, contentRight + lanePad + LANE_LABEL_HEADER_SIZE);
            laneLayoutById[laneId] = {
              preferredStart: nextY,
              preferredEnd: nextBottom,
              minEnd: contentBottom + lanePad,
              preferredX: nextX,
              preferredRight: nextRight,
            };
          ...
          const nextY = Math.min(lane.y, contentTop - lanePad - LANE_LABEL_HEADER_SIZE);
          const nextBottom = Math.max(contentBottom + lanePad + LANE_LABEL_HEADER_SIZE, nextY + minLaneSpan);
          laneLayoutById[laneId] = {
        ...
        Object.entries(laneMembershipContext.participantLaneIdsById).forEach(([participantId, laneIds]) => {
          ...
          const lanes = laneIds
            .map((laneId) => laneMembershipContext.laneById[laneId])
            .filter(Boolean);
          ...
          if (horizontal) {
            ...
            sortedLanes.forEach((lane) => {
              ...
              const nextY = stackedBottom == null
                ? layout.preferredStart
                : Math.max(layout.preferredStart, stackedBottom);
            ...
            sortedLanes.forEach((lane) => {
              if (Math.abs(lane.x - minX) >= 1 || Math.abs(lane.width - (maxX - minX)) >= 1) {
                modeling.resizeShape(lane, {
                  x: minX,
                  y: lane.y,
                  width: maxX - minX,
          ...
          sortedLanes.forEach((lane) => {
            ...
            const nextX = stackedRight == null
              ? layout.preferredStart
              : Math.max(layout.preferredStart, stackedRight);
        ...
        allElements
          .filter((element) => element && element.type === 'bpmn:Participant')
          .forEach((participant) => {
            const groupId = participant.parent && participant.parent.id;
            if (!groupId) {
              return;
            }
            (participantGroups[groupId] ||= []).push(participant);
          });

        function moveParticipantWithContents(participant, delta) {
          allElements
            .filter((element) =>
              element &&
              element !== participant &&
              ...
              findAncestorByType(element, 'bpmn:Participant')?.id === participant.id
            )
            .forEach((element) => {
          ...
          modeling.resizeShape(participant, {
            x: participant.x + delta.x,
            y: participant.y + delta.y,
            width: participant.width,
            height: participant.height,
          });
        }

        const participantGap = Math.max(30, lanePad);
        Object.values(participantGroups).forEach((participants) => {
          if (participants.length < 2) {
            return;
          }

          if (horizontal) {
            const minX = Math.min(...participants.map((participant) => participant.x));
            const maxX = Math.max(...participants.map((participant) => participant.x + participant.width));
            participants.forEach((participant) => {
              const participantLanes = (laneMembershipContext.participantLaneIdsById[participant.id] || [])
                .map((laneId) => laneMembershipContext.laneById[laneId])
                .filter(Boolean);
              participantLanes.forEach((lane) => modeling.resizeShape(lane, {
                x: minX + PARTICIPANT_LABEL_HEADER_SIZE,
                y: lane.y,
                width: maxX - minX - PARTICIPANT_LABEL_HEADER_SIZE,
                height: lane.height,
            ...
            participants.slice().sort((a, b) => a.y - b.y).forEach((participant) => {
              const nextY = previousBottom == null
                ? participant.y
                : Math.max(participant.y, previousBottom + participantGap);
              if (Math.abs(nextY - participant.y) >= 1) {
                moveParticipantWithContents(participant, { x: 0, y: nextY - participant.y });
              }
              previousBottom = nextY + participant.height;
            });
            return;
          }

          const minY = Math.min(...participants.map((participant) => participant.y));
          const maxY = Math.max(...participants.map((participant) => participant.y + participant.height));
          participants.forEach((participant) => {
            const participantLanes = (laneMembershipContext.participantLaneIdsById[participant.id] || [])
              .map((laneId) => laneMembershipContext.laneById[laneId])
              .filter(Boolean);
          ...
          participants.slice().sort((a, b) => a.x - b.x).forEach((participant) => {
            const nextX = previousRight == null
              ? participant.x
              : Math.max(participant.x, previousRight + participantGap);
            if (Math.abs(nextX - participant.x) >= 1) {
              moveParticipantWithContents(participant, { x: nextX - participant.x, y: 0 });
            }
            previousRight = nextX + participant.width;
          });
        });
        laneIds.forEach((laneId) => {
          const memberIds = laneMembershipContext.laneMemberIdsByLaneId[laneId] || [];
          memberIds
            ...
            .forEach((element) => {
              const desiredPosition = clampNodePositionToLaneBand(element, {
                x: element.x,
                y: element.y,
              }, lanePad);
              const dx = desiredPosition.x - element.x;
              const dy = desiredPosition.y - element.y;
              ...
              try {
                ...
                console.warn('[BPMN-Layout] lane label band clamp failed:', element.id, err);
              }
            });
        });
      }
      ...
      function rebalanceBottomMergeSinkTasks(allElements) {
        ...
        const candidates = allElements.filter(el => {
          if (phasePlan.constrained.has(el?.id)) return false;
        ...
        candidates.forEach(target => {
          ...
          const desiredPosition = clampNodePositionToLaneBand(target, {
          ...
          if (!element.waypoints) {
            ...
            if (elementToMove && elementToMove.id && typeof elementToMove.x === 'number') {
              ...
              if (COORD_DEBUG_MOVE) {
                ...
                console.log('[BPMN-Layout] MOVE', elementToMove.id, label,
                  '| (', elementToMove.x?.toFixed(0), ',', elementToMove.y?.toFixed(0), ') → (', newX?.toFixed(0), ',', newY?.toFixed(0), ')',
      ...
      function resizeContainerPreservingChildren(container, bounds) {
        const childPositions = (container.children || [])
          ...
          .map(child => ({ child, x: child.x, y: child.y }));

        modeling.resizeShape(container, bounds);

        childPositions.forEach(({ child, x, y }) => {
          const dx = x - child.x;
          const dy = y - child.y;
          if (Math.abs(dx) < 1 && Math.abs(dy) < 1) return;
          modeling.moveShape(child, { x: dx, y: dy });
        });
      }
      ...
      graph.groups.forEach(group => {
        ...
        if (element.type === 'bpmn:Participant') {
          if (horizontal) {
            resizeContainerPreservingChildren(element, {
      ...
      Object.entries(participantChildrenMap).forEach(([participantId, laneIds]) => {
        ...
        if (participantElement) {
          ...
          // 가로 배치 모드인 경우 x, y 값 교환 고려
          if (horizontal) {
            resizeContainerPreservingChildren(participantElement, {
              x: minY - margin, // x에 minY 사용
              y: minX - margin, // y에 minX 사용
              width: maxY - minY + margin * 2, // width에 y 차이 사용
              height: maxX - minX + margin * 2 // height에 x 차이 사용
      ...
      syncPhaseContainersWithParticipants(bpmnModeler, { horizontal, phaseGrids, phaseLengths: phasePlan.lengths });
      ...
      adjustLabelsAfterLayout(bpmnModeler, true);
      if (!includeWaypoints) {
      ...
      function routeBoundarySourceFlow(flow, currentWaypoints) {
        ...
        const targetSide = inferDockSideFromPoint(flow.target, currentWaypoints[currentWaypoints.length - 1]) || pickPreferredSide(flow.target, flow.source);
        ...
        return compactWaypointList(orthogonalizeNormalizedWaypoints([
          sourceDock,
          sourceLead,
          bridge,
          targetLead,
      ...
      function normalizeSequenceFlowEndpointFacing(flow) {
        ...
        if (isGatewayShape(flow.target) && currentTargetSide && nextWaypoints.length >= 2) {
          ...
          if (targetDock && !pointClose(nextWaypoints[lastIdx], targetDock)) {
            ...
            nextWaypoints[lastIdx - 1] = alignPointFromSide(nextWaypoints[lastIdx], nextWaypoints[lastIdx - 1], currentTargetSide);
            changed = true;
          }
        }
        ...
        const orthogonalWaypoints = orthogonalizeNormalizedWaypoints(nextWaypoints);
        const needsOrthogonalization = orthogonalWaypoints.length !== nextWaypoints.length ||
          orthogonalWaypoints.some((point, index) => !pointClose(point, nextWaypoints[index]));
        return changed || needsOrthogonalization ? orthogonalWaypoints : null;
      }
      ...
      function routeInternalSubProcessFeedback(flow) {
        ...
        const internalNodes = elementRegistry.getAll().filter((element) =>
          element &&
          element.businessObject &&
          element.businessObject.$type !== 'bpmn:SequenceFlow' &&
          !element.labelTarget &&
          spGetNearestSubProcessId(element) === sourceSpId
        );
        return spRouteInternalFeedback(flow, subProcess, internalNodes);
      }

      setSequenceFlowRoutingFinalizer(elementRegistry, flow =>
        routeInternalSubProcessFeedback(flow) || (!feedbackFlowIds.has(flow.id) ? normalizeSequenceFlowEndpointFacing(flow) : null));
      ...
      sequenceFlowsForLayout.forEach(flow => {
        ...
        try {
          ...
          const finalWaypoints = computeFinalSequenceFlowWaypoints(
            flow,
          ...
          if (Array.isArray(finalWaypoints) && finalWaypoints.length >= 2) {
            modeling.updateWaypoints(flow, finalWaypoints);
          }
          ...
          if (LAYOUT_DEBUG && !COORD_DEBUG) {
            console.log('[BPMN-Layout] layoutConnection OK:', flow.id,
              '| source:', src?.id, '→ target:', tgt?.id,
              '| waypoints:', wpBeforeLen, '→', wpAfterLen);
          }
          ...
          if (COORD_DEBUG && metricsBefore && wpAfterLen >= 2) {
            ...
            const deltaSrcFirst = metricsBefore.distSrcToFirstWp != null && dSrcFirstAfter != null
              ? (dSrcFirstAfter - metricsBefore.distSrcToFirstWp).toFixed(1) : '-';
            const deltaLastTgt = metricsBefore.distLastWpToTgt != null && dLastTgtAfter != null
            ...
            const deltaLastWp = Math.sqrt(
              Math.pow(lastWpAfter.x - metricsBefore.lastWp.x, 2) +
              Math.pow(lastWpAfter.y - metricsBefore.lastWp.y, 2)
            ).toFixed(1);

            console.log('[BPMN-Layout] COORD', flow.id,
              '| source:', src.id, `(${src.x?.toFixed(0)},${src.y?.toFixed(0)})`,
              '| target:', tgt.id, `(${tgt.x?.toFixed(0)},${tgt.y?.toFixed(0)})`);
            console.log('[BPMN-Layout]   dist(source→첫WP):', metricsBefore.distSrcToFirstWp?.toFixed(1), '→', dSrcFirstAfter?.toFixed(1), '(Δ' + deltaSrcFirst + ')',
              '| dist(끝WP→target):', metricsBefore.distLastWpToTgt?.toFixed(1), '→', dLastTgtAfter?.toFixed(1), '(Δ' + deltaLastTgt + ')');
            console.log('[BPMN-Layout]   첫WP 이동:', deltaFirstWp, 'px | 끝WP 이동:', deltaLastWp, 'px',
              '| src↔tgt:', metricsBefore.distSrcToTgt?.toFixed(1), '→', dSrcTgtAfter?.toFixed(1));
          }
        } catch (e) {
      ...
      const messageFlowsForLayout = allElements.filter(el =>
        el.businessObject &&
        el.businessObject.$type === 'bpmn:MessageFlow' &&
        el.source &&
        el.target
      );
      messageFlowsForLayout.forEach(flow => {
        try {
          modeling.layoutConnection(flow);
          const finalWaypoints = computeFinalSequenceFlowWaypoints(
            flow,
            (flow.waypoints || []).map(point => ({ x: point.x, y: point.y })),
            elementRegistry,
            {
          ...
          console.warn('[BPMN-Layout] MessageFlow layoutConnection 실패:', flow.id, error);
        }
      });
      ...
      const feedbackFlowsForLayout = allElements.filter(el => {
        ...
        const isSourceBoundary =
          el.source.businessObject &&
          el.source.businessObject.$type === 'bpmn:BoundaryEvent';
        const isTargetBoundary =
          el.target.businessObject &&
          el.target.businessObject.$type === 'bpmn:BoundaryEvent';
        return !isSourceBoundary && !isTargetBoundary;
      });
      ...
      boundaryFlows.forEach(flow => {
        try {
          ...
          const newWaypoints = routeBoundarySourceFlow(flow, computeFinalSequenceFlowWaypoints(
            flow,
            [start, ...restoredInner, end],
            elementRegistry,
            {
              horizontalLayout: horizontal,
   ...
   * - 마지막으로 라벨 위치를 정리한다.
   */
  BpmnAutoLayout.__applyLegacyAutoLayout = function(bpmnModeler, options = {}, onLoadStart = () => {}, onLoadEnd = () => {}) {
    const { horizontal = false, layoutOptions = {}, includeWaypoints = true } = options;
    const timingStartMs = nowMs();
    console.log('[BPMN-Layout] applyAutoLayout 시작 | horizontal:', horizontal);
    onLoadStart();
    ...
    const deferredRouting = deferSequenceFlowRouting(bpmnModeler.get('elementRegistry'), { fresh: true });

    try {
      const elementRegistry = bpmnModeler.get('elementRegistry');
      const all = elementRegistry.getAll();
      __spatialIndex.build(all);
      const phaseGrids = capturePhaseGrid(all, horizontal);
      ...
      if (!subProcesses.length) {
        BpmnAutoLayout.__applyAutoLayoutInternal(
          bpmnModeler,
          { horizontal, layoutOptions, includeWaypoints, phaseGrids },
          () => {},
          () => {}
        );
        deferredRouting.finish(bpmnModeler.get('modeling'));
      ...
      try {
        BpmnAutoLayout.__applyAutoLayoutInternal(
          bpmnModeler,
          { horizontal, layoutOptions, includeWaypoints: false, phaseGrids },
          () => {},
          () => {}
        );
        ...
        adjustLabelsAfterLayout(bpmnModeler);
        withDepth.forEach(({ element: sp }) => {
          resizeSubProcessToContents(bpmnModeler, sp);
        });
  ...
  BpmnAutoLayout.applyAutoLayout = function(bpmnModeler, options = {}, onLoadStart = () => {}, onLoadEnd = () => {}) {
    ...
    onLoadStart();
    try {
      const result = computeModelerLayout(bpmnModeler, { ...options.layoutOptions, horizontal });
      if (result.plan.status !== 'computed') {
        bpmnModeler.get('eventBus').fire('autoLayout.failed', { result });
        throw new Error(`Structured layout ${result.plan.status}: ${(result.plan.validationErrors || []).join(', ')}`);
      }
      bpmnModeler.get('atomicModeling').execute(() => {
        applyComputedLayout(bpmnModeler, result.plan, { input: result.input, nodeOffsets: options.layoutOptions?.nodeOffsets });
        window.isHorizontalLayout = horizontal;
        bpmnModeler.get('eventBus').fire('autoLayout.complete', { result });
      });
      return result;
    } finally {
      pushLayoutTiming('applyAutoLayout', timingStartMs, { horizontal, includeWaypoints: true, engine: 'structured' });
      onLoadEnd();
    }
  };
