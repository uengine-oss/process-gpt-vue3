        ...
        for (let i = 0; i < this.layers.length; i++) {
            ...
            for (const groupId in nodesPerGroup) {
                ...
                groupNodes.forEach((node) => {
                    ...
                    console.log(
                        `[DEBUG_NODE_SIZE] 노드 크기 통일: ID=${node.id}, 원래 너비=${originalWidth || 'undefined'}, 새 너비=${node.width}`
                    );
                });
                ...
                if (totalNodesWidth + totalSpacing > availableWidth) {
                    ...
                    console.log(
                        `[DEBUG_SPACING] 노드 간격 조정: 그룹=${groupId}, 원래 간격=${horizontalSpacing}, 새 간격=${actualSpacing.toFixed(
                            1
                        )}`
                    );
                }
