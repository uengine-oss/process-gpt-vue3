...
export function adjustLabelsAfterLayout(bpmnModeler, nodesOnly = false) {
  ...
  const placementObstacles = elements.filter(el => !el.labelTarget);
  elements.forEach((el) => {
    ...
    el.labels.forEach((label) => {
      ...
      if (boType === 'bpmn:Lane') {
        ...
        const pos = findGatewayLabelPosition(el, label, placementObstacles);
        centerX = pos.x;
        centerY = pos.y;
      } else {
