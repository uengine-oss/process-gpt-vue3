...
(function(global) {
  ...
  function normalizeWaypoints(points, epsilon, minSeg) {
    ...
    for (let i = 1; i < points.length; i++) {
      ...
      if (Math.abs(dx) > ORTHO_EPS && Math.abs(dy) > ORTHO_EPS) {
        ...
        const mid = ax
          ? { x: snapValue(curr.x, eps), y: prev.y }
          : { x: prev.x, y: snapValue(curr.y, eps) };
  ...
  function createRouteQueue() {
    ...
    let nextOrder = 0;
    let freshFrom = 0;
    const compare = (a, b) => a.turns - b.turns || a.distance - b.distance || a.order - b.order;
    function push(item) {
    ...
    function pop() {
      if (heap.length > 1000) {
        // Reconstruct the legacy array: previously sorted entries, then new entries.
        // Its stable overflow sort uses a different ranking from normal extraction.
        heap.sort((a, b) => {
          const aFresh = a.order >= freshFrom;
          const bFresh = b.order >= freshFrom;
          if (aFresh !== bFresh) return aFresh ? 1 : -1;
          return aFresh ? a.order - b.order : compare(a, b);
        });
        heap.sort((a, b) => a.turns * 10 + a.distance - (b.turns * 10 + b.distance));
        const retained = heap.slice(0, 100);
        heap = [];
        nextOrder = 0;
        retained.forEach(push);
      }
      freshFrom = nextOrder;
  ...
  function findOrthogonalPath(start, end, obstacles, options) {
    ...
    const isAllowed = (p) => allowedPoints.some(a =>
      Math.abs(a.x - p.x) < EPS && Math.abs(a.y - p.y) < EPS);
    ...
    queue.push({ point: start, path: [start], turns: 0, distance: manhattan(start, end) });
    const serialize = (p) => `${p.x},${p.y}`;

    for (let steps = 0; steps < maxSteps; steps++) {
      const candidate = queue.pop();
      if (!candidate) break;
      const { point, path, turns, distance } = candidate;
      ...
      for (const d of dirs) {
        const next = { x: Math.round((point.x + d.dx) / gridSize) * gridSize, y: Math.round((point.y + d.dy) / gridSize) * gridSize };
        const key = serialize(next);
        if (visited.has(key)) continue;
        visited.add(key);
        if (!intersectsObstacle(next, obstacles)) {
          let newTurns = turns;
          if (path.length > 1 && currDir && (currDir.dx !== 0 !== (d.dx !== 0) || currDir.dy !== 0 !== (d.dy !== 0))) newTurns++;
          queue.push({ point: next, path: [...path, next], turns: newTurns, distance: manhattan(next, end) });
        }
      }
    }
  ...
  EdgeRouterOrthogonal.computeWaypoints = function(source, target, obstacles, options) {
    ...
    if (!validateOrthogonal(result)) {
      const sp = sourcePorts.right;
      const tp = targetPorts.left;
      result = normalizeWaypoints([sp, tp].map(p => ({ x: p.x, y: p.y })), opts.epsilon, opts.minSegmentLength);
    }
    return result;
  };
