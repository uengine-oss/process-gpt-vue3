...
import { SequenceFlowSegmentIndex } from './sequence-flow/sequence-flow-segment-index.js';
import {
...
export function scoreFacingCandidatePhase1(sourceExitSide, targetEntrySide, source, target, primaryPair) {
  if (
    primaryPair &&
    sourceExitSide === primaryPair.sourceSide &&
    targetEntrySide === primaryPair.targetSide
  ) {
    return 0;
  }
...
function buildP2TemplateCandidatesForFacing(source, target, connection, facing, clearance = EDGE_CLEARANCE_PX, spreadSameDirection = false) {
  if (!source || !target || !facing) {
    return [];
  }
  var startDock = connection
    ? dockingPointOnSharedSide(source, facing.sourceExitSide, connection, spreadSameDirection)
    : dockingPointOnSideMid(source, facing.sourceExitSide);
  var endDock = connection
    ? dockingPointOnSharedSide(target, facing.targetEntrySide, connection, spreadSameDirection)
    : dockingPointOnSideMid(target, facing.targetEntrySide);
  var exitPoint = projectPointFromDockSide(startDock, facing.sourceExitSide, clearance);
  var approachPoint = projectPointFromDockSide(endDock, facing.targetEntrySide, clearance);
...
function createSequenceFlowObstacleContext(connection, elementRegistry) {
  ...
  return {
    ...
    flowNodes: all.filter(function (el) {
      return !el.labelTarget;
    }),
  };
}

function selectBestP2TemplateCandidate(source, target, connection, p1Candidates, obstacleContext, routeConflict) {
  ...
  for (var i = 0; i < p1Candidates.length; i++) {
    ...
    var clearance = routeConflict
      ? NODE_NO_RUN_CLEARANCE_PX + Math.max(elementClearancePadPx(source), elementClearancePadPx(target)) + 2
      : EDGE_CLEARANCE_PX;
    var templates = buildP2TemplateCandidatesForFacing(source, target, connection, facing, clearance, !!routeConflict);
    for (var j = 0; j < templates.length; j++) {
      var template = templates[j];
      if (routeConflict && (routeConflict(template.points) || !validFinalRouteCandidate(connection, template.points, obstacles))) continue;
      candidateCount++;
...
function dockingPointOnSharedSide(shape, side, connection, spreadSameDirection = false) {
  ...
  var sameSide = attached.filter(function (flow) {
    if (freshLayoutConnections.has(connection)) {
      const participant = findAncestorParticipant(shape);
      const facing = computeSequenceFacingSides(flow.source, flow.target, participant?.di?.isHorizontal !== false);
      return (flow.source === shape ? facing.sourceExitSide : facing.targetEntrySide) === side;
    }
  ...
  var hasIncoming = sameSide.some(function (flow) { return flow.target === shape; });
  var hasOutgoing = sameSide.some(function (flow) { return flow.source === shape; });
  if ((!hasIncoming || !hasOutgoing) && !spreadSameDirection) {
    return base;
  }
...
export function isSequenceFlowLayoutObstacle(el, connection, source, target) {
  ...
  if (isDiagramAncestor(source, el) || isDiagramAncestor(target, el)) {
    return false;
  }
  if (el.labelTarget) {
    if (freshLayoutConnections.has(connection) && el.labelTarget.businessObject?.$type === 'bpmn:SequenceFlow') return false;
...
export function obstacleRectForSegmentHitTest(el, clearancePx) {
  ...
  var index = getActiveObstacleSpatialIndex();
  var cached = index && index.rectCache.get(el);
  if (cached && cached.x === el.x && cached.y === el.y && cached.width === el.width && cached.height === el.height && cached.type === el.businessObject?.$type) {
    var hit = cached.rects.get(c);
    if (hit) return hit;
  } else if (index) {
    cached = { x: el.x, y: el.y, width: el.width, height: el.height, type: el.businessObject?.$type, rects: new Map() };
    index.rectCache.set(el, cached);
  }
  var w = el.width;
  var h = el.height;
  ...
  var rect = { x: el.x - c, y: el.y - c, width: w + 2 * c, height: h + 2 * c };
  if (cached) cached.rects.set(c, rect);
  return rect;
}
...
export function segmentIntersectsRect(ax, ay, bx, by, rect) {
  ...
  function pin(x, y) {
    return (
      x >= rect.x - eps &&
      y >= rect.y - eps &&
      x <= rect.x + rect.width + eps &&
      y <= rect.y + rect.height + eps
    );
  }
...
export function waypointsIntersectFlowObstacles(waypoints, obstacleElements, clearancePx) {
  ...
  if (spatialIndex && obstacleElements.length > 8) {
    var obstacleSet = spatialIndex.obstacleSetCache.get(obstacleElements);
    if (!obstacleSet) {
      obstacleSet = new Set(obstacleElements);
      spatialIndex.obstacleSetCache.set(obstacleElements, obstacleSet);
    }
...
function getEdgeRouterOrthogonal() {
  ...
  return g.EdgeRouterOrthogonal && typeof g.EdgeRouterOrthogonal.computeWaypoints === 'function'
    ? g.EdgeRouterOrthogonal
    : null;
}
...
export function ensureOrthogonalEndSegment(merged, tgtSide, orthoEps) {
  ...
  if (
    typeof pPrev.x !== 'number' ||
    typeof pPrev.y !== 'number' ||
...
export function snapSequenceFlowWaypointsToFacingEndpoints(pts, source, target, layoutHorizontal, lockedFacingSides) {
  ...
  if (!lockedFacingSides && pts.length >= 2) {
    ...
    if (
      sourceSideFromPath &&
      (
        dockSideFacesOther(source, target, sourceSideFromPath) ||
        !dockSideFacesOther(source, target, sides.sourceSide)
      )
    ) {
  ...
  if (!lockedFacingSides && pts.length >= 3) {
    ...
    if (
      targetSideFromPath &&
      (
        dockSideFacesOther(target, source, targetSideFromPath) ||
        !dockSideFacesOther(target, source, sides.targetSide)
      )
    ) {
...
export function applySequenceFlowObstacleReroute(
  connection,
  waypoints,
  elementRegistry,
  horizontalLayout,
    ...
    if (Array.isArray(routed) && routed.length >= 2) {
      ...
      if (
        waypointsIntersectFlowObstacles(
          merged,
          obstacles,
          SEQUENCE_FLOW_OBSTACLE_SEGMENT_CLEARANCE_PX,
        ) ||
        sequenceFlowWaypointsObstacleHitsExhaustive(
          merged,
          obstacles,
          SEQUENCE_FLOW_OBSTACLE_SEGMENT_CLEARANCE_PX,
        )
...
function collapseSubpixelOrthogonalRails(waypoints) {
  var pts = cloneWaypoints(waypoints);
  var tolerance = 1;

  for (var i = 0; i + 3 < pts.length; i++) {
    var a = pts[i];
    var b = pts[i + 1];
    var c = pts[i + 2];
    var d = pts[i + 3];
    var narrowVerticalReturn =
      Math.abs(a.x - b.x) < ORTHO_EPS &&
      Math.abs(b.y - c.y) < ORTHO_EPS &&
      Math.abs(c.x - d.x) < ORTHO_EPS &&
      Math.abs(b.x - c.x) <= tolerance &&
      Math.abs(a.y - d.y) < ORTHO_EPS;
    var narrowHorizontalReturn =
      Math.abs(a.y - b.y) < ORTHO_EPS &&
      Math.abs(b.x - c.x) < ORTHO_EPS &&
      Math.abs(c.y - d.y) < ORTHO_EPS &&
      Math.abs(b.y - c.y) <= tolerance &&
      Math.abs(a.x - d.x) < ORTHO_EPS;

    if (narrowVerticalReturn || narrowHorizontalReturn) {
      pts.splice(i + 1, 2);
      i = Math.max(-1, i - 2);
    }
  }
...
function logObstacleHit(connection, segIndex, ax, ay, bx, by, obstacleEl) {
  ...
  var flowName =
    flowBo && flowBo.name != null && String(flowBo.name).trim() !== ''
      ? String(flowBo.name).trim()
      : '';
...
function buildHorizontalDetour(ax, ay, bx, by, rect, extraPad, source, target, stripHalfForHitTest) {
  ...
  var hitH =
    stripH > 0
      ? segmentStripIntersectsRect(ax, ay, bx, by, stripH, rect)
...
function buildVerticalDetour(ax, ay, bx, by, rect, extraPad, source, target, stripHalfForHitTest) {
  ...
  var hitV =
    stripV > 0
      ? segmentStripIntersectsRect(ax, ay, bx, by, stripV, rect)
      : segmentIntersectsRect(ax, ay, bx, by, rect);
  if (!hitV) {
    return null;
  }
...
function collapseMicroOrthogonalJogs(pts, connection, elementRegistry) {
  ...
  while (changed && out.length >= 4) {
    ...
    for (i = 1; i < out.length - 2; i++) {
      ...
      var h01 =
        Math.abs(p0.y - p1.y) < ORTHO_EPS &&
        Math.abs(p2.y - p3.y) < ORTHO_EPS &&
        Math.abs(p1.x - p2.x) < ORTHO_EPS;
      if (h01) {
...
function scorePathAgainstNodeNoRunBands(points, source, target, obstacles) {
  ...
  for (var i = 0; i < points.length - 1; i++) {
    ...
    for (var j = 0; j < scoringObstacles.length; j++) {
      ...
      if (
        segmentRunsParallelNearRect(
          a.x,
          a.y,
          b.x,
          b.y,
          rect,
          NODE_NO_RUN_CLEARANCE_PX,
          NODE_NO_RUN_MIN_SEGMENT_PX,
        )
      ) {
...
function candidateWorsensNodeNoRunBands(candidate, current, source, target, obstacles) {
  return noRunBandScore(candidate, source, target, obstacles) >
    noRunBandScore(current, source, target, obstacles);
}
...
function pathHasInlineBackwardReturnRun(points, source, target, obstacles) {
  ...
  var top = Math.min.apply(null, relevant.map(function (el) { return el.y; }));
  var bottom = Math.max.apply(null, relevant.map(function (el) { return el.y + (el.height || 0); }));
...
function simplifyBackwardReturnOuterDogleg(points, source, target, connection, obstacleElements) {
  ...
  var targetEntry = connection
    ? dockingPointOnSharedSide(target, 'r', connection)
    : dockingPointOnSideMid(target, 'r');
  var targetEntryLead = makePt(targetEntry.x + EDGE_CLEARANCE_PX, targetEntry.y);
  var candidateSpecs = [
    [
      snapshotWaypoint(out[0]),
      snapshotWaypoint(out[1]),
      makePt(rail.x, last.y),
      snapshotWaypoint(out[out.length - 1]),
    ],
  ];
  var outerY =
    targetMid.y < sourceMid.y
      ? target.y - EDGE_CLEARANCE_PX
      : target.y + (target.height || 0) + EDGE_CLEARANCE_PX;
  candidateSpecs.push([
    snapshotWaypoint(out[0]),
    snapshotWaypoint(out[1]),
...
function simplifyBackwardGatewayReturnExit(points, source, target, connection, obstacleElements) {
  ...
  var sourceDock = connection
    ? dockingPointOnSharedSide(source, sourceSide, connection)
    : dockingPointOnSideMid(source, sourceSide);
  var candidate = withoutRedundantPoints(ensureStrictlyOrthogonalWaypoints([
    snapshotWaypoint(sourceDock),
    makePt(sourceDock.x, railY),
    railEnd,
  ].concat(out.slice(bestIndex + 2).map(snapshotWaypoint))));
...
export function enforceFinalTargetApproachClearance(points, target) {
  if (!Array.isArray(points) || points.length < 2 || !target) {
    return points;
  }
  var out = cloneWaypoints(points);
  var lastIndex = out.length - 1;
  var dockSide = inferDockSideFromPointOnShape(target, out[lastIndex]);
  var pathSide = inferTargetDockSideFromLastApproachSegment(out[lastIndex - 1], out[lastIndex]);
  var side = pathSide || dockSide;
  if (!side) {
    return out;
  }
  if (pathSide && dockSide !== pathSide) {
    out[lastIndex] = dockingPointOnSideMid(target, pathSide);
  }
  var originalPrevious = snapshotWaypoint(out[lastIndex - 1]);
  ensureMinApproachLegLength(out[lastIndex - 1], out[lastIndex], side, EDGE_CLEARANCE_PX);
  if (out.length >= 3) {
    var previous = out[lastIndex - 1];
    var beforePrevious = out[lastIndex - 2];
    var movedX = Math.abs(previous.x - originalPrevious.x) > ORTHO_EPS;
    var movedY = Math.abs(previous.y - originalPrevious.y) > ORTHO_EPS;
    if (movedX && Math.abs(beforePrevious.x - originalPrevious.x) <= ORTHO_EPS) {
      out.splice(lastIndex - 1, 0, makePt(previous.x, beforePrevious.y));
    } else if (movedY && Math.abs(beforePrevious.y - originalPrevious.y) <= ORTHO_EPS) {
      out.splice(lastIndex - 1, 0, makePt(beforePrevious.x, previous.y));
    }
  }
  return withoutRedundantPoints(ensureStrictlyOrthogonalWaypoints(out));
}

export function enforceFinalSourceExitClearance(points, source) {
  if (!Array.isArray(points) || points.length < 2 || !source) {
    return points;
  }
  var out = cloneWaypoints(points);
  var dockSide = inferDockSideFromPointOnShape(source, out[0]);
  var pathSide = inferSourceDockSideFromFirstSegment(out[0], out[1]);
  var side = pathSide || dockSide;
  if (!side) {
    return out;
  }
  if (pathSide && dockSide !== pathSide) {
    out[0] = dockingPointOnSideMid(source, pathSide);
  }
  out[1] = alignSecondPointToExitNormal(out[0], out[1], side);
  ensureMinExitLegLength(out[0], out[1], side, EDGE_CLEARANCE_PX);
  if (out.length >= 3) {
    var second = out[1];
    var third = out[2];
    if (Math.abs(second.x - third.x) > ORTHO_EPS && Math.abs(second.y - third.y) > ORTHO_EPS) {
      out.splice(2, 0, side === 't' || side === 'b'
        ? makePt(third.x, second.y)
        : makePt(second.x, third.y));
    }
  }
  return withoutRedundantPoints(ensureStrictlyOrthogonalWaypoints(out));
}
function repairSameLaneBackwardReturnNoRun(points, source, target, connection, obstacleElements) {
  ...
  var minTop = Math.min.apply(null, relevant.map(function (el) { return el.y; }));
  var maxBottom = Math.max.apply(null, relevant.map(function (el) { return el.y + (el.height || 0); }));
  var railCandidates = [
...
function collapseLeadingLocalJog(points, connection, source, target, elementRegistry, obstacleElements) {
  ...
  if (h01 && v12 && h23) {
    ...
    if (firstLeg <= LEADING_LOCAL_JOG_MAX_PX && midLeg > ORTHO_EPS && runLeg > ORTHO_EPS) {
      var collapsedHVH = ensureStrictlyOrthogonalWaypoints(
        withoutRedundantPoints([out[0], makePt(p0.x, p2.y)].concat(out.slice(3)))
      );
      if (
  ...
  if (v01 && h12 && v23) {
    ...
    if (firstLegY <= LEADING_LOCAL_JOG_MAX_PX && midLegX > ORTHO_EPS && runLegY > ORTHO_EPS) {
      var collapsedVHV = ensureStrictlyOrthogonalWaypoints(
        withoutRedundantPoints([out[0], makePt(p2.x, p0.y)].concat(out.slice(3)))
      );
      if (
...
function simplifyLocalClusterWaypoints(points, connection, source, target, elementRegistry, obstacleElements) {
  ...
  var candidates = [
    buildLocalClusterOneBendCandidate(start, end, true),
    buildLocalClusterOneBendCandidate(start, end, false),
  ];
...
function rerouteTailAwayFromTargetNoRunBand(points, target) {
  ...
  if (Math.abs(penultimate.x - last.x) < ORTHO_EPS) {
    var outsideY =
      last.y >= rect.y + rect.height / 2
        ? rect.y + rect.height + outsidePad
        : rect.y - outsidePad;
  ...
  if (Math.abs(penultimate.y - last.y) < ORTHO_EPS) {
    var outsideX =
      last.x >= rect.x + rect.width / 2
        ? rect.x + rect.width + outsidePad
        : rect.x - outsidePad;
...
function breakAxisSegmentsAlongTargetNoRunBand(pts, target, connection, elementRegistry) {
  if (
    isSequenceFlowImportLayoutActive() ||
    !Array.isArray(pts) ||
    pts.length < 3 ||
    !target ||
    !connection ||
...
export function resolveSequenceFlowWaypointObstacles(pts, connection, elementRegistry, obstacleElements) {
  ...
  resolveObstacleLoop: for (iter = 0; iter < MAX_ITERATIONS; iter++) {
    ...
    for (seg = 0; seg < pts.length - 1; seg++) {
      ...
      var segmentFilterPadPx =
        firstStripHalf > 0
          ? SEGMENT_OBSTACLE_FILTER_PAD_PX + firstStripHalf
          : SEGMENT_OBSTACLE_FILTER_PAD_PX;

      var obstaclesSeg = filterObstaclesForSegment(
        obstacles,
        ax,
        ay,
        bx,
...
function collapseTinyTargetJog(pts, target, side) {
  ...
  if (side === 'l' || side === 'r') {
    if (
      ...
      penultimate.y - last.y <= TARGET_TINY_JOG_MAX_PX
    ) {
  ...
  if (side === 't' || side === 'b') {
    if (
      ...
      penultimate.x - last.x <= TARGET_TINY_JOG_MAX_PX
    ) {
...
function snapEndpointsToAdjacentSegmentSides(pts, source, target, connection) {
  ...
  // In a two-point route the adjacent point is the other endpoint, not a bend.
  if (out.length === 2 && sourceSide && targetSide) {
    var start = connection ? dockingPointOnSharedSide(source, sourceSide, connection) : dockingPointOnSideMid(source, sourceSide);
    var end = connection ? dockingPointOnSharedSide(target, targetSide, connection) : dockingPointOnSideMid(target, targetSide);
    var sourceHorizontal = sourceSide === 'l' || sourceSide === 'r';
    var targetHorizontal = targetSide === 'l' || targetSide === 'r';
    var kind = sourceHorizontal === targetHorizontal ? (sourceHorizontal ? 'hvh' : 'vhv') : (sourceHorizontal ? 'hv' : 'vh');
    return withoutRedundantPoints(buildOrthogonalPathTemplate(start, end, kind));
  }
  ...
  if (targetSide) {
    last = out.length - 1;
    out[last] = connection
      ? dockingPointOnSharedSide(target, targetSide, connection)
      : dockingPointOnSideMid(target, targetSide);
...
export function isShortAdjacentSequenceFlow(source, target) {
  ...
  return (
    (gapX <= SHORT_LINK_GAP_PX || gapY <= SHORT_LINK_GAP_PX) && centerDistance <= SHORT_LINK_CENTER_DISTANCE_PX
  );
}
...
function countSequenceFlowObstacleHits(pts, obstacleElements, clearancePx) {
  var hits = 0;
  for (var i = 0; i < pts.length - 1; i++) {
    var a = getWpXY(pts[i]);
    var b = getWpXY(pts[i + 1]);
    for (var j = 0; j < obstacleElements.length; j++) {
      var rect = obstacleRectForSegmentHitTest(obstacleElements[j], clearancePx);
      if (segmentIntersectsRect(a.x, a.y, b.x, b.y, rect)) hits++;
    }
  }
  return hits;
}

function endpointLegLeavesShapeNormally(shape, endpoint, adjacent) {
  var side = inferDockSideFromPointOnShape(shape, endpoint);
  if (!side || !endpoint || !adjacent) return false;
  if (side === 'l' || side === 'r') {
    if (Math.abs(adjacent.y - endpoint.y) > ORTHO_EPS) return false;
    return side === 'l' ? adjacent.x < endpoint.x - ORTHO_EPS : adjacent.x > endpoint.x + ORTHO_EPS;
  }
  if (Math.abs(adjacent.x - endpoint.x) > ORTHO_EPS) return false;
  return side === 't' ? adjacent.y < endpoint.y - ORTHO_EPS : adjacent.y > endpoint.y + ORTHO_EPS;
}

function simplifyAdjacentEndpointRoute(pts, source, target, obstacleElements, clearancePx) {
  if (!Array.isArray(pts) || pts.length < 3) return pts;
  var start = getWpXY(pts[0]);
  var end = getWpXY(pts[pts.length - 1]);
  if (Math.abs(end.x - start.x) + Math.abs(end.y - start.y) > 260) return pts;
  ...
  var sourceCenterX = source.x + (source.width || 0) / 2;
  var sourceCenterY = source.y + (source.height || 0) / 2;
  var targetCenterX = target.x + (target.width || 0) / 2;
  var targetCenterY = target.y + (target.height || 0) / 2;
  var sourceBottom = source.y + (source.height || 0);
  var targetBottom = target.y + (target.height || 0);
  var sourceRight = source.x + (source.width || 0);
  var targetRight = target.x + (target.width || 0);

  if (target.y >= sourceBottom && Math.abs(sourceCenterX - targetCenterX) <= Math.max(source.width || 0, target.width || 0) / 2) {
    var downMidY = (sourceBottom + target.y) / 2;
    candidateSpecs.push([
      { x: sourceCenterX, y: sourceBottom },
      { x: sourceCenterX, y: downMidY },
      { x: targetCenterX, y: downMidY },
      { x: targetCenterX, y: target.y },
    ]);
  } else if (source.y >= targetBottom && Math.abs(sourceCenterX - targetCenterX) <= Math.max(source.width || 0, target.width || 0) / 2) {
    var upMidY = (targetBottom + source.y) / 2;
    candidateSpecs.push([
      { x: sourceCenterX, y: source.y },
      { x: sourceCenterX, y: upMidY },
      { x: targetCenterX, y: upMidY },
      { x: targetCenterX, y: targetBottom },
    ]);
  } else if (target.x >= sourceRight && Math.abs(sourceCenterY - targetCenterY) <= Math.max(source.height || 0, target.height || 0) / 2) {
    var rightMidX = (sourceRight + target.x) / 2;
    candidateSpecs.push([
      { x: sourceRight, y: sourceCenterY },
      { x: rightMidX, y: sourceCenterY },
      { x: rightMidX, y: targetCenterY },
      { x: target.x, y: targetCenterY },
    ]);
  } else if (source.x >= targetRight && Math.abs(sourceCenterY - targetCenterY) <= Math.max(source.height || 0, target.height || 0) / 2) {
    var leftMidX = (targetRight + source.x) / 2;
    candidateSpecs.push([
      { x: source.x, y: sourceCenterY },
      { x: leftMidX, y: sourceCenterY },
      { x: leftMidX, y: targetCenterY },
      { x: targetRight, y: targetCenterY },
    ]);
  }

  var candidates = candidateSpecs.map(function (candidate) {
    return withoutRedundantPoints(ensureStrictlyOrthogonalWaypoints(candidate));
  }).filter(function (candidate) {
    return (
      endpointLegLeavesShapeNormally(source, candidate[0], candidate[1]) &&
      endpointLegLeavesShapeNormally(
        target,
        candidate[candidate.length - 1],
        candidate[candidate.length - 2],
      ) &&
      countSequenceFlowObstacleHits(candidate, obstacleElements, clearancePx) === 0
    );
  });
...
function repairRemainingObstacleCrossings(pts, obstacleElements, clearancePx) {
  var next = cloneWaypoints(pts);
  var maxPasses = Math.max(4, obstacleElements.length * 2);
  for (var pass = 0; pass < maxPasses; pass++) {
    var currentHits = countSequenceFlowObstacleHits(next, obstacleElements, clearancePx);
    if (!currentHits) break;

    var candidates = [];
    for (var i = 0; i < next.length - 1 && !candidates.length; i++) {
      var a = getWpXY(next[i]);
      var b = getWpXY(next[i + 1]);
      for (var j = 0; j < obstacleElements.length; j++) {
        var rect = obstacleRectForSegmentHitTest(obstacleElements[j], clearancePx);
        if (!segmentIntersectsRect(a.x, a.y, b.x, b.y, rect)) continue;
        if (Math.abs(a.x - b.x) <= ORTHO_EPS) {
          [rect.x - 1, rect.x + rect.width + 1].forEach(function (x) {
            candidates.push(next.slice(0, i).concat([
              snapshotWaypoint(a),
              { x: x, y: a.y },
              { x: x, y: b.y },
              snapshotWaypoint(b),
        ...
        } else if (Math.abs(a.y - b.y) <= ORTHO_EPS) {
          [rect.y - 1, rect.y + rect.height + 1].forEach(function (y) {
            candidates.push(next.slice(0, i).concat([
              snapshotWaypoint(a),
              { x: a.x, y: y },
              { x: b.x, y: y },
    ...
    var ranked = candidates.map(function (candidate) {
      var compact = withoutRedundantPoints(ensureStrictlyOrthogonalWaypoints(candidate));
      var length = 0;
      for (var index = 1; index < compact.length; index++) {
        length += Math.abs(compact[index].x - compact[index - 1].x) +
          Math.abs(compact[index].y - compact[index - 1].y);
      }
      return {
        points: compact,
        hits: countSequenceFlowObstacleHits(compact, obstacleElements, clearancePx),
        length: length,
      };
    }).sort(function (left, right) {
      return left.hits - right.hits || left.length - right.length;
    });
    if (!ranked.length || ranked[0].hits >= currentHits) break;
    next = ranked[0].points;
  }
  return next;
}
/**
...
export function applyOrthogonalManhattanSequenceFlow(
  ...
  var obstacleContext =
    _connection && _elementRegistry ? createSequenceFlowObstacleContext(_connection, _elementRegistry) : null;
  var routeStart = getWpXY(pts[0]);
  var routeEnd = getWpXY(pts[pts.length - 1]);
  var isLocalClusterRange =
    Math.abs(routeEnd.x - routeStart.x) <= LOCAL_CLUSTER_MAX_DX_PX &&
    Math.abs(routeEnd.y - routeStart.y) <= LOCAL_CLUSTER_MAX_DY_PX;
  var routeMetrics = {
  ...
  if (_elementRegistry && _connection && !shortLink) {
    try {
      p1Candidates = getTopFacingCandidates(source, target, _elementRegistry, {
  ...
  var shouldRunP2Skeleton =
    ...
    (
      pts.length > 4 ||
      !sequenceFlowSegmentsAreOrthogonal(pts) ||
      (obstacleContext && obstacleContext.flowNodes.length > 0)
    );
  ...
  var resolveSourceSide = function () {
    ...
    var p1SourceSide = p1Best && p1Best.sourceExitSide
      ? (shortLink ? p1Best.sourceExitSide : preferNearColumnDockSide(source, target, p1Best.sourceExitSide))
      : null;
  ...
  var resolveTargetSide = function () {
    ...
    var p1TargetSide = p1Best && p1Best.targetEntrySide
      ? (shortLink ? p1Best.targetEntrySide : preferNearColumnDockSide(target, source, p1Best.targetEntrySide))
      : null;
  ...
  if (_connection && _elementRegistry && !isSequenceFlowImportLayoutActive()) {
    ...
    var runRepairLoop = function (startPass, maxPasses) {
      ...
      for (outerPass = startPass; outerPass < maxPasses; outerPass++) {
        ...
        var obstacleClean = !sequenceFlowWaypointsObstacleHits(
          pts,
          _connection,
          _elementRegistry,
          obstacleElements,
        ...
        resolveSequenceFlowWaypointObstacles(
          pts,
          _connection,
          _elementRegistry,
          obstacleElements,
        ...
        pts = ensureStrictlyOrthogonalWaypoints(pts);
        pts = simplifyLocalClusterWaypoints(
          pts,
          _connection,
          source,
          target,
    ...
    var maxOuterPasses = routeMetrics.selectedFromP2
      ? ROUTE_ENGINE_REDUCED_OUTER_PASSES
      : MAX_ORTHO_OBSTACLE_OUTER_PASSES;
  ...
  if (!_lockedFacingSides) {
    var finalSourceSide = p1Best && p1Best.sourceExitSide
      ? (shortLink ? p1Best.sourceExitSide : preferNearColumnDockSide(source, target, p1Best.sourceExitSide))
      : computeSequenceFacingSides(source, target, _horizontalLayout === true).sourceSide;
    var finalTargetSide = p1Best && p1Best.targetEntrySide
      ? (shortLink ? p1Best.targetEntrySide : preferNearColumnDockSide(target, source, p1Best.targetEntrySide))
  ...
  outPts = enforceFinalSourceExitClearance(outPts, source);
  outPts = enforceFinalTargetApproachClearance(outPts, target);
  outPts = withoutRedundantPoints(outPts);
  if (_connection && _elementRegistry && Array.isArray(finalObstacles)) {
    outPts = simplifyAdjacentEndpointRoute(
      outPts,
      source,
      target,
      finalObstacles,
    ...
    outPts = repairRemainingObstacleCrossings(
      outPts,
      finalObstacles,
      SEQUENCE_FLOW_OBSTACLE_SEGMENT_CLEARANCE_PX,
    );
    if (
      outPts.length >= 3 &&
      (
        !endpointLegLeavesShapeNormally(source, outPts[0], outPts[1]) ||
        !endpointLegLeavesShapeNormally(
          target,
          outPts[outPts.length - 1],
          outPts[outPts.length - 2],
      ...
      var endpointFacings = enumerateSequenceFlowFacingCandidates(
        source,
        target,
        _elementRegistry,
        { horizontalLayout: _horizontalLayout },
      );
      var endpointRepair = selectBestP2TemplateCandidate(
        source,
        target,
        _connection,
        endpointFacings,
      ...
      if (
        endpointRepair &&
        Array.isArray(endpointRepair.points) &&
        endpointRepair.points.length >= 2 &&
        endpointLegLeavesShapeNormally(
          source,
          endpointRepair.points[0],
          endpointRepair.points[1],
        ) &&
        endpointLegLeavesShapeNormally(
          target,
          endpointRepair.points[endpointRepair.points.length - 1],
          endpointRepair.points[endpointRepair.points.length - 2],
        )
      ) {
        outPts = endpointRepair.points;
      }
    }
  }
  outPts = enforceFinalSourceExitClearance(outPts, source);
  outPts = enforceFinalTargetApproachClearance(outPts, target);
  outPts = collapseSubpixelOrthogonalRails(outPts);
  outPts = withoutRedundantPoints(outPts);
...
// Keep automatic movement routing provisional until a stable layout phase.
const deferredSequenceFlowRouting = new WeakMap();
const freshLayoutConnections = new WeakSet();

export function deferSequenceFlowRouting(elementRegistry, options = {}) {
  let active = true;
  const pending = new Set();
  pending.fresh = options.fresh === true;
  const freshConnections = pending.fresh ? elementRegistry.getAll().filter(e => !e.labelTarget && e.source && e.target && e.businessObject?.$type === 'bpmn:SequenceFlow') : [];
  freshConnections.forEach(e => freshLayoutConnections.add(e));
  deferredSequenceFlowRouting.set(elementRegistry, pending);
  return {
    finish(modeling) {
      if (!active) return;
      active = false;
      getActiveObstacleSpatialIndex()?.rebuild(elementRegistry.getAll());
      try {
        const flows = elementRegistry.getAll().filter(element => !element.labelTarget && element.source && element.target && element.businessObject?.$type === 'bpmn:SequenceFlow');
        for (const connection of pending.fresh ? flows : pending) {
          // Boundary routing is explicitly completed by the layout's attachment pass.
          if (pending.fresh && [connection.source, connection.target].some(e => e.businessObject?.$type === 'bpmn:BoundaryEvent')) continue;
          modeling.layoutConnection(connection);
          const normalized = pending.finalize?.(connection);
          if (normalized) {
            const obstacles = createSequenceFlowObstacleContext(connection, elementRegistry).flowNodes;
            if (sequenceFlowSegmentsAreOrthogonal(normalized) &&
                !waypointsIntersectFlowObstacles(normalized, obstacles, SEQUENCE_FLOW_OBSTACLE_SEGMENT_CLEARANCE_PX) &&
                !candidateWorsensNodeNoRunBands(normalized, connection.waypoints, connection.source, connection.target, obstacles)) {
              modeling.updateWaypoints(connection, normalized);
            }
          }
        }
        if (pending.fresh) {
          for (const connection of flows) {
            const original = connection.waypoints || [];
            const extended = extendShortEndpointRails(original);
            if (JSON.stringify(original) === JSON.stringify(extended)) continue;
            const obstacles = createSequenceFlowObstacleContext(connection, elementRegistry).flowNodes;
            const oldNoRunScore = scorePathAgainstNodeNoRunBands(original, connection.source, connection.target, obstacles);
            if (!validFinalRouteCandidate(connection, extended, obstacles, oldNoRunScore) ||
                waypointsIntersectFlowObstacles(extended, obstacles, SEQUENCE_FLOW_OBSTACLE_SEGMENT_CLEARANCE_PX)) continue;
            const addsConflict = flows.some(other => {
              if (other === connection) return false;
              const pair = new SequenceFlowSegmentIndex([other]);
              return pair.intersects(connection, extended) && !pair.intersects(connection, original);
            });
            if (!addsConflict) modeling.updateWaypoints(connection, extended);
          }
        }
        const segmentIndex = new SequenceFlowSegmentIndex(flows);
        for (const connection of flows) {
          const conflict = points => segmentIndex.intersects(connection, points);
          if (!conflict(connection.waypoints || [])) continue;
          const primary = computeSequenceFacingSides(connection.source, connection.target,
            isDirectionHorizontal(connection.source, elementRegistry));
          const candidates = [];
          for (const sourceSide of FACING_AXIS_SIDES) {
            for (const targetSide of FACING_AXIS_SIDES) {
              candidates.push({
                sourceExitSide: sourceSide,
                targetEntrySide: targetSide,
                phase1Score: scoreFacingCandidatePhase1(sourceSide, targetSide, connection.source, connection.target, primary),
              });
            }
          }
          candidates.sort((a, b) => a.phase1Score - b.phase1Score);
          const alternative = selectBestP2TemplateCandidate(
            connection.source, connection.target, connection,
            candidates,
            createSequenceFlowObstacleContext(connection, elementRegistry),
            conflict,
          );
          if (alternative) {
            modeling.updateWaypoints(connection, alternative.points);
            segmentIndex.update(connection);
          }
        }
      } finally {
        freshConnections.forEach(e => freshLayoutConnections.delete(e));
        deferredSequenceFlowRouting.delete(elementRegistry);
        pending.clear();
      }
    },
    cancel() {
...
export function setSequenceFlowRoutingFinalizer(elementRegistry, finalize) {
  const pending = deferredSequenceFlowRouting.get(elementRegistry);
  if (pending?.fresh) pending.finalize = finalize;
}
export function markSequenceFlowRoutingComplete(connection, elementRegistry) {
  deferredSequenceFlowRouting.get(elementRegistry)?.delete(connection);
}
...
export function computeFinalSequenceFlowWaypoints(connection, waypoints, elementRegistry, options) {
  ...
  var horizontalLayout =
    typeof options.horizontalLayout === 'boolean'
      ? options.horizontalLayout
      : isDirectionHorizontal(source, elementRegistry);
  var pts = waypoints.map(snapshotWaypoint);

  // Auto-layout starts from geometry, not the route produced by the previous click.
  if (deferredSequenceFlowRouting.get(elementRegistry)?.fresh) {
    const facing = computeSequenceFacingSides(source, target, horizontalLayout);
    pts = [dockingPointOnShapeBoundary(source, facing.sourceExitSide, 0),
      dockingPointOnShapeBoundary(target, facing.targetEntrySide, 0)];
  }
  ...
  const finalWaypoints = applyOrthogonalManhattanSequenceFlow(
    pts,
    source,
    target,
    horizontalLayout,
  ...
  markSequenceFlowRoutingComplete(connection, elementRegistry);
...
function extendShortEndpointRails(points) {
  const out = cloneWaypoints(points);
  if (out.length < 4) return out;
  for (const reverse of [false, true]) {
    if (reverse) out.reverse();
    const [end, bend, next] = out;
    const dx = bend.x - end.x, dy = bend.y - end.y;
    const length = Math.abs(dx) + Math.abs(dy);
    if (length > 0 && length < EDGE_CLEARANCE_PX) {
      if (Math.abs(dy) < ORTHO_EPS && Math.abs(bend.x - next.x) < ORTHO_EPS) {
        const delta = Math.sign(dx) * (EDGE_CLEARANCE_PX - length);
        bend.x += delta;
        next.x += delta;
      } else if (Math.abs(dx) < ORTHO_EPS && Math.abs(bend.y - next.y) < ORTHO_EPS) {
        const delta = Math.sign(dy) * (EDGE_CLEARANCE_PX - length);
        bend.y += delta;
        next.y += delta;
      }
    }
...
function validFinalRouteCandidate(connection, points, obstacles, maxNoRunScore = 0) {
  const { source, target } = connection;
  if (!source || !target || points.length < 2) return false;
  const validLeg = (shape, endpoint, adjacent) => {
    const side = inferDockSideFromPointOnShape(shape, endpoint);
    const dx = adjacent.x - endpoint.x, dy = adjacent.y - endpoint.y;
    if (side === 'l') return Math.abs(dy) < 0.5 && dx <= -12;
    if (side === 'r') return Math.abs(dy) < 0.5 && dx >= 12;
    if (side === 't') return Math.abs(dx) < 0.5 && dy <= -12;
    if (side === 'b') return Math.abs(dx) < 0.5 && dy >= 12;
    return false;
  };
  if (!validLeg(source, points[0], points[1]) ||
      !validLeg(target, points[points.length - 1], points[points.length - 2])) return false;
  if (scorePathAgainstNodeNoRunBands(points, source, target, obstacles) > maxNoRunScore) return false;
  if (waypointsIntersectFlowObstacles(points.slice(1, -1), [source, target], 0)) return false;
  for (let parent = source.parent; parent; parent = parent.parent) {
    if (parent.businessObject?.$type !== 'bpmn:SubProcess') continue;
    for (let other = target.parent; other; other = other.parent) {
      if (other !== parent) continue;
      if (points.some(p => p.x < parent.x || p.y < parent.y ||
          p.x > parent.x + parent.width || p.y > parent.y + parent.height)) return false;
    }
  }
  return true;
}
...
SequenceFlowFinalLayouter.prototype.layoutSequenceFlowFinal = function (connection, hints) {
  ...
  const pending = deferredSequenceFlowRouting.get(this._elementRegistry);
  if (pending && hints && (hints.connectionStart || hints.connectionEnd)) {
