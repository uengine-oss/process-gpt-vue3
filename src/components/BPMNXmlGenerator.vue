...
export default {
    ...
    methods: {
        ...
        transformJsonModel(jsonModel) {
            ...
            console.log(
                '변환된 모델:',
                Array.isArray(transformedModel.elements) ? transformedModel.elements.length : Object.keys(transformedModel.elements).length,
                '개의 요소'
            );
        ...
        createActivity(element, data, laneMap, outMap, inMap, process, xmlDoc) {
            ...
            const task = xmlDoc.createElementNS('http://www.omg.org/spec/BPMN/20100524/MODEL', taskType);
        ...
        createGateway(element, laneMap, outMap, inMap, process, xmlDoc) {
            ...
            const gw = xmlDoc.createElementNS('http://www.omg.org/spec/BPMN/20100524/MODEL', gwType);
        ...
        createEvent(element, process, xmlDoc) {
            ...
            const evt = xmlDoc.createElementNS('http://www.omg.org/spec/BPMN/20100524/MODEL', evtTag);
        ...
        attachEventDefinition(evt, element, xmlDoc) {
            switch (element.eventType) {
                case 'Timer':
                    const timerDef = xmlDoc.createElementNS('http://www.omg.org/spec/BPMN/20100524/MODEL', 'bpmn:timerEventDefinition');
                    evt.appendChild(timerDef);
                    break;

                // 예시: Message
                case 'Message':
                    const messageDef = xmlDoc.createElementNS('http://www.omg.org/spec/BPMN/20100524/MODEL', 'bpmn:messageEventDefinition');
            ...
            if (jsonModel.elements) {
                ...
                displayableElements.forEach((element) => {
                    ...
                    if (element.x && element.y) {
                        ...
                        const elementWidth =
                            element.width || (element.elementType === 'Gateway' ? 50 : element.elementType === 'Event' ? 34 : 100);
                        const elementHeight =
                            element.height || (element.elementType === 'Gateway' ? 50 : element.elementType === 'Event' ? 34 : 80);
        ...
        createLaneShapesInAutoLayout(xmlDoc, bpmnPlane, jsonModel, isHorizontal) {
            ...
            jsonModel.roles.forEach((role, index) => {
                ...
                console.log(
                    `'${role.name}' 역할의 레인 모양 생성: x=${role.boundary.minX}, y=${role.boundary.minY}, width=${role.boundary.width}, height=${role.boundary.height}`
                );
        ...
        createSequenceEdges(xmlDoc, jsonModel, bpmnPlane, offsetPos, activityPos, isHorizontal) {
            ...
            sequences.forEach((sequence) => {
                ...
                if (sequence.waypoints) {
                    ...
                    let sourceDirection = this.determineDirection(sourceActivityPos, targetActivityPos, isHorizontal, 'source');
                    let targetDirection = this.determineDirection(sourceActivityPos, targetActivityPos, isHorizontal, 'target');
                    ...
                    this.createEdgeWaypoints(xmlDoc, bpmnEdge, startPosition, endPosition, isHorizontal);
                }
        ...
        createSequenceFlows(xmlDoc, jsonModel, process) {
            ...
            console.log(
                'createSequenceFlows 완료',
                Object.keys(inComing).length,
                '개의 incoming,',
                Object.keys(outGoing).length,
                '개의 outgoing 생성'
            );
