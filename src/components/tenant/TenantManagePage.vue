<template>
    <v-container v-if="!isLoading" class="bg-surface" style="height: 100%">
        ...
        <v-row no-gutters justify="center" class="mt-5">
            <v-card
                @click="!isNavigating ? toAddTenentPage() : null"
                elevation="9"
                variant="outlined"
                style="
                    padding: 10px;
        ...
        <div style="display: flex; justify-content: center; align-items: center">
            <div style="height: 450px; width: 500px; overflow-y: auto">
                <v-row no-gutters justify="center" class="mt-3" v-for="(tenantInfo, index) in tenantInfos" :key="index">
                    <v-card
                        @click="!isNavigating && !tenantInfo.isDeleted ? toSelectedTenantPage(tenantInfo.id) : null"
                        elevation="9"
                        variant="outlined"
                        style="
                            padding: 10px;
                    ...
                    >
                        <v-row class="pa-0 ma-0 align-center">
                            ...
                            <v-progress-circular
                                v-if="isNavigating && selectedTenantId === tenantInfo.id"
                                indeterminate
                                size="20"
                                width="2"
                                color="primary"
                            ...
                            <div v-else>
                                ...
                                <div v-if="tenantInfo.isOwned || tenantInfo.isAdmin">
                                    <v-tooltip text="수정" class="mr-2">
                                        <template v-slot:activator="{ props }">
                                            <v-btn
                                                @click.stop="!isNavigating ? toEditTenantPage(tenantInfo.id) : null"
                                                class="mr-2"
                                        ...
                                        <template v-slot:activator="{ props }">
                                            <v-btn
                                                @click.stop="
                                                    !isNavigating
                                                        ? ((deleteDialog = true),
                                                          (tenantIdToDelete = tenantInfo.id),
                                ...
                                <div v-if="tenantInfo.isDeleted && tenantInfo.isOwned">
                                    <v-btn
                                        @click.stop="restoreTenant(tenantInfo.id)"
                                        v-bind="props"
                                        color="error"
                                        variant="outlined"
                                        size="small"
    ...
    <v-dialog v-model="deleteDialog" max-width="500">
        <v-card>
            <v-row class="ma-0 pa-4 pb-0 align-center">
                <v-card-title class="pa-0 text-h4"> "{{ tenantIdToDelete }}" 회사 삭제 </v-card-title>
                <v-spacer></v-spacer>
                <v-btn
                    @click="
                        deleteDialog = false;
    ...
    <v-dialog v-model="restoreDialog" max-width="500">
        <v-card>
            <v-row class="ma-0 pa-4 pb-0 align-center">
                <v-card-title class="pa-0 text-h4"> 삭제 취소 </v-card-title>
                <v-spacer></v-spacer>
                <v-btn @click="restoreDialog = false" class="ml-auto" variant="text" density="compact" icon>
                    <v-icon>mdi-close</v-icon>
                </v-btn>
            </v-row>
            <v-card-text class="pa-4 pb-0">
