...
export default function AddLaneToPhaseHandler(commandStack, modeling, elementFactory, eventBus, selection) {
    ...
    function isPhaseContext(context) {
        ...
        while (parent) {
            if (parent.type === 'phase:PhaseContainer' || (parent.businessObject && parent.businessObject.$type === 'phase:PhaseContainer'))
                return true;
            parent = parent.parent;
        }
        const processRef =
            (shape.businessObject && shape.businessObject.processRef) ||
            (shape.parent && shape.parent.businessObject && shape.parent.businessObject.processRef);
    ...
    function addPhaseAndSetNewLane(context) {
        ...
        if (
            !phaseContainer ||
            (phaseContainer.type !== 'phase:PhaseContainer' && phaseContainer.businessObject?.$type !== 'phase:PhaseContainer')
        )
            return;
