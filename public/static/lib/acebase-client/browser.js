(function (f) {
    ...
    return (function () {
                                    ...
                                    this.cache = {
                                        enabled:
                                            typeof ((_a = settings.cache) === null || _a === void 0 ? void 0 : _a.db) !== 'object'
                                                ? false
                                                : typeof ((_b = settings.cache) === null || _b === void 0 ? void 0 : _b.enabled) ===
                                                  'boolean'
                                                ? settings.cache.enabled
                                                : true,
                                        db:
                                            typeof ((_c = settings.cache) === null || _c === void 0 ? void 0 : _c.db) === 'object'
                                                ? settings.cache.db
                                                : null,
                                        priority:
        ...
        */
                                    ...
                                    this.debug = new acebase_core_1.DebugLogger(
                                        settings.logLevel,
                                        `[${settings.dbname}]`.colorize(acebase_core_1.ColorStyle.blue)
                                    ); // `[ ${settings.dbname} ]`
                                    ...
                                    this.on('connect', () => {
                                        if (
                                            settings.sync.timing === 'connect' ||
                                            (settings.sync.timing === 'signin' && this.auth.accessToken)
                                        ) {
                                            syncPendingChanges();
                                    ...
                                    if (
                                        ...
                                        typeof (process === null || process === void 0 ? void 0 : process.on) === 'function'
                                    ) {
                                ...
                                get cache() {
                                    ...
                                    const get = async (path, cursor = null) => {
                                        ...
                                        const options = Object.freeze(
                                            cursor ? { cache_mode: 'allow', cache_cursor: cursor } : { cache_mode: 'force' }
                                        );
                ...
                { './api-web': 2, './auth': 3, './server-date': 12, _process: 40, 'acebase-core': 25 }
            ],
            2: [
                function (require, module, exports) {
                    'use strict';
                        ...
                        return new Promise((resolve, reject) => {
                            ...
                            const handle = (response) => {
                                if (response.req_id === requestId) {
                                    ...
                                    const message =
                                        typeof response.reason === 'object' ? response.reason.message : `request failed: ${code}`;
                                ...
                                socket.on('connect', async () => {
                                    ...
                                    if (this.accessToken) {
                                        ...
                                        try {
                                            ...
                                            this.debug.error(
                                                `Could not automatically sign in user with access token upon reconnect: ${
                                    ...
                                    const subscribeToMutatedEvents = async () => {
                                        ...
                                        const promises = Object.keys(this._subscriptions)
                                            .filter((path) =>
                                                this._subscriptions[path].some((sub) => sub.event === 'mutated' && sub.state !== 'canceled')
                                            )
                                            .filter(
                                                (path, i, arr) =>
                                                    !arr.some((otherPath) => acebase_core_1.PathInfo.get(otherPath).isAncestorOf(path))
                                            )
                ...
                */
                                    const causedByUs =
                                        ((_a = context.acebase_mutation) === null || _a === void 0 ? void 0 : _a.client_id) === this._id;
                                    ...
                                    if (updateCache) {
                                        if (data.path.startsWith('__')) {
                                            ...
                                            this.cache.db.api.set(
                                                acebase_core_1.PathInfo.getChildPath(`${this.dbname}/cache`, data.path),
                                                null,
                                                { suppress_events: !fireCacheEvents, context }
                                            ); // Remove cached value
                                        ...
                                        data.event === 'mutated'
                                            ? Object.keys(this._subscriptions)
                                                  .filter((path) => {
                                                      ...
                                                      return (
                                                          path === data.path ||
                                                          pathInfo.equals(data.subscr_path) ||
                                                          pathInfo.isAncestorOf(data.path)
                                                      );
                                                  })
                                                  .reduce((subs, path) => {
                        ...
                        async subscribe(path, event, callback, settings) {
                            ...
                            if (event === 'mutated') {
                                ...
                                Object.keys(this._subscriptions)
                                    ...
                                    .map((path) =>
                                        _websocketRequest(this.socket, 'unsubscribe', { path, event: 'mutated' }, this.accessToken)
                                    )
                        ...
                        async unsubscribe(path, event, callback) {
                            ...
                            if (!((_a = this.settings.network) === null || _a === void 0 ? void 0 : _a.realtime)) {
                                throw new Error(
                                    `Cannot unsubscribe from realtime events because it has been disabled in the network settings`
                                );
                            }
                            ...
                            if (pathSubs.length === 0) {
                                ...
                                promise = _websocketRequest(
                                    ...
                                    this.debug.error(`Failed to unsubscribe from event "${event}" on "${path}": ${err.message}`)
                                );
                            }
                            if (this.isConnected && hadMutatedEvents && !hasMutatedEvents) {
                                ...
                                const promises = Object.keys(this._subscriptions)
                                    ...
                                    .map((path) =>
                                        _websocketRequest(this.socket, 'subscribe', { path: path, event: 'mutated' }, this.accessToken)
                                    )
                                    .map((promise) =>
                                        promise.catch((err) =>
                                            this.debug.error(`Failed to subscribe to event "${event}" on path "${path}": ${err.message}`)
                                        )
                                    );
                            ...
                            return new Promise(async (resolve, reject) => {
                                ...
                                if (this.isConnected && ((_a = this.settings.network) === null || _a === void 0 ? void 0 : _a.realtime)) {
                                    ...
                                    try {
                                        ...
                                        if (
                                            ['ETIMEDOUT', 'ENOTFOUND', 'ECONNRESET', 'ECONNREFUSED', 'EPIPE', 'fetch_failed'].includes(
                                                err.code
                                            )
                                        ) {
                        ...
                        async _request(options) {
                            ...
                            if (this.isConnected || options.ignoreConnectionState === true) {
                                ...
                                if (
                                    !this.isConnecting ||
                                    !((_a = this.settings.network) === null || _a === void 0 ? void 0 : _a.realtime)
                                ) {
                        ...
                        async forgotPassword(email) {
                            ...
                            const result = await this._request({
                                method: 'POST',
                                url: `${this.url}/auth/${this.dbname}/forgot_password`,
                                data: { email }
                            });
                            return result;
                        }
                        ...
                        async signUp(details, signIn = true) {
                            ...
                            const result = await this._request({
                                method: 'POST',
                                url: `${this.url}/auth/${this.dbname}/signup`,
                                data: details
                            });
                            if (signIn) {
                                return this.handleSignInResult(result);
                        ...
                        async updateUserDetails(details) {
                            ...
                            const result = await this._request({
                                method: 'POST',
                                url: `${this.url}/auth/${this.dbname}/update`,
                                data: details
                            });
                        ...
                        async deleteAccount(uid, signOut = true) {
                            ...
                            const result = await this._request({
                                method: 'POST',
                                url: `${this.url}/auth/${this.dbname}/delete`,
                                data: { uid }
                            });
                            if (signOut) {
                            ...
                            try {
                                    ...
                                    try {
                                        ...
                                        for (const m of mutations) {
                                            ...
                                            try {
                                                ...
                                                cacheApi
                                                    .set(`${this.dbname}/stats/last_sync_error`, errorReport)
                                ...
                                if (this.hasCache) {
                                    ...
                                    const hasStaleValue = (sub) => {
                                        ...
                                        // |   yes  |  b/disct  |  no     |
                                        // --------------------------------
                                        const addedWhileOffline =
                                            sub.added > this._eventTimeline.disconnect && sub.added < this._eventTimeline.connect;
                                    ...
                                    strategy.reload = subscriptionPaths
                                        ...
                                        .reduce((reloadPaths, path) => {
                                            !reloadPaths.some((p) => p === path || acebase_core_1.PathInfo.get(p).isAncestorOf(path)) &&
                                    ...
                                    strategy.fallback = subscriptionPaths
                                        .filter(
                                            (path) =>
                                                !strategy.reload.some(
                                                    (p) => p === path || acebase_core_1.PathInfo.get(p).isAncestorOf(path)
                                                )
                                        )
                                        ...
                                        : subscriptionPaths
                                              ...
                                              .reduce((cursorItems, path) => {
                                                  ...
                                                  const events = subs
                                                      ...
                                                      .reduce(
                                                          (events, sub) => (events.includes(sub.event) || events.push(sub.event)) && events,
                                                          []
                                                      );
                                    ...
                                    strategy.warn = subscriptionPaths
                                        ...
                                        .reduce((warnItems, path) => {
                                            ...
                                            subs.forEach((sub) => {
                                                if (
                                                    typeof sub.settings.syncFallback === 'function' ||
                                                    sub.added > this._eventTimeline.connect
                                                ) {
                                                    strategy.noop.push(sub);
                                                } else if (
                                                    !strategy.cursor.some(
                                                        (item) => item.path === sub.path && item.events.includes(sub.event)
                                                    )
                                                ) {
                                    ...
                                    if (strategy.reload.length > 0) {
                                        this.debug.log(
                                            `SYNC reloading data for event paths ${strategy.reload.map((path) => `"/${path}"`).join(', ')}`
                                        );
                                        ...
                                        const fallbackPromise = (async () => {
                                            const promises = strategy.fallback.map(async (sub) => {
                                                ...
                                                try {
                                                    ...
                                                    this.debug.error(
                                                        `SYNC: error running fallback function for ${sub.event} on "/${sub.path}"`,
                                                        err
                                                    );
                        ...
                        async getMutations(filter) {
                            ...
                            const { data, context } = await this._request({
                                url: `${this.url}/sync/mutations/${this.dbname}?${query}`,
                                includeContext: true
                            });
                            const mutations = acebase_core_1.Transport.deserialize2(data);
                            return {
                                used_cursor: (_a = filter.cursor) !== null && _a !== void 0 ? _a : null,
                                new_cursor: context.acebase_cursor,
                                mutations
                            };
                        }
                        /**
                        ...
                        async getChanges(filter) {
                            ...
                            const query = Object.keys(filter)
                                ...
                                .join('&');
                            const { data, context } = await this._request({
                                url: `${this.url}/sync/changes/${this.dbname}?${query}`,
                                includeContext: true
                            });
                        ...
                        set(
                            ...
                            Promise.all([tryCachePromise, tryServerPromise]).then(([cacheResult, serverResult]) => {
                                ...
                                const networkError =
                                    ...
                                    ((_a = serverResult === null || serverResult === void 0 ? void 0 : serverResult.error) === null ||
                                    _a === void 0
                                        ? void 0
                                ...
                                if (serverPromise && !networkError) {
                                    ...
                                    addPendingTransaction().catch((err) => {
                                        this.debug.error(
                                            `Failed to add pending sync action for "${path}", rolling back cache to previous value. Error:`,
                                            err
                                        );
                                        rollbackCache().catch((err) => {
                                            this.debug.error(`Failed to roll back cache? Error:`, err);
                            ...
                            return ((_a = this._cache) === null || _a === void 0 ? void 0 : _a.priority) === 'cache'
                                ? cachePromise
                                : serverPromise;
                        }
                        update(
            ...
            */
                            ...
                            Promise.all([tryCachePromise, tryServerPromise]).then(([cacheResult, serverResult]) => {
                                const networkError =
                                    serverResult.executed && !serverResult.success && serverResult.error.isNetworkError === true;
                            ...
                            // return server promise by default, so caller can handle potential authorization issues
                            return ((_b = this._cache) === null || _b === void 0 ? void 0 : _b.priority) === 'cache'
                                ? cachePromise
                                : serverPromise;
                        }
                        /**
                        ...
                        async get(
                            ...
                            const getServerValue = async () => {
                                ...
                                if (this._cache) {
                                    ...
                                    if (!filtered) {
                                        ...
                                        this._cache.db.api
                                            .set(cachePath, value, {
                                                context: { acebase_operation: 'update_cache', acebase_server_context: context }
                                            })
                                            .catch((err) => {
                            ...
                            const getCacheValue = async (throwOnNull = false) => {
                                ...
                                const result = await this._cache.db.api.get(
                                    acebase_core_1.PathInfo.getChildPath(`${this.dbname}/cache`, path),
                                    options
                                );
                            ...
                            return new Promise((resolve, reject) => {
                                ...
                                const gotValue = (source, val) => {
                                    ...
                                    if (source === 'server') {
                                        ...
                                        if (!wait) {
                                            const serverError =
                                                (_a = errors.find((e) => e.source === 'server')) === null || _a === void 0
                                                    ? void 0
                                                    : _a.error;
                                ...
                                const gotError = (source, error) => {
                                    ...
                                    if (errors.length === 2) {
                                        ...
                                        reject(
                                            (_a = errors.find((e) => e.source === 'server')) === null || _a === void 0 ? void 0 : _a.error
                                        );
                                    }
                                };
                                getServerValue()
                        ...
                        callExtension(method, path, data) {
                            ...
                            if (data && !['PUT', 'POST'].includes(method)) {
                                ...
                                if (typeof data === 'object') {
                                    ...
                                    throw new Error(
                                        'data must be an object, or a string with query parameters, like "index=3&name=Something"'
                                    );
                                }
                                url += `?` + data;
                            }
                        ...
                        async query(
                            path,
                            query,
                            options = { snapshots: false, cache_mode: 'allow', monitor: { add: false, change: false, remove: false } }
                        ) {
                            const useCache =
                                this.hasCache && (options.cache_mode === 'force' || (options.cache_mode === 'allow' && !this.isConnected));
                            if (useCache) {
                                // Not connected, or "force" cache_mode: query cache db
                                const data = await this.cache.db.api.query(
                                    acebase_core_1.PathInfo.getChildPath(`${this.dbname}/cache`, path),
                                    query,
                                    options
                                );
                            ...
                            if (
                                ...
                                (typeof options.monitor === 'object' &&
                                    (options.monitor.add || options.monitor.change || options.monitor.remove))
                            ) {
                                console.assert(
                                    typeof options.eventHandler === 'function',
                                    `no eventHandler specified to handle realtime changes`
                                );
                                if (!this.socket) {
                                    throw new Error(
                        ...
                        async createIndex(path, key, options) {
                            if (options && options.config && Object.values(options.config).find((val) => typeof val === 'function')) {
                                throw new Error(
                                    `Cannot create an index with callback functions through a client. Move your code serverside`
                                );
                            }
                        ...
                        export(path, write, options = { format: 'json', type_safe: true }) {
                            ...
                            const url = `${this.url}/export/${this.dbname}/${path}?format=${options.format}&type_safe=${
                                options.type_safe ? 1 : 0
                            }`;
                        ...
                        import(path, read, options = { format: 'json', suppress_events: false }) {
                            ...
                            const url = `${this.url}/import/${this.dbname}/${path}?format=${options.format}&suppress_events=${
                                options.suppress_events ? 1 : 0
                            }`;
                    ...
                    class AceBaseClientAuth {
                        ...
                        async deleteAccount(uid) {
                            ...
                            if (uid && this.user.uid !== 'admin') {
                                throw {
                                    code: 'not_admin',
                                    message: 'Cannot remove other accounts than signed into account, unless you are admin'
                                };
                            }
...
*/
            ...
            7: [
                function (require, module, exports) {
                    ...
                    Object.defineProperty(exports, '__esModule', { value: true });
                    exports.Transport =
                        exports.PartialArray =
                        exports.proxyAccess =
                        exports.ID =
                        exports.ObjectCollection =
                        exports.TypeMappings =
                        exports.PathReference =
                        exports.EventSubscription =
                        exports.EventStream =
                        exports.DataReferencesArray =
                        exports.DataSnapshotsArray =
                        exports.DataSnapshot =
                        exports.DataReference =
                        exports.CachedValueUnavailableError =
                        exports.ServerDate =
                        exports.AceBaseUser =
                        exports.AceBaseClientAuth =
                        exports.AceBaseClient =
                            void 0;
                    /**
...
*/
                    ...
                    Object.defineProperty(exports, 'AceBaseClient', {
                        enumerable: true,
                        get: function () {
                            return acebase_client_1.AceBaseClient;
                        }
                    });
                    var auth_1 = require('./auth');
                    Object.defineProperty(exports, 'AceBaseClientAuth', {
                        enumerable: true,
                        get: function () {
                            return auth_1.AceBaseClientAuth;
                        }
                    });
                    var user_1 = require('./user');
                    Object.defineProperty(exports, 'AceBaseUser', {
                        enumerable: true,
                        get: function () {
                            return user_1.AceBaseUser;
                        }
                    });
                    var server_date_1 = require('./server-date');
                    Object.defineProperty(exports, 'ServerDate', {
                        enumerable: true,
                        get: function () {
                            return server_date_1.ServerDate;
                        }
                    });
                    var errors_1 = require('./errors');
                    Object.defineProperty(exports, 'CachedValueUnavailableError', {
                        enumerable: true,
                        get: function () {
                            return errors_1.CachedValueUnavailableError;
                        }
                    });
                    ...
                    Object.defineProperty(exports, 'DataReference', {
                        enumerable: true,
                        get: function () {
                            return acebase_core_1.DataReference;
                        }
                    });
                    Object.defineProperty(exports, 'DataSnapshot', {
                        enumerable: true,
                        get: function () {
                            return acebase_core_1.DataSnapshot;
                        }
                    });
                    Object.defineProperty(exports, 'DataSnapshotsArray', {
                        enumerable: true,
                        get: function () {
                            return acebase_core_1.DataSnapshotsArray;
                        }
                    });
                    Object.defineProperty(exports, 'DataReferencesArray', {
                        enumerable: true,
                        get: function () {
                            return acebase_core_1.DataReferencesArray;
                        }
                    });
                    Object.defineProperty(exports, 'EventStream', {
                        enumerable: true,
                        get: function () {
                            return acebase_core_1.EventStream;
                        }
                    });
                    Object.defineProperty(exports, 'EventSubscription', {
                        enumerable: true,
                        get: function () {
                            return acebase_core_1.EventSubscription;
                        }
                    });
                    Object.defineProperty(exports, 'PathReference', {
                        enumerable: true,
                        get: function () {
                            return acebase_core_1.PathReference;
                        }
                    });
                    Object.defineProperty(exports, 'TypeMappings', {
                        enumerable: true,
                        get: function () {
                            return acebase_core_1.TypeMappings;
                        }
                    });
                    Object.defineProperty(exports, 'ObjectCollection', {
                        enumerable: true,
                        get: function () {
                            return acebase_core_1.ObjectCollection;
                        }
                    });
                    Object.defineProperty(exports, 'ID', {
                        enumerable: true,
                        get: function () {
                            return acebase_core_1.ID;
                        }
                    });
                    Object.defineProperty(exports, 'proxyAccess', {
                        enumerable: true,
                        get: function () {
                            return acebase_core_1.proxyAccess;
                        }
                    });
                    Object.defineProperty(exports, 'PartialArray', {
                        enumerable: true,
                        get: function () {
                            return acebase_core_1.PartialArray;
                        }
                    });
                    Object.defineProperty(exports, 'Transport', {
                        enumerable: true,
                        get: function () {
                            return acebase_core_1.Transport;
                        }
                    });
                },
                { './acebase-client': 1, './auth': 3, './errors': 6, './server-date': 12, './user': 13, 'acebase-core': 25 }
            ],
            8: [
                function (require, module, exports) {
                    'use strict';
                            ...
                            if (supportsStreaming) {
                                ...
                                init.body = new ReadableStream({
                                    async pull(controller) {
                                        ...
                                        const chunk = await ((_a = options.dataRequestCallback) === null || _a === void 0
                                            ? void 0
                                            : _a.call(options, chunkSize));
                            ...
                            await new Promise((resolve, reject) => {
                                (async function readNext() {
                                    ...
                                    try {
                                        ...
                                        (_a = options.dataReceivedCallback) === null || _a === void 0
                                            ? void 0
                                            : _a.call(options, result === null || result === void 0 ? void 0 : result.value);
...
*/
                ...
                },
                {
                    './data-reference': 21,
                    './debug': 23,
                    './optional-observable': 27,
                    './simple-colors': 34,
                    './simple-event-emitter': 35,
                    './type-mappings': 38
                }
            ],
            15: [
            ...
            17: [
                function (require, module, exports) {
                    ...
                    const env = typeof window === 'object' ? window : self,
                        ...
                        mimeTypesLength =
                            (_b = (_a = navigator.mimeTypes) === null || _a === void 0 ? void 0 : _a.length) !== null && _b !== void 0
                                ? _b
                                : 0,
                        clientId = (0, pad_1.default)(
                            (mimeTypesLength + navigator.userAgent.length).toString(36) + globalCount.toString(36),
                            4
                        );
                ...
                function (require, module, exports) {
                        ...
                        static async create(ref, options) {
                            ...
                            const subscription = ref.on('mutations', { syncFallback }).subscribe(async (snap) => {
                                ...
                                const proceed = mutations.every((mutation) => {
                                    ...
                                    const changeSnap = new data_snapshot_1.DataSnapshot(
                                        changeRef,
                                        mutation.val,
                                        false,
                                        mutation.prev,
                            ...
                            const pushLocalMutations = async () => {
                                ...
                                for (let i = 0, m = mutationQueue[0]; i < mutationQueue.length; i++, m = mutationQueue[i]) {
                                    if (
                                        !transactions.find(
                                            (t) =>
                                                RelativeNodeTarget.areEqual(t.target, m.target) ||
                                                RelativeNodeTarget.isAncestor(t.target, m.target)
                                        )
                                    ) {
                                ...
                                process_1.default.nextTick(() => {
                                    ...
                                    mutations.forEach((mutation) => {
                                        ...
                                        const mutationSnap = new data_snapshot_1.DataSnapshot(
                                            mutationRef,
                                            mutation.value,
                                            false,
                                            mutation.previous,
                                    ...
                                    const snap = new data_snapshot_1.MutationsDataSnapshot(
                                        ref,
                                        mutations.map((m) => ({ target: m.target, val: m.value, prev: m.previous })),
                                        context
                                    );
                                ...
                                processPromise = mutations
                                        ...
                                        if (target.length === 0) {
                                            ...
                                            if (parentUpdate) {
                                                ...
                                                updates.push({
                                                    ref: parentRef,
                                                    target: parentTarget,
                                                    value: { [key]: cacheValue },
                                                    type: 'update',
                                                    previous: { [key]: prevValue }
                                                });
                                            }
                                        }
                                        return updates;
                                    ...
                                    .reduce(async (promise, update /*, i, updates */) => {
                                        ...
                                        await update.ref
                                            ...
                                            .catch((err) => {
                                                clientEventEmitter.emit('error', {
                                                    source: 'update',
                                                    message: `Error processing update of "/${ref.path}"`,
                                                    details: err
                                                });
                                                ...
                                                if (update.type === 'set') {
                                                    ...
                                                    Object.keys(update.previous).forEach((key) => {
                                                        ...
                                                        clientEventEmitter.emit('mutation', { snapshot: mutationSnap, isRemote: false });
                                                        mutations.push({
                                                            target: update.target.concat(key),
                                                            val: update.previous[key],
                                                            prev: update.value[key]
                                                        });
                                                    });
                                                }
                            ...
                            const addOnChangeHandler = (target, callback) => {
                                ...
                                const mutationsHandler = async (details) => {
                                    ...
                                    const context = snap.context();
                                    const causedByOurProxy =
                                        ((_a = context.acebase_proxy) === null || _a === void 0 ? void 0 : _a.id) === proxyId;
                                    if (details.origin === 'remote' && causedByOurProxy) {
                                        // Any local changes already triggered subscription callbacks
                                        console.error(
                                            'DEV ISSUE: mutationsHandler was called from remote event originating from our own proxy'
                                        );
                                        return;
                                    }
                                    ...
                                    if (singleMutation) {
                                        ...
                                        newValue = trailKeys.reduce(
                                            (val, key) => (!isObject(val) || !(key in val) ? null : val[key]),
                                            singleMutation.val
                                        );
                                        previousValue = trailKeys.reduce(
                                            (val, key) => (!isObject(val) || !(key in val) ? null : val[key]),
                                            singleMutation.prev
                                        );
                                    } else {
                                        ...
                                        mutations.forEach((mutation) => {
                                            ...
                                            for (let i = 0, val = newValue, prev = previousValue; i < trailKeys.length; i++) {
                                                // arr = PathInfo.getPathKeys(mutationPath).slice(PathInfo.getPathKeys(targetRef.path).length)
                                                const last = i + 1 === trailKeys.length,
                                                    key = trailKeys[i];
                                                if (last) {
                                    ...
                                    process_1.default.nextTick(() => {
                                        ...
                                        try {
                                            keepSubscription =
                                                false !==
                                                callback(Object.freeze(newValue), Object.freeze(previousValue), !causedByOurProxy, context);
                                        } catch (err) {
                                            clientEventEmitter.emit('error', {
                                                source: origin === 'remote' ? 'remote_update' : 'local_update',
                                                message: 'Error running subscription callback',
                                                details: err
                                            });
                                        }
                                ...
                                const stop = () => {
                                    ...
                                    clientSubscriptions.splice(
                                        clientSubscriptions.findIndex((cs) => cs.stop === stop),
                                        1
                                    );
                                };
                            ...
                            const handleFlag = (flag, target, args) => {
                                if (flag === 'write') {
                                    ...
                                    const hasConflictingTransaction = transactions.some(
                                        (t) =>
                                            RelativeNodeTarget.areEqual(target, t.target) ||
                                            RelativeNodeTarget.isAncestor(target, t.target) ||
                                            RelativeNodeTarget.isDescendant(target, t.target)
                                    );
                                    if (hasConflictingTransaction) {
                                        // TODO: Wait for this transaction to finish, then try again
                                        return Promise.reject(
                                            new Error('Cannot start transaction because it conflicts with another transaction')
                                        );
                                    }
                                    return new Promise(async (resolve) => {
                                        // If there are pending mutations on target (or deeper), wait until they have been synchronized
                                        const hasPendingMutations = mutationQueue.some(
                                            (m) =>
                                                RelativeNodeTarget.areEqual(target, m.target) ||
                                                RelativeNodeTarget.isAncestor(target, m.target)
                                        );
                                        ...
                                        tx.transaction = {
                                            ...
                                            get mutations() {
                                                return mutationQueue.filter(
                                                    (m) =>
                                                        RelativeNodeTarget.areEqual(tx.target, m.target) ||
                                                        RelativeNodeTarget.isAncestor(tx.target, m.target)
                                                );
                                            },
                                            get hasMutations() {
                            ...
                            const snap = await ref.get({
                                cache_mode: 'allow',
                                cache_cursor: options === null || options === void 0 ? void 0 : options.cursor
                            });
                            ...
                            if (
                                cache === null &&
                                typeof (options === null || options === void 0 ? void 0 : options.defaultValue) !== 'undefined'
                            ) {
                            ...
                            return {
                                async destroy() {
                                    await processPromise;
                                    const promises = [subscription.stop(), ...clientSubscriptions.map((cs) => cs.stop())];
                                    await Promise.all(promises);
                                ...
                                onMutation(callback) {
                                    ...
                                    clientEventEmitter.on('mutation', ({ snapshot, isRemote }) => {
                                        try {
                                            ...
                                            clientEventEmitter.emit('error', {
                                                source: 'mutation_callback',
                                                message: 'Error in dataproxy onMutation callback',
                                                details: err
                                            });
                                        }
                                    });
                        ...
                        const handler = {
                            get(target, prop, receiver) {
                                ...
                                if (target === null || typeof target !== 'object') {
                                    throw new Error(
                                        `Cannot read property "${prop}" of ${target}. Value of path "/${targetRef.path}" is not an object (anymore)`
                                    );
                                }
                                ...
                                const proxifyChildValue = (prop) => {
                                    ...
                                    const newChildProxy = createProxy({
                                        root: context.root,
                                        target: context.target.concat(prop),
                                        id: context.id,
                                        flag: context.flag
                                    });
                                ...
                                const unproxyValue = (value) => {
                                    return value !== null && typeof value === 'object' && value[isProxy] ? value.getTarget() : value;
                                };
                            ...
                            set(target, prop, value, receiver) {
                                ...
                                if (target === null || typeof target !== 'object') {
                                    throw new Error(
                                        `Cannot set property "${prop}" of ${target}. Value of path "/${targetRef.path}" is not an object`
                                    );
                                }
                                ...
                                if (value !== null) {
                                    ...
                                    if ((0, utils_1.valuesAreEqual)(value, target[prop])) {
                                        //if (compareValues(value, target[prop]) === 'identical') { // (typeof value !== 'object' && target[prop] === value) {
                                ...
                                if (context.target.some((key) => typeof key === 'number')) {
                                    ...
                                    context.flag(
                                        'write',
                                        context.target.slice(
                                            0,
                                            context.target.findIndex((key) => typeof key === 'number')
                                        )
                                    );
                    ...
                    function proxyAccess(proxiedValue) {
                        if (typeof proxiedValue !== 'object' || !proxiedValue[isProxy]) {
                            throw new Error(
                                'Given value is not proxied. Make sure you are referencing the value through the live data proxy.'
                            );
                        }
                        return proxiedValue;
                    }
                    ...
                    class OrderedCollectionProxy {
                        ...
                        add(item, index, from) {
                            ...
                            let minOrder = Number.POSITIVE_INFINITY,
                                maxOrder = Number.NEGATIVE_INFINITY;
            ...
            21: [
                function (require, module, exports) {
                    ...
                    exports.DataReferencesArray =
                        exports.DataSnapshotsArray =
                        exports.DataReferenceQuery =
                        exports.DataReference =
                        exports.QueryDataRetrievalOptions =
                        exports.DataRetrievalOptions =
                            void 0;
                    const data_snapshot_1 = require('./data-snapshot');
                    ...
                    class DataRetrievalOptions {
                        ...
                        constructor(options) {
                            ...
                            this.cache_mode =
                                typeof options.cache_mode === 'string'
                                    ? options.cache_mode
                    ...
                    class DataReference {
                        ...
                        async update(updates, onComplete) {
                            try {
                                ...
                                if (
                                    ...
                                    updates instanceof ArrayBuffer ||
                                    updates instanceof Date
                                ) {
                        ...
                        on(event, callback, cancelCallback) {
                                ...
                                ourCallback: (err, path, newValue, oldValue, eventContext) => {
                                    ...
                                    if (event.startsWith('notify_')) {
                                        ...
                                        if (event === 'child_removed') {
                                            callbackObject = new data_snapshot_1.DataSnapshot(
                                                ref,
                                                values.previous,
                                                true,
                                                values.previous,
                            ...
                            const subscribe = () => {
                                ...
                                const cancelSubscription = (err) => {
                                    ...
                                    this.db.debug.error(
                                        `Subscription "${event}" on path "/${this.path}" canceled because of an error: ${err.message}`
                                    );
                                    eventPublisher.cancel(err.message);
                                };
                                const authorized = this.db.api.subscribe(this.path, event, cb.ourCallback, {
                        ...
                        off(event, callback) {
                            ...
                            const stopSubs = subscriptions.filter(
                                (sub) => (!event || sub.event === event) && (!callback || sub.userCallback === callback)
                            );
                            if (stopSubs.length === 0) {
                                this.db.debug.warn(
                                    `Can't find event subscriptions to stop (path: "${this.path}", event: ${
                                        event || '(any)'
                                    }, callback: ${callback})`
                                );
                            }
                        ...
                        get(optionsOrCallback, callback) {
                            ...
                            const options = new DataRetrievalOptions(
                                typeof optionsOrCallback === 'object' ? optionsOrCallback : { cache_mode: 'allow' }
                            );
                            const promise = this.db.api.get(this.path, options).then((result) => {
                                ...
                                if (!isNewApiResult) {
                                    ...
                                    console.warn(
                                        'AceBase api.get method returned an old response value. Update your acebase or acebase-client package'
                                    );
                        ...
                        proxy(options) {
                            const isOptionsArg =
                                typeof options === 'object' &&
                                (typeof options.cursor !== 'undefined' || typeof options.defaultValue !== 'undefined');
                            if (typeof options !== 'undefined' && !isOptionsArg) {
                                this.db.debug.warn(
                                    'Warning: live data proxy is being initialized with a deprecated method signature. Use ref.proxy(options) instead of ref.proxy(defaultValue)'
                                );
                        ...
                        observe(options) {
                            ...
                            return new Observable((observer) => {
                                ...
                                const updateCache = (snap) => {
                                    ...
                                    const trailKeys = path_info_1.PathInfo.getPathKeys(mutatedPath).slice(
                                        path_info_1.PathInfo.getPathKeys(this.path).length
                                    );
                                    let target = cache;
                                    while (trailKeys.length > 1) {
                                        const key = trailKeys.shift();
                        ...
                        async getMutations(cursorOrDate) {
                            ...
                            const timestamp =
                                cursorOrDate === null || typeof cursorOrDate === 'undefined'
                                    ? 0
                                    : cursorOrDate instanceof Date
                                    ? cursorOrDate.getTime()
                                    : undefined;
                    ...
                    class DataReferenceQuery {
                        ...
                        filter(key, op, compare) {
                            if ((op === 'in' || op === '!in') && (!(compare instanceof Array) || compare.length === 0)) {
                                throw new Error(
                                    `${op} filter for ${key} must supply an Array compare argument containing at least 1 value`
                                );
                            }
                        ...
                        get(optionsOrCallback, callback) {
                            ...
                            callback =
                                typeof optionsOrCallback === 'function'
                                    ...
                                    ? callback
                                    : undefined;
                            const options = new QueryDataRetrievalOptions(
                                typeof optionsOrCallback === 'object' ? optionsOrCallback : { snapshots: true, cache_mode: 'allow' }
                            );
                ...
                function (require, module, exports) {
                    ...
                    class DebugLogger {
                        ...
                        setLevel(level) {
                            ...
                            this.write = (text) => {
                                const isRunKit =
                                    typeof process_1.default !== 'undefined' &&
                                    process_1.default.env &&
                                    typeof process_1.default.env.RUNKIT_ENDPOINT_PATH === 'string';
                                if (text && isRunKit) {
            ...
            25: [
                function (require, module, exports) {
                    ...
                    exports.ObjectCollection =
                        ...
                        exports.ID =
                        exports.DebugLogger =
                        exports.OrderedCollectionProxy =
                        exports.proxyAccess =
                        exports.MutationsDataSnapshot =
                        exports.DataSnapshot =
                        exports.DataReferencesArray =
                        exports.DataSnapshotsArray =
                        exports.QueryDataRetrievalOptions =
                        ...
                        exports.DataReference =
                        exports.Api =
                        exports.AceBaseBaseSettings =
                        exports.AceBaseBase =
                            void 0;
                    var acebase_base_1 = require('./acebase-base');
                    Object.defineProperty(exports, 'AceBaseBase', {
                        enumerable: true,
                        get: function () {
                            return acebase_base_1.AceBaseBase;
                        }
                    });
                    Object.defineProperty(exports, 'AceBaseBaseSettings', {
                        enumerable: true,
                        get: function () {
                            return acebase_base_1.AceBaseBaseSettings;
                        }
                    });
                    ...
                    Object.defineProperty(exports, 'DataReference', {
                        enumerable: true,
                        get: function () {
                            return data_reference_1.DataReference;
                        }
                    });
                    Object.defineProperty(exports, 'DataReferenceQuery', {
                        enumerable: true,
                        get: function () {
                            return data_reference_1.DataReferenceQuery;
                        }
                    });
                    Object.defineProperty(exports, 'DataRetrievalOptions', {
                        enumerable: true,
                        get: function () {
                            return data_reference_1.DataRetrievalOptions;
                        }
                    });
                    Object.defineProperty(exports, 'QueryDataRetrievalOptions', {
                        enumerable: true,
                        get: function () {
                            return data_reference_1.QueryDataRetrievalOptions;
                        }
                    });
                    Object.defineProperty(exports, 'DataSnapshotsArray', {
                        enumerable: true,
                        get: function () {
                            return data_reference_1.DataSnapshotsArray;
                        }
                    });
                    Object.defineProperty(exports, 'DataReferencesArray', {
                        enumerable: true,
                        get: function () {
                            return data_reference_1.DataReferencesArray;
                        }
                    });
                    ...
                    Object.defineProperty(exports, 'DataSnapshot', {
                        enumerable: true,
                        get: function () {
                            return data_snapshot_1.DataSnapshot;
                        }
                    });
                    Object.defineProperty(exports, 'MutationsDataSnapshot', {
                        enumerable: true,
                        get: function () {
                            return data_snapshot_1.MutationsDataSnapshot;
                        }
                    });
                    ...
                    Object.defineProperty(exports, 'proxyAccess', {
                        enumerable: true,
                        get: function () {
                            return data_proxy_1.proxyAccess;
                        }
                    });
                    Object.defineProperty(exports, 'OrderedCollectionProxy', {
                        enumerable: true,
                        get: function () {
                            return data_proxy_1.OrderedCollectionProxy;
                        }
                    });
                    ...
                    Object.defineProperty(exports, 'PathReference', {
                        enumerable: true,
                        get: function () {
                            return path_reference_1.PathReference;
                        }
                    });
                    ...
                    Object.defineProperty(exports, 'TypeMappings', {
                        enumerable: true,
                        get: function () {
                            return type_mappings_1.TypeMappings;
                        }
                    });
                    ...
                    Object.defineProperty(exports, 'SimpleCache', {
                        enumerable: true,
                        get: function () {
                            return simple_cache_1.SimpleCache;
                        }
                    });
                    ...
                    Object.defineProperty(exports, 'SimpleEventEmitter', {
                        enumerable: true,
                        get: function () {
                            return simple_event_emitter_1.SimpleEventEmitter;
                        }
                    });
                    ...
                    Object.defineProperty(exports, 'ColorStyle', {
                        enumerable: true,
                        get: function () {
                            return simple_colors_1.ColorStyle;
                        }
                    });
                    Object.defineProperty(exports, 'Colorize', {
                        enumerable: true,
                        get: function () {
                            return simple_colors_1.Colorize;
                        }
                    });
                    ...
                    Object.defineProperty(exports, 'SimpleObservable', {
                        enumerable: true,
                        get: function () {
                            return optional_observable_1.SimpleObservable;
                        }
                    });
                    ...
                    Object.defineProperty(exports, 'PartialArray', {
                        enumerable: true,
                        get: function () {
                            return partial_array_1.PartialArray;
                        }
                    });
                    ...
                    Object.defineProperty(exports, 'ObjectCollection', {
                        enumerable: true,
                        get: function () {
                            return object_collection_1.ObjectCollection;
                        }
                    });
                },
                {
                ...
                function (require, module, exports) {
                    ...
                    class PathInfo {
                        constructor(path) {
                            ...
                            this.path = this.keys.reduce(
                                (path, key, i) => (i === 0 ? `${key}` : typeof key === 'string' ? `${path}/${key}` : `${path}[${key}]`),
                                ''
                            );
                        }
                        static get(path) {
                            ...
                            if (typeof childKey === 'string') {
                                ...
                                keys.forEach((key) => {
                                    ...
                                    if (/[\x00-\x08\x0b\x0c\x0e-\x1f/[\]\\]/.test(key)) {
                                        throw new Error(
                                            `Invalid child key "${key}" for path "${this.path}". Keys cannot contain control characters or any of the following characters: \\ / [ ]`
                                        );
                                    }
                                    if (key.length > 128) {
                        ...
                        static fillVariables(varPath, fullPath) {
                            ...
                            const merged = keys.map((key, index) => {
                                if (key === pathKeys[index] || index >= pathKeys.length) {
                                    ...
                                    throw new Error(
                                        `Path "${fullPath}" cannot be used to fill variables of path "${varPath}" because they do not match`
                                    );
                                }
                            });
                            let mergedPath = '';
                ...
                function (require, module, exports) {
                    ...
                    function parse(definition) {
                        ...
                        function readProperty() {
                            ...
                            while (
                                ...
                                c === '_' ||
                                    ...
                                    (c >= 'a' && c <= 'z') ||
                                    (c >= 'A' && c <= 'Z') ||
                                    (prop.name.length > 0 && c >= '0' && c <= '9') ||
                                    (prop.name.length === 0 && c === '*'))
                            ) {
                                prop.name += c;
                                pos++;
                            }
                        ...
                        function readType() {
                            ...
                            if (name.length === 0) {
                                ...
                                ['string', 'number', 'boolean', 'bigint', 'undefined', 'String', 'Number', 'Boolean', 'BigInt'].includes(
                                    name
                                )
                            ) {
                    ...
                    function checkObject(path, properties, obj, partial) {
                        ...
                        if (invalidProperties.length > 0) {
                            return {
                                ok: false,
                                reason: `Object at path "${path}" cannot have propert${
                                    invalidProperties.length === 1 ? 'y' : 'ies'
                        ...
                        function checkProperty(property) {
                            ...
                            if (
                                hasValue &&
                                !property.types.some((type) => checkType(`${path}/${property.name}`, type, obj[property.name], false).ok)
                            ) {
                                return {
                                    ok: false,
                                    reason: `Property at path "${path}/${property.name}" does not match any of ${property.types.length} allowed types`
                                };
                            }
                            return { ok: true };
                        }
                    ...
                    function checkType(path, type, value, partial, trailKeys) {
                        ...
                        if (
                            ...
                            type.genericTypes &&
                            !value.every((v) => type.genericTypes.some((t) => checkType(path, t, v, false).ok))
                        ) {
                        ...
                        if (type.matches && !type.matches.test(value)) {
                            return {
                                ok: false,
                                reason: `path "${path}" must match regular expression /${type.matches.source}/${type.matches.flags}`
                            };
                        }
                        return ok;
                    }
                                        ...
                                        Object.keys(obj)
                                            .map((key) => {
                                                ...
                                                if (val === undefined) {
                                                    ...
                                                    throw new Error(
                                                        `Type definition for key "${key}" must be a string, number, boolean, bigint, object, regular expression, or one of these classes: String, Number, Boolean, Date, BigInt`
                                                    );
                                                }
                                                return `${key}:${val}`;
                                            })
                ...
                function (require, module, exports) {
                    ...
                    function ColorsSupported() {
                        ...
                        if (
                            ...
                            process_1.default.platform === 'browser'
                        ) {
                            return false;
                        }
                    ...
                    class SimpleEventEmitter {
                        ...
                        off(event, callback) {
                            this._subscriptions = this._subscriptions.filter(
                                (s) => s.event !== event || (callback && s.callback !== callback)
                            );
                            return this;
                        }
                        once(event, callback) {
                        ...
                        constructor(eventPublisherCallback) {
                            ...
                            this.subscribe = (callback, activationCallback) => {
                                ...
                                const sub = {
                                    ...
                                    activationCallback: function (activated, cancelReason) {
                                        activationCallback === null || activationCallback === void 0
                                            ? void 0
                            ...
                            const checkActiveSubscribers = () => {
                                ...
                                if (subscribers.length === 0) {
                                    ret =
                                        noMoreSubscribersCallback === null || noMoreSubscribersCallback === void 0
                                            ? void 0
                                            : noMoreSubscribersCallback();
                            ...
                            const cancel = (reason) => {
                                ...
                                subscribers.forEach((sub) => {
                                    var _a;
                                    (_a = sub.activationCallback) === null || _a === void 0
                                        ? void 0
                                        : _a.call(sub, false, reason || new Error('unknown reason'));
                                });
            ...
            37: [
                function (require, module, exports) {
                    'use strict';
                    Object.defineProperty(exports, '__esModule', { value: true });
                    exports.deserialize2 =
                        exports.serialize2 =
                        exports.serialize =
                        exports.detectSerializeVersion =
                        exports.deserialize =
                            void 0;
                    const path_reference_1 = require('./path-reference');
                    const utils_1 = require('./utils');
...
*/
                    ...
                    const serialize = (obj) => {
                        ...
                        if (
                            ...
                            obj instanceof path_reference_1.PathReference ||
                ...
                { './ascii85': 16, './partial-array': 28, './path-info': 29, './path-reference': 30, './utils': 39 }
            ],
            38: [
                function (require, module, exports) {
                    'use strict';
                    ...
                    function process(db, mappings, path, obj, action) {
                        ...
                        const changes = [];
                        m.sort((a, b) =>
                            path_info_1.PathInfo.getPathKeys(a.path).length > path_info_1.PathInfo.getPathKeys(b.path).length ? -1 : 1
                        ); // Deepest paths first
                        m.forEach((mapping) => {
                        ...
                        bind(path, type, options = {}) {
                            ...
                            if (typeof options.serializer === 'undefined') {
                                ...
                                if (typeof type.prototype[options.serializer] === 'function') {
                                    ...
                                    throw new TypeError(
                                        `${type.name}.prototype.${options.serializer} is not a function, cannot use it as serializer`
                                    );
                                }
                            } else if (typeof options.serializer !== 'function') {
                                throw new TypeError(
                                    `serializer for class ${type.name} must be a function, or the name of a prototype method`
                                );
                            }
                ...
                { './data-reference': 21, './data-snapshot': 22, './path-info': 29, './utils': 39 }
            ],
            39: [
                function (require, module, exports) {
                    (function (global, Buffer) {
                        (function () {
                            ...
                            exports.getGlobalObject =
                                exports.defer =
                                exports.getChildValues =
                                exports.getMutations =
                                exports.compareValues =
                                exports.ObjectDifferences =
                                exports.valuesAreEqual =
                                exports.cloneObject =
                                exports.concatTypedArrays =
                            ...
                            function cloneObject(original, stack) {
                                var _a;
                                if (
                                    ((_a = original === null || original === void 0 ? void 0 : original.constructor) === null ||
                                    _a === void 0
                                        ? void 0
                                        : _a.name) === 'DataSnapshot'
                                ) {
                                ...
                                if (
                                    ...
                                    original instanceof path_reference_1.PathReference ||
                                ...
                                const cloneValue = (val) => {
                                    ...
                                    if (
                                        ...
                                        val instanceof path_reference_1.PathReference ||
                                ...
                                const clone =
                                    original instanceof Array
                                        ? []
                                        : original instanceof partial_array_1.PartialArray
                                        ? new partial_array_1.PartialArray()
                                        : {};
                            ...
                            function valuesAreEqual(val1, val2) {
                                ...
                                if (typeof val1 === 'object' || typeof val2 === 'object') {
                                    ...
                                    if (val1 instanceof path_reference_1.PathReference || val2 instanceof path_reference_1.PathReference) {
                                        return (
                                            val1 instanceof path_reference_1.PathReference &&
                                            val2 instanceof path_reference_1.PathReference &&
                                            val1.path === val2.path
                                        );
                                    }
                                    ...
                                    if (val1 instanceof Array || val2 instanceof Array) {
                                        return (
                                            ...
                                            val1.every((item, i) => valuesAreEqual(val1[i], val2[i]))
                                        );
                                    }
                                    ...
                                    return (
                                        ...
                                        keys1.every((key) => valuesAreEqual(val1[key], val2[key]))
                                    );
                                }
                                return false;
                            }
                            ...
                            function compareValues(oldVal, newVal, sortedResults = false) {
                                ...
                                if (oldVal === newVal) {
                                    ...
                                    return typed1.byteLength === typed2.byteLength && typed1.every((val, i) => typed2[i] === val)
                                        ? 'identical'
                                        : 'changed';
                                } else if (oldVal instanceof Date || newVal instanceof Date) {
                                    return oldVal instanceof Date && newVal instanceof Date && oldVal.getTime() === newVal.getTime()
                                        ? 'identical'
                                        : 'changed';
                                } else if (
                                    oldVal instanceof path_reference_1.PathReference ||
                                    newVal instanceof path_reference_1.PathReference
                                ) {
                                    return oldVal instanceof path_reference_1.PathReference &&
                                        newVal instanceof path_reference_1.PathReference &&
                                        oldVal.path === newVal.path
                                        ? 'identical'
                                        : 'changed';
                                    ...
                                    if (addedKeys.length === 0 && removedKeys.length === 0 && changedKeys.length === 0) {
                                        ...
                                        return new ObjectDifferences(
                                            addedKeys,
                                            removedKeys,
                                            sortedResults ? changedKeys.sort((a, b) => (a.key < b.key ? -1 : 1)) : changedKeys
                                        );
                                    }
                                }
                                return 'changed';
                            ...
                            function getMutations(oldVal, newVal, sortedResults = false) {
                                const process = (target, compareResult, prev, val) => {
                                    switch (compareResult) {
                                        ...
                                        default: {
                                            let changes = [];
                                            compareResult.added.forEach((key) =>
                                                changes.push({ target: target.concat(key), prev: null, val: val[key] })
                                            );
                                            compareResult.removed.forEach((key) =>
                                                changes.push({ target: target.concat(key), prev: prev[key], val: null })
                                            );
                                            compareResult.changed.forEach((item) => {
                                                const childChanges = process(
                                                    target.concat(item.key),
                                                    item.change,
                                                    prev[item.key],
                                                    val[item.key]
                                                );
                ...
                { './partial-array': 28, './path-reference': 30, './process': 31, buffer: 40 }
            ],
                            ...
                            !(function (t, e) {
                                ...
                                return (function (t) {
                                    ...
                                    function (t, e) {
                                        ...
                                        'undefined' != typeof Blob && ((n.prototype = Blob.prototype), (o.prototype = Blob.prototype)),
                                            (t.exports = (function () {
