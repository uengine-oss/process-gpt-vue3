            ...
            onOk: function () {
                ...
                if (b.info) {
                    ...
                    if (!this._.selectedElement)
                        for (
                            var h = c.append(n('tbody')), f = parseInt(b.txtRows, 10) || 0, g = parseInt(b.txtCols, 10) || 0, k = 0;
                            k < f;
                            k++
                        )
                    ...
                    if (null !== c.$.tHead && 'row' != f && 'both' != f) {
                        ...
                        for (m = h.getFirst(); 0 < l.getChildCount(); ) {
                            h = l.getFirst();
                            for (k = 0; k < h.getChildCount(); k++)
                                (g = h.getChild(k)), g.type == CKEDITOR.NODE_ELEMENT && (g.renameNode('td'), g.removeAttribute('scope'));
                            h.insertBefore(m);
                        }
                        l.remove();
                    }
                    if (!this.hasColumnHeaders && ('col' == f || 'both' == f))
                        for (l = 0; l < c.$.rows.length; l++)
                            (g = new CKEDITOR.dom.element(c.$.rows[l].cells[0])), g.renameNode('th'), g.setAttribute('scope', 'row');
                    if (this.hasColumnHeaders && 'col' != f && 'both' != f)
                        for (k = 0; k < c.$.rows.length; k++)
                            (l = new CKEDITOR.dom.element(c.$.rows[k])),
                                'tbody' == l.getParent().getName() &&
                                    ((g = new CKEDITOR.dom.element(l.$.cells[0])), g.renameNode('td'), g.removeAttribute('scope'));
