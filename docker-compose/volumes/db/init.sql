...
create table if not exists public.credit_purchase (
    ...
    tenant_id TEXT NOT NULL REFERENCES tenants(id),   -- 테넌트
    ...
    payment_id UUID REFERENCES payment(id),           -- 결제 ID 정보(구매자 추적용)
    expires_at TIMESTAMP WITH TIME ZONE,              -- 만료일(생성일 기준 + validity_months)
    created_at TIMESTAMP WITH TIME ZONE DEFAULT NOW() -- 생성일(자동생성)
);
...
create table if not exists public.credit_usage (
    ...
    tenant_id TEXT NOT NULL REFERENCES tenants(id),                   -- 테넌트ID
    usage_id UUID NOT NULL REFERENCES usage(id),                      -- 사용량 ID
    credit_purchase_id UUID REFERENCES credit_purchase(id),           -- 연결된 구매 크레딧 ID
    used_credit DECIMAL(12,7) NOT NULL,                               -- 실제로 이만큼 소진
    created_at TIMESTAMP WITH TIME ZONE DEFAULT NOW()                 -- 생성 날짜
);

-- service(개별 서비스 식별)
create table if not exists public.service (
    id          TEXT       PRIMARY KEY,                      -- 서비스 ID
    name        TEXT,                                        -- 서비스 이름
    created_at  TIMESTAMP WITH TIME ZONE DEFAULT NOW(),      -- 생성일
    tenant_id   TEXT       REFERENCES public.tenants(id),    -- 테넌트

    CONSTRAINT uq_service_id_tenant UNIQUE(id, tenant_id)    -- id+tenant 단위 유니크
);	
...
create table if not exists public.usage (
    ...
    service_rate_id TEXT,                                 -- 트리거: 해당 시점의 가격
    group_id TEXT, 									      -- 연결된 사용량 ID
    ...
    usage_start_at TIMESTAMP WITH TIME ZONE NOT NULL,     -- 사용 시작
    usage_end_at TIMESTAMP WITH TIME ZONE DEFAULT NOW()   -- 사용 종료(자동 생성)
);
...
CREATE INDEX idx_usage_tenant_master_date
    ON public.usage (tenant_id, service_master_id, usage_start_at);
...
create table if not exists public.payment (
    ...
    approved_at TIMESTAMP,                      -- 결제 완료 시간
